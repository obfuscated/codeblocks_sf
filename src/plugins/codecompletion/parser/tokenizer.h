...
public:
    ...
    inline unsigned int GetNestingLevel() const
    {
        return m_NestLevel;
    };
...
protected:
    ...
    inline bool MoveToNextChar(const unsigned int amount = 1)
    {
        assert(amount);
...
private:
    inline bool CharInString(const char ch, const char* chars) const
    {
