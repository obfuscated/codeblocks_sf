...
WX_DECLARE_HASH_MAP(long, wxMemoryOutputStream*, wxIntegerHash, wxIntegerEqual, PageHashMap);

/// Hashmap class for boolean values
WX_DECLARE_HASH_MAP(long, bool, wxIntegerHash, wxIntegerEqual, BoolHashMap);

/// Hashmap class for document links
WX_DECLARE_HASH_MAP(long, wxPdfLink*, wxIntegerHash, wxIntegerEqual, LinkHashMap);

/// Hashmap class for page links
WX_DECLARE_HASH_MAP(long, wxArrayPtrVoid*, wxIntegerHash, wxIntegerEqual, PageLinksMap);
...
WX_DECLARE_STRING_HASH_MAP(wxPdfFont*, FontHashMap);

/// Hash map class for embedded images
WX_DECLARE_STRING_HASH_MAP(wxPdfImage*, ImageHashMap);
...
// TODO: Constructor with explicit papersize
  ...
  * \param lineStyle: Line style of polygon. \see SetLineStyle()
  ...
  */
  virtual void MultiCell(double w, double h, const wxString& txt,
                         int border = 0, int align = wxPDF_ALIGN_JUSTIFY, int fill = 0);
...
protected:
  ...
  /// Format a floating point with a fixed precision
...
private:
  ...
  wxArrayInt     m_offsets;            ///< array of object offsets

  wxMemoryOutputStream m_buffer;       ///< buffer holding in-memory PDF
  PageHashMap*   m_pages;              ///< array containing pages
  ...
  BoolHashMap*   m_orientationChanges; ///< array indicating orientation changes
  ...
  DiffHashMap*   m_diffs;              ///> array of encoding differences
