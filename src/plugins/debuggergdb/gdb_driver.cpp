...
void GDB_driver::InitializeScripting()
{
    ...
    Sqrat::Class<GDB_driver> gdb_driver(vm->GetVM(),"GDB_driver");
    gdb_driver.Func("RegisterType",&GDB_driver::RegisterType);
    Sqrat::RootTable(vm->GetVM()).Bind("GDB_driver",gdb_driver);
    ...
    if (!script.IsEmpty())
    {
        ...
        Sqrat::Function func = Sqrat::RootTable(vm->GetVM()).GetFunction("RegisterTypes");
        if(func.IsNull())
        {
        // A error occur
        } else {
