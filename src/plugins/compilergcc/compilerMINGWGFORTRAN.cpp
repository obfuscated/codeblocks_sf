...
CompilerMINGWGFORTRAN::CompilerMINGWGFORTRAN(const wxString& name, const wxString& ID)
    : Compiler(name, ID)
{
    Reset();
}

CompilerMINGWGFORTRAN::~CompilerMINGWGFORTRAN()
{
    //dtor
}

Compiler * CompilerMINGWGFORTRAN::CreateCopy()
{
    Compiler* c = new CompilerMINGWGFORTRAN(*this);
    c->SetExtraPaths(m_ExtraPaths); // wxArrayString doesn't seem to be copied with the default copy ctor...
    return c;
}

CompilerCommandGenerator* CompilerMINGWGFORTRAN::GetCommandGenerator()
{
    return new CompilerMINGWGenerator;
}

void CompilerMINGWGFORTRAN::Reset()
{
    if (platform::windows)
    {
...
void CompilerMINGWGFORTRAN::LoadDefaultRegExArray()
{
    m_RegExes.Clear();
    ...
    m_RegExes.Add(RegExStruct(_("FortranInfo"), cltInfo, _T("(.*):([0-9]+)(.*):.*"), 1, 1, 2, 2, 3));
    m_RegExes.Add(RegExStruct(_("FortranError"), cltError, _T(".*([Ee]rror.*)"), 1));
}

AutoDetectResult CompilerMINGWGFORTRAN::AutoDetectInstallationDir()
{
...
void CompilerMINGWGFORTRAN::SetVersionString()
{
