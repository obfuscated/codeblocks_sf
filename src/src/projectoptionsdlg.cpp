...
BEGIN_EVENT_TABLE(ProjectOptionsDlg, wxScrollingDialog)
    ...
    EVT_BUTTON(XRCID("btnBrowseImportLibraryFilename"),
               ProjectOptionsDlg::OnBrowseImportLibraryFilenameClick)
    EVT_BUTTON(XRCID("btnBrowseDefinitionFileFilename"),
               ProjectOptionsDlg::OnBrowseDefinitionFileFilenameClick)
...
ProjectOptionsDlg::ProjectOptionsDlg(wxWindow *parent, cbProject *project)
  : m_Project(project), m_Current_Sel(-1), m_pCompiler(nullptr)
{
    wxXmlResource::Get()->LoadObject(this, parent, _T("dlgProjectOptions"),
    ...
    XRCCTRL(*this, "txtPlatformProj", wxTextCtrl)
        ->SetValue(GetStringFromPlatforms(m_Project->GetPlatforms()));
    ...
    XRCCTRL(*this, "chkExtendedObjNames", wxCheckBox)
        ->SetValue(m_Project->GetExtendedObjectNamesGeneration());
    ...
    XRCCTRL(*this, "chkCheckFiles", wxCheckBox)
        ->SetValue(m_Project->GetCheckForExternallyModifiedFiles());
    ...
    XRCCTRL(*this, "txtNotes", wxTextCtrl)
        ->Connect(wxEVT_KEY_DOWN, wxKeyEventHandler(ProjectOptionsDlg::OnKeyDown), nullptr, this);
...
ProjectOptionsDlg::~ProjectOptionsDlg()
{
    ...
    XRCCTRL(*this, "txtNotes", wxTextCtrl)
        ->Disconnect(wxEVT_KEY_DOWN, wxKeyEventHandler(ProjectOptionsDlg::OnKeyDown), nullptr,
                     this);
}
...
void ProjectOptionsDlg::AddPluginPanels()
{
    ...
    Manager::Get()->GetPluginManager()->GetProjectConfigurationPanels(nb, m_Project,
                                                                      m_PluginPanels);
    ...
    if (cmb && chkCR && txt && browse)
    {
        ...
        switch ((TargetType)cmb->GetSelection())
        {
            ...
            case ttStaticLib:
                ...
                txtO->Enable(true);
                browse->Enable(true);
                browseI->Enable(chkSL->IsChecked()
                                && (TargetType)cmb->GetSelection() == ttDynamicLib);
                browseD->Enable(chkCD->IsChecked()
                                && (TargetType)cmb->GetSelection() == ttDynamicLib);
...
void ProjectOptionsDlg::DoBeforeTargetChange(bool force)
{
    ...
    if (force || lstTargets->GetSelection() != m_Current_Sel)
    {
        ...
        target->SetTargetFilenameGenerationPolicy(
            XRCCTRL(*this, "chkAutoGenPrefix", wxCheckBox)->GetValue() ? tgfpPlatformDefault
                                                                       : tgfpNone,
            XRCCTRL(*this, "chkAutoGenExt", wxCheckBox)->GetValue() ? tgfpPlatformDefault
                                                                    : tgfpNone);

        target->SetTargetType(
            TargetType(XRCCTRL(*this, "cmbProjectType", wxComboBox)->GetSelection()));
...
void ProjectOptionsDlg::OnProjectTypeChanged(cb_unused wxCommandEvent &event)
{
    ...
    wxString libextI = compiler ? compiler->GetSwitches().libExtension
                                : _T(""); // TODO: add specialized compiler option for this
    ...
    wxString libpreI = compiler ? compiler->GetSwitches().libPrefix
                                : _T(""); // TODO: add specialized compiler option for this
    wxString libpreD = compiler ? compiler->GetSwitches().libPrefix
                                : _T(""); // TODO: add specialized compiler option for this
...
{
    wxString targetName =
        cbGetTextFromUser(_("Enter the new build target name:"), _("New build target"));
    ...
    if (!target)
    {
        cbMessageBox(_("The new target could not be added..."), _("Error"), wxOK | wxICON_ERROR,
                     this);
        return;
    }
...
void ProjectOptionsDlg::OnCopyBuildTargetClick(cb_unused wxCommandEvent &event)
{
    ...
    wxString newTargetName =
        cbGetTextFromUser(_("Enter the duplicated build target's name:"),
                          _("Duplicate build target"), _("Copy of ") + target->GetTitle());
...
void ProjectOptionsDlg::OnRemoveBuildTargetClick(cb_unused wxCommandEvent &event)
{
    ...
    if (targetIdx == wxNOT_FOUND)
    {
        Manager::Get()->GetLogManager()->Log(
            _("No target removed, because nothing is selected in the Build target list box!"));
        return;
    }

    wxString caption;
    caption.Printf(_("Are you sure you want to delete build target \"%s\"?"),
                   lstTargets->GetStringSelection().c_str());
    if (cbMessageBox(caption, _("Confirmation"), wxYES_NO | wxNO_DEFAULT | wxICON_QUESTION, this)
        == wxID_NO)
        return;
...
void ProjectOptionsDlg::OnExportTargetClick(cb_unused wxCommandEvent &event)
{
    ...
    if (dlg.ShowModal() == AnnoyingDialog::rtYES)
    {
        if (m_Project->ExportTargetAsProject(target->GetTitle()))
            cbMessageBox(_("New project created successfully!"), _("Information"),
...
void ProjectOptionsDlg::OnBrowseDirClick(wxCommandEvent &event)
{
    ...
    wxString path = ChooseDirectory(this, _("Select directory"),
                                    fname.GetPath(wxPATH_GET_VOLUME | wxPATH_GET_SEPARATOR),
                                    m_Project->GetBasePath(), true, true);
    if (path.IsEmpty())
        return;
...
void ProjectOptionsDlg::OnBrowseOutputFilenameClick(cb_unused wxCommandEvent &event)
{
    ...
    fname.Normalize(wxPATH_NORM_ALL & ~wxPATH_NORM_CASE, m_Project->GetBasePath());
    wxFileDialog dlg(this, _("Select output filename"), fname.GetPath(), fname.GetFullName(),
...
void ProjectOptionsDlg::OnBrowseImportLibraryFilenameClick(cb_unused wxCommandEvent &event)
{
    ...
    fname.Assign(XRCCTRL(*this, "txtImportLibraryFilename", wxTextCtrl)->GetValue());
    fname.Normalize(wxPATH_NORM_ALL & ~wxPATH_NORM_CASE, m_Project->GetBasePath());
    wxFileDialog dlg(this, _("Select import library filename"), fname.GetPath(),
                     fname.GetFullName(), FileFilters::GetFilterAll(),
...
void ProjectOptionsDlg::OnBrowseDefinitionFileFilenameClick(cb_unused wxCommandEvent &event)
{
    ...
    fname.Assign(XRCCTRL(*this, "txtDefinitionFileFilename", wxTextCtrl)->GetValue());
    fname.Normalize(wxPATH_NORM_ALL & ~wxPATH_NORM_CASE, m_Project->GetBasePath());
    wxFileDialog dlg(this, _("Select definition file filename"), fname.GetPath(),
                     fname.GetFullName(), FileFilters::GetFilterAll(),
                     wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
...
bool ProjectOptionsDlg::IsScriptValid(ProjectBuildTarget *target, const wxString &script)
{
    ...
    try
    {
        ...
        script_nomacro = wxFileName(script_nomacro).IsAbsolute()
                             ? script_nomacro
                             : m_Project->GetBasePath() + wxFILE_SEP_PATH + script_nomacro;
        Manager::Get()->GetScriptingManager()->LoadBuffer(
            clearout_buildscripts); // clear previous script's context
...
bool ProjectOptionsDlg::ValidateTargetName(const wxString &name)
{
    ...
    {
        cbMessageBox(_("A target with this name already exists in this project!"), _("Error"),
...
void ProjectOptionsDlg::OnAddScript(cb_unused wxCommandEvent &event)
{
    ...
    EditPathDlg dlg(this, fname.GetFullName(),
                    wxEmptyString, // fname.GetPath(),
                    _("Add script(s)"), _("Select script file(s)"), false, true,
    ...
    for (size_t i = 0; i < paths.GetCount(); ++i)
    {
        ...
        CompileOptionsBase *base = sel == tc->GetRootItem()
                                       ? static_cast<CompileOptionsBase *>(m_Project)
                                       : static_cast<CompileOptionsBase *>(
                                             m_Project->GetBuildTarget(tc->GetItemText(sel)));
...
void ProjectOptionsDlg::OnPlatform(wxCommandEvent &event)
{
    ...
    MultiSelectDlg dlg(this, arr, isAll, _("Select supported platforms:"),
                       _("Build target platforms"));
    if (!isAll)
    {
...
void ProjectOptionsDlg::OnUpdateUI(cb_unused wxUpdateUIEvent &event)
{
    ...
    XRCCTRL(*this, "spnPreScripts", wxSpinButton)
        ->Enable(scrsel && presel && lstPreScripts->GetCount() > 1);
...
void ProjectOptionsDlg::EndModal(int retCode)
{
    if (retCode == wxID_OK)
    {
        ...
        m_Project->SetPlatforms(
            GetPlatformsFromString(XRCCTRL(*this, "txtPlatformProj", wxTextCtrl)->GetValue()));
        ...
        m_Project->SetMakefileExecutionDir(
            XRCCTRL(*this, "txtExecutionDir", wxTextCtrl)->GetValue());
        m_Project->SetTargetType(
            TargetType(XRCCTRL(*this, "cmbProjectType", wxComboBox)->GetSelection()));
        m_Project->SetModeForPCH(
            (PCHMode)XRCCTRL(*this, "rbPCHStrategy", wxRadioBox)->GetSelection());
        m_Project->SetExtendedObjectNamesGeneration(
            XRCCTRL(*this, "chkExtendedObjNames", wxCheckBox)->GetValue());
        m_Project->SetShowNotesOnLoad(XRCCTRL(*this, "chkShowNotes", wxCheckBox)->GetValue());
        m_Project->SetCheckForExternallyModifiedFiles(
            XRCCTRL(*this, "chkCheckFiles", wxCheckBox)->GetValue());
