...
MainFrame::MainFrame(wxWindow *parent)
  : wxFrame(parent, -1, _T("MainWin"), wxDefaultPosition, wxDefaultSize,
            wxDEFAULT_FRAME_STYLE | wxNO_FULL_REPAINT_ON_RESIZE),
    m_LayoutManager(this),
    m_pAccel(nullptr),
    m_filesHistory(_("&File"), wxT("/recent_files"), idFileOpenRecentFileClearHistory,
                   wxID_CBFILE01),
    m_projectsHistory(_("&File"), wxT("/recent_projects"), idFileOpenRecentProjectClearHistory,
                      wxID_CBFILE17),
    ...
    m_pHighlightButton(nullptr)
{
    ...
    // even it is possible that the statusbar is not visible at the moment, create the statusbar so
    // the plugins can create their own fields on the it:
    DoCreateStatusBar();
...
void MainFrame::RegisterEvents()
{
    ...
    m->RegisterEventSink(cbEVT_EDITOR_UPDATE_UI, new cbEventFunctor<MainFrame, CodeBlocksEvent>(
                                                     this, &MainFrame::OnEditorUpdateUI));

    m->RegisterEventSink(cbEVT_PROJECT_ACTIVATE, new cbEventFunctor<MainFrame, CodeBlocksEvent>(
                                                     this, &MainFrame::OnProjectActivated));
    m->RegisterEventSink(cbEVT_PROJECT_OPEN, new cbEventFunctor<MainFrame, CodeBlocksEvent>(
                                                 this, &MainFrame::OnProjectOpened));
    m->RegisterEventSink(cbEVT_PROJECT_CLOSE, new cbEventFunctor<MainFrame, CodeBlocksEvent>(
                                                  this, &MainFrame::OnProjectClosed));
    m->RegisterEventSink(cbEVT_EDITOR_CLOSE, new cbEventFunctor<MainFrame, CodeBlocksEvent>(
                                                 this, &MainFrame::OnEditorClosed));
    m->RegisterEventSink(cbEVT_EDITOR_OPEN, new cbEventFunctor<MainFrame, CodeBlocksEvent>(
                                                this, &MainFrame::OnEditorOpened));
    m->RegisterEventSink(cbEVT_EDITOR_ACTIVATED, new cbEventFunctor<MainFrame, CodeBlocksEvent>(
                                                     this, &MainFrame::OnEditorActivated));
    m->RegisterEventSink(cbEVT_EDITOR_SAVE, new cbEventFunctor<MainFrame, CodeBlocksEvent>(
                                                this, &MainFrame::OnEditorSaved));
    m->RegisterEventSink(cbEVT_EDITOR_MODIFIED, new cbEventFunctor<MainFrame, CodeBlocksEvent>(
                                                    this, &MainFrame::OnEditorModified));

    m->RegisterEventSink(cbEVT_ADD_DOCK_WINDOW, new cbEventFunctor<MainFrame, CodeBlocksDockEvent>(
                                                    this, &MainFrame::OnRequestDockWindow));
    m->RegisterEventSink(cbEVT_REMOVE_DOCK_WINDOW,
                         new cbEventFunctor<MainFrame, CodeBlocksDockEvent>(
                             this, &MainFrame::OnRequestUndockWindow));
    m->RegisterEventSink(cbEVT_SHOW_DOCK_WINDOW, new cbEventFunctor<MainFrame, CodeBlocksDockEvent>(
                                                     this, &MainFrame::OnRequestShowDockWindow));
    m->RegisterEventSink(cbEVT_HIDE_DOCK_WINDOW, new cbEventFunctor<MainFrame, CodeBlocksDockEvent>(
                                                     this, &MainFrame::OnRequestHideDockWindow));
    m->RegisterEventSink(cbEVT_DOCK_WINDOW_VISIBILITY,
                         new cbEventFunctor<MainFrame, CodeBlocksDockEvent>(
                             this, &MainFrame::OnDockWindowVisibility));

    m->RegisterEventSink(cbEVT_PLUGIN_ATTACHED, new cbEventFunctor<MainFrame, CodeBlocksEvent>(
                                                    this, &MainFrame::OnPluginLoaded));
    m->RegisterEventSink(cbEVT_PLUGIN_RELEASED, new cbEventFunctor<MainFrame, CodeBlocksEvent>(
                                                    this, &MainFrame::OnPluginUnloaded));
    m->RegisterEventSink(cbEVT_PLUGIN_INSTALLED, new cbEventFunctor<MainFrame, CodeBlocksEvent>(
                                                     this, &MainFrame::OnPluginInstalled));
    m->RegisterEventSink(cbEVT_PLUGIN_UNINSTALLED, new cbEventFunctor<MainFrame, CodeBlocksEvent>(
                                                       this, &MainFrame::OnPluginUninstalled));

    m->RegisterEventSink(
        cbEVT_UPDATE_VIEW_LAYOUT,
        new cbEventFunctor<MainFrame, CodeBlocksLayoutEvent>(this, &MainFrame::OnLayoutUpdate));
    m->RegisterEventSink(
        cbEVT_QUERY_VIEW_LAYOUT,
        new cbEventFunctor<MainFrame, CodeBlocksLayoutEvent>(this, &MainFrame::OnLayoutQuery));
    m->RegisterEventSink(
        cbEVT_SWITCH_VIEW_LAYOUT,
        new cbEventFunctor<MainFrame, CodeBlocksLayoutEvent>(this, &MainFrame::OnLayoutSwitch));

    m->RegisterEventSink(cbEVT_ADD_LOG_WINDOW, new cbEventFunctor<MainFrame, CodeBlocksLogEvent>(
                                                   this, &MainFrame::OnAddLogWindow));
    m->RegisterEventSink(cbEVT_REMOVE_LOG_WINDOW, new cbEventFunctor<MainFrame, CodeBlocksLogEvent>(
                                                      this, &MainFrame::OnRemoveLogWindow));
    m->RegisterEventSink(cbEVT_HIDE_LOG_WINDOW, new cbEventFunctor<MainFrame, CodeBlocksLogEvent>(
                                                    this, &MainFrame::OnHideLogWindow));
    m->RegisterEventSink(
        cbEVT_SWITCH_TO_LOG_WINDOW,
        new cbEventFunctor<MainFrame, CodeBlocksLogEvent>(this, &MainFrame::OnSwitchToLogWindow));
    m->RegisterEventSink(
        cbEVT_GET_ACTIVE_LOG_WINDOW,
        new cbEventFunctor<MainFrame, CodeBlocksLogEvent>(this, &MainFrame::OnGetActiveLogWindow));
    m->RegisterEventSink(cbEVT_SHOW_LOG_MANAGER, new cbEventFunctor<MainFrame, CodeBlocksLogEvent>(
                                                     this, &MainFrame::OnShowLogManager));
    m->RegisterEventSink(cbEVT_HIDE_LOG_MANAGER, new cbEventFunctor<MainFrame, CodeBlocksLogEvent>(
                                                     this, &MainFrame::OnHideLogManager));
    m->RegisterEventSink(cbEVT_LOCK_LOG_MANAGER, new cbEventFunctor<MainFrame, CodeBlocksLogEvent>(
                                                     this, &MainFrame::OnLockLogManager));
    m->RegisterEventSink(
        cbEVT_UNLOCK_LOG_MANAGER,
        new cbEventFunctor<MainFrame, CodeBlocksLogEvent>(this, &MainFrame::OnUnlockLogManager));
}

void MainFrame::ShowTips(bool forceShow)
{
    bool showAtStartup =
        Manager::Get()->GetConfigManager(_T("app"))->ReadBool(_T("/show_tips"), false);
    if (forceShow || showAtStartup)
    {
        ...
        Manager::Get()->GetConfigManager(_T("app"))->Write(_T("/next_tip"),
                                                           (int)tipProvider->GetCurrentTip());
        delete tipProvider;
    }
}
...
{
    int leftW = Manager::Get()->GetConfigManager(_T("app"))->ReadInt(
        _T("/main_frame/layout/left_block_width"), 200);
    wxSize clientsize = GetClientSize();

    // Create CloseFullScreen Button, and hide it initially
    m_pCloseFullScreenBtn =
        new wxButton(this, idCloseFullScreen, _("Close full screen"), wxDefaultPosition);
    ...
    if (!Manager::IsBatchBuild())
    {
        ...
        m_LayoutManager.AddPane(m_pPrjManUI->GetNotebook(),
                                wxAuiPaneInfo()
                                    .Name(wxT("ManagementPane"))
                                    .Caption(_("Management"))
    ...
    // editor manager
    m_LayoutManager.AddPane(m_pEdMan->GetNotebook(),
                            wxAuiPaneInfo().Name(wxT("MainPane")).CentrePane());

    // script console
    m_pScriptConsole = new ScriptConsole(this, -1);
    m_LayoutManager.AddPane(m_pScriptConsole, wxAuiPaneInfo()
                                                  .Name(wxT("ScriptConsole"))
                                                  .Caption(_("Scripting console"))
                                                  .Float()
                                                  .MinSize(100, 100)
...
{
    // allow new docked windows to use be 3/4 of the available space, the default (0.3) is sometimes
    // too small, especially for "Logs & others"
    m_LayoutManager.SetDockSizeConstraint(0.75, 0.75);

    int bottomH = Manager::Get()->GetConfigManager(_T("app"))->ReadInt(
        _T("/main_frame/layout/bottom_block_height"), 150);
    wxSize clientsize = GetClientSize();
    ...
    if (!Manager::IsBatchBuild())
    {
        ...
        m_LayoutManager.AddPane(m_pInfoPane, wxAuiPaneInfo()
                                                 .Name(wxT("MessagesPane"))
                                                 .Caption(_("Logs & others"))
        ...
        for (size_t i = LogManager::app_log; i < LogManager::max_logs; ++i)
        {
            if ((log = mgr->Slot(i).GetLogger()->CreateControl(m_pInfoPane)))
                m_pInfoPane->AddLogger(mgr->Slot(i).GetLogger(), log, mgr->Slot(i).title,
                                       mgr->Slot(i).icon);
        }
        ...
        if (mgr->Read(_T("/startup_scripts/") + keys[i], &ser))
        {
            ...
            try
            {
                ...
                if (wxFileName(se.script).IsRelative())
                    startup =
                        ConfigManager::LocateDataFile(se.script, sdScriptsUser | sdScriptsGlobal);
                if (!startup.IsEmpty())
                {
                    if (!se.registered)
                        Manager::Get()->GetScriptingManager()->LoadScript(startup);
                    else if (!se.menu.IsEmpty())
                        Manager::Get()->GetScriptingManager()->RegisterScriptMenu(se.menu, startup,
                                                                                  false);
                    else
                        Manager::Get()->GetLogManager()->LogWarning(
                            F(_("Startup script/function '%s' not loaded: invalid configuration"),
                              se.script.wx_str()));
                }
                else
                    Manager::Get()->GetLogManager()->LogWarning(
                        F(_("Startup script '%s' not found"), se.script.wx_str()));
            }
        ...
        if (m_PluginsTools[plug]) // if plugin has a toolbar
        {
            ...
            if (viewToolbars)
            {
                if (viewToolbars->FindItem(info->title) != wxNOT_FOUND)
                    continue;
                wxMenuItem *item =
                    AddPluginInMenus(viewToolbars, plug,
                                     (wxObjectEventFunction)(wxEventFunction)(
                                         wxCommandEventFunction)&MainFrame::OnToggleBar,
                                     -1, true);
                if (item)
                {
...
void MainFrame::RecreateMenuBar()
{
    ...
    for (LayoutViewsMap::iterator it = m_LayoutViews.begin(); it != m_LayoutViews.end(); ++it)
    {
        if (it->first.IsEmpty())
            continue;
        SaveViewLayout(it->first, it->second, m_LayoutMessagePane[it->first],
    ...
    if (tmpidx != wxNOT_FOUND)
    {
            ...
            if (colour_set)
            {
                ...
                for (size_t i = 0; i < langs.GetCount(); ++i)
                {
                    ...
                    hl->AppendRadioItem(
                        id, langs[i],
                        wxString::Format(
                            _("Switch highlighting mode for current document to \"%s\""),
                            langs[i].wx_str()));
                    Connect(id, -1, wxEVT_COMMAND_MENU_SELECTED,
                            (wxObjectEventFunction)(wxEventFunction)(
                                wxCommandEventFunction)&MainFrame::OnEditHighlightMode);
                }
            }
        }
        ...
        if (info
            && ((info->Language >= wxLANGUAGE_CHINESE
                ...
                || info->Language == wxLANGUAGE_JAPANESE || info->Language == wxLANGUAGE_KOREAN))
        {
...
void MainFrame::CreateToolbars()
{
    ...
    m_pToolbar->Connect(wxID_ANY, wxEVT_COMMAND_TOOL_RCLICKED,
    ...
    m_debuggerToolbarHandler->GetToolbar()->Connect(
        wxID_ANY, wxEVT_COMMAND_TOOL_RCLICKED,
        wxCommandEventHandler(MainFrame::OnToolBarRightClick), NULL, this);

    std::vector<ToolbarInfo> toolbars;

    toolbars.push_back(ToolbarInfo(
        m_pToolbar, wxAuiPaneInfo().Name(wxT("MainToolbar")).Caption(_("Main Toolbar")), 0));
    toolbars.push_back(ToolbarInfo(
        m_debuggerToolbarHandler->GetToolbar(),
        wxAuiPaneInfo().Name(wxT("DebuggerToolbar")).Caption(_("Debugger Toolbar")), 2));
        ...
        if (plug && plug->IsAttached())
        {
            ...
            if (info.toolbar)
            {
                ...
                info.toolbar->Connect(wxID_ANY, wxEVT_COMMAND_TOOL_RCLICKED,
                                      wxCommandEventHandler(MainFrame::OnToolBarRightClick), NULL,
                                      this);
            }
        }
    }
    ...
    for (std::vector<ToolbarInfo>::iterator it = toolbars.begin(); it != toolbars.end(); ++it)
    {
        ...
        m_LayoutManager.AddPane(it->toolbar,
                                paneInfo.ToolbarPane().Top().Row(row).Position(position));
...
void MainFrame::AddToolbarItem(int id, const wxString &title, const wxString &shortHelp,
                               const wxString &longHelp, const wxString &image)
{
...
wxMenuItem *MainFrame::AddPluginInMenus(wxMenu *menu, cbPlugin *plugin,
                                        wxObjectEventFunction callback, int pos, bool checkable)
{
    ...
    while (!item)
    {
        if (!pos || title.CmpNoCase(menu->FindItemByPosition(pos - 1)->GetItemLabelText()) > 0)
            item = menu->Insert(pos, id, title, wxEmptyString,
                                checkable ? wxITEM_CHECK : wxITEM_NORMAL);
...
void MainFrame::AddPluginInPluginsMenu(cbPlugin *plugin)
{
    ...
    AddPluginInMenus(m_PluginsMenu, plugin,
                     (wxObjectEventFunction)(wxEventFunction)(
                         wxCommandEventFunction)&MainFrame::OnPluginsExecuteMenu,
...
void MainFrame::AddPluginInHelpPluginsMenu(cbPlugin *plugin)
{
    AddPluginInMenus(m_HelpPluginsMenu, plugin,
                     (wxObjectEventFunction)(wxEventFunction)(
                         wxCommandEventFunction)&MainFrame::OnHelpPluginMenu);
}

void MainFrame::LoadWindowState()
{
    wxArrayString subs =
        Manager::Get()->GetConfigManager(_T("app"))->EnumerateSubPaths(_T("/main_frame/layout"));
    for (size_t i = 0; i < subs.GetCount(); ++i)
    {
        wxString name = Manager::Get()->GetConfigManager(_T("app"))->Read(_T("/main_frame/layout/")
                                                                          + subs[i] + _T("/name"));
        wxString layout = Manager::Get()->GetConfigManager(_T("app"))->Read(
            _T("/main_frame/layout/") + subs[i] + _T("/data"));
        wxString layoutMP = Manager::Get()->GetConfigManager(_T("app"))->Read(
            _T("/main_frame/layout/") + subs[i] + _T("/dataMessagePane"));
        SaveViewLayout(name, layout, layoutMP);
    }
    wxString deflayout =
        Manager::Get()->GetConfigManager(_T("app"))->Read(_T("/main_frame/layout/default"));
    LoadViewLayout(deflayout);

    // load manager and messages selected page
    if (m_pPrjManUI->GetNotebook())
        m_pPrjManUI->GetNotebook()->SetSelection(
            Manager::Get()->GetConfigManager(_T("app"))->ReadInt(
                _T("/main_frame/layout/left_block_selection"), 0));
    m_pInfoPane->SetSelection(Manager::Get()->GetConfigManager(_T("app"))->ReadInt(
        _T("/main_frame/layout/bottom_block_selection"), 0));

    // Cryogen 23/3/10 wxAuiNotebook can't set it's own tab position once instantiated, for some
    // reason. This code fails in InfoPane::InfoPane(). Moved here as this seems like a resonable
    // place to do UI setup. Feel free to move it elsewhere.
    if (Manager::Get()->GetConfigManager(_T("app"))->ReadBool(
            _T("/environment/infopane_tabs_bottom"), false))
...
void MainFrame::LoadWindowSize()
{
...
#else
    int x = 0;
    int y = wxSystemSettings::GetMetric(wxSYS_MENU_Y,
                                        this); // make sure it doesn't hide under the menu bar
#endif
    int w = 1000;
    int h = 800;
    ...
    int last_display_index =
        Manager::Get()->GetConfigManager(_T("app"))->ReadInt(_T("/main_frame/layout/display"), 0);
    // load window size and position
    wxRect rect(
        Manager::Get()->GetConfigManager(_T("app"))->ReadInt(_T("/main_frame/layout/left"), x),
    ...
    // maximize if needed
    bool maximized = Manager::Get()->GetConfigManager(_T("app"))->ReadBool(
        _T("/main_frame/layout/maximized"), true);
    Maximize(maximized); // toggle

    // set display, size and position
    int display_index_window =
        wxDisplay::GetFromWindow(this); // C::B usually starts on primary display...
...
void MainFrame::SaveWindowState()
{
    ...
    // if the new amount of layouts is less than the previous, because only the first layouts will
    // be overwritten
    wxArrayString subs =
        Manager::Get()->GetConfigManager(_T("app"))->EnumerateSubPaths(_T("/main_frame/layout"));
    for (size_t i = 0; i < subs.GetCount(); ++i)
    {
        Manager::Get()->GetConfigManager(_T("app"))->DeleteSubPath(_T("/main_frame/layout/")
                                                                   + subs[i]);
    }
    ...
    for (LayoutViewsMap::iterator it = m_LayoutViews.begin(); it != m_LayoutViews.end(); ++it)
    {
        ...
        if (!m_LayoutMessagePane[it->first].IsEmpty())
            Manager::Get()->GetConfigManager(_T("app"))->Write(key + _T("dataMessagePane"),
    ...
    if (m_pPrjManUI->GetNotebook())
    {
        ...
        Manager::Get()->GetConfigManager(_T("app"))->Write(
            _T("/main_frame/layout/left_block_selection"), selection);
    }
    Manager::Get()->GetConfigManager(_T("app"))->Write(
        _T("/main_frame/layout/bottom_block_selection"), m_pInfoPane->GetSelection());

    // save display, window size and position
    Manager::Get()->GetConfigManager(_T("app"))->Write(_T("/main_frame/layout/display"),
                                                       wxDisplay::GetFromWindow(this));
    if (!IsMaximized() && !IsIconized())
    {
        Manager::Get()->GetConfigManager(_T("app"))->Write(_T("/main_frame/layout/left"),
                                                           GetPosition().x);
        Manager::Get()->GetConfigManager(_T("app"))->Write(_T("/main_frame/layout/top"),
                                                           GetPosition().y);
        Manager::Get()->GetConfigManager(_T("app"))->Write(_T("/main_frame/layout/width"),
                                                           GetSize().x);
        Manager::Get()->GetConfigManager(_T("app"))->Write(_T("/main_frame/layout/height"),
                                                           GetSize().y);
    }
    Manager::Get()->GetConfigManager(_T("app"))->Write(_T("/main_frame/layout/maximized"),
                                                       IsMaximized());
}
...
void MainFrame::SaveViewLayout(const wxString &name, const wxString &layout,
                               const wxString &layoutMP, bool select)
{
    if (name.IsEmpty())
        return;
    ...
    {
        int id = wxNewId();
        viewLayouts->InsertCheckItem(
            viewLayouts->GetMenuItemCount() - 3, id, name,
            wxString::Format(_("Switch to %s perspective"), name.wx_str()));
        Connect(id, wxEVT_COMMAND_MENU_SELECTED,
                (wxObjectEventFunction)(wxEventFunction)(
                    wxCommandEventFunction)&MainFrame::OnViewLayout);
...
bool MainFrame::LayoutMessagePaneDifferent(const wxString &layout1, const wxString &layout2,
                                           bool checkSelection)
{
    wxStringTokenizer strTok;
    wxArrayString arLayout1;
    ...
    if (!m_LastLayoutName.IsEmpty()
        && (LayoutDifferent(lastlayout, m_LastLayoutData)
            || LayoutMessagePaneDifferent(
                   lastmessagepanelayout, m_LastMessagePaneLayoutData,
                   Manager::Get()
                       ->GetConfigManager(_T("message_manager"))
                       ->ReadBool(_T("/save_selection_change_in_mp"), true))))
    {
        AnnoyingDialog dlg(
            _("Layout changed"),
            wxString::Format(_("The perspective '%s' has changed. Do you want to save it?"),
                             m_LastLayoutName.wx_str()),
            wxART_QUESTION, canCancel ? AnnoyingDialog::YES_NO_CANCEL : AnnoyingDialog::YES_NO,
            // partial fix for bug 18970 (fix is incomplete to prevent the user from saving
            // 'rtCANCEL')
...
void MainFrame::DoSelectLayout(const wxString &name)
{
    ...
    if (viewLayouts)
    {
        ...
        if (!m_LastLayoutIsTemp)
            Manager::Get()->GetConfigManager(_T("app"))->Write(_T("/main_frame/layout/default"),
                                                               name);
    }
}
...
ToolbarInfo MainFrame::DoAddPluginToolbar(cbPlugin *plugin)
{
    ...
    if (plugin->BuildToolBar(info.toolbar))
    {
        ...
        info.paneInfo.Name(pluginInfo->name + _T("Toolbar"))
            .Caption(pluginInfo->title + _(" Toolbar"));
    }
    else
    {
        delete info.toolbar;
...
{
    // Manager::Get()->GetLogManager()->DebugLog(_T("Adding plugin: %s"),
    // plugin->GetInfo()->name.wx_str());
    ...
    else
    {
        ...
        // Don't load the toolbars during the initial loading of the plugins, this code should be
        // executed only when a single plugins is loaded from the Plugins -> Manager ... window.
        if (!m_ScanningForPlugins)
        {
            ...
            {
                // Place the new toolbar at the bottom of the toolbar pane. Try to reuse the last
                // row if there is enough space in it, otherwise place the new toolbar at a new row.
                int row = 0;
...
bool MainFrame::Open(const wxString &filename, bool addToHistory)
{
    ...
    fn.Normalize(); // really important so that two same files with different names are not loaded
                    // twice
...
{
    wxFileDialog dlg(this, caption, wxEmptyString, wxEmptyString, filter,
...
bool MainFrame::OpenGeneric(const wxString &filename, bool addToHistory)
{
    ...
    switch (FileTypeOf(filename))
    {
        ...
        case ftCodeBlocksWorkspace:
            ...
            else
            {
                if (DoCloseCurrentWorkspace())
                {
                    ...
                    if (!ret)
                        ShowHideStartPage(); // show/hide startherepage, dependant of settings, if
                                             // loading failed
                    else if (addToHistory)
                        m_projectsHistory.AddToHistory(
        ...
        case ftCodeBlocksProject:
        {
            ...
            {
                // NOTE (Morten#1#): A message here will prevent batch-builds from working and is
                // shown sometimes even if correct
        ...
        {
            cbMimePlugin *plugin =
                Manager::Get()->GetPluginManager()->GetMIMEHandlerForFile(filename);
            // warn user that "Files extension handler" is disabled
            if (!plugin)
            {
                cbMessageBox(_("Could not open file ") + filename
...
void MainFrame::DoCreateStatusBar()
{
    ...
    {
        m_pHighlightButton = new wxButton(sb, idHighlightButton, wxT("bla"), wxDefaultPosition,
                                          wxDefaultSize, wxBORDER_NONE | wxBU_LEFT | wxBU_EXACTFIT);
        // Adjust status bar height to fit the button.
        // This affects wx3.x build more than wx2.8 builds. At least on wxGTK.
        const int height =
            std::max(sb->GetMinHeight(), m_pHighlightButton->GetClientSize().GetHeight());
...
void MainFrame::DoUpdateStatusBar()
{
    ...
    if (ed)
    {
        ...
        if (m_pHighlightButton)
        {
            EditorColourSet *colour_set = Manager::Get()->GetEditorManager()->GetColourSet();
            if (colour_set)
                changeButtonLabel(*m_pHighlightButton,
                                  colour_set->GetLanguageName(ed->GetLanguage()));
            else
        ...
        msg.Printf(_("Line %d, Col %d, Pos %d"), control->GetCurrentLine() + 1,
                   control->GetColumn(pos) + 1, pos);
        SetStatusText(msg, panel++);
...
void MainFrame::DoUpdateEditorStyle(cbAuiNotebook *target, const wxString &prefix,
                                    long defaultStyle)
{
    if (!target)
        return;
...
void MainFrame::DoUpdateEditorStyle()
{
    ...
    long closestyle =
        Manager::Get()->GetConfigManager(_T("app"))->ReadInt(_T("/environment/tabs_closestyle"), 0);
    switch (closestyle)
    {
        case 1: // current tab
    ...
    if (Manager::Get()->GetConfigManager(_T("app"))->ReadBool(_T("/environment/hide_editor_tabs"),
                                                              false))
...
#ifndef __WXGTK__
    m_LayoutManager.SetFlags(wxAUI_MGR_DEFAULT | wxAUI_MGR_ALLOW_ACTIVE_PANE
                             | wxAUI_MGR_TRANSPARENT_DRAG);
#else // #ifndef __WXGTK__
    // workaround for a wxWidgets-bug that makes C::B crash when a floating window gets docked and
    // composite-effects are enabled
    m_LayoutManager.SetFlags((wxAUI_MGR_DEFAULT | wxAUI_MGR_ALLOW_ACTIVE_PANE
                              | wxAUI_MGR_TRANSPARENT_DRAG | wxAUI_MGR_VENETIAN_BLINDS_HINT)
                             & ~wxAUI_MGR_TRANSPARENT_HINT);
#endif // #ifndef __WXGTK__

    art->SetMetric(wxAUI_DOCKART_PANE_BORDER_SIZE,
                   cfg->ReadInt(_T("/environment/aui/border_size"),
                                art->GetMetric(wxAUI_DOCKART_PANE_BORDER_SIZE)));
    art->SetMetric(wxAUI_DOCKART_SASH_SIZE, cfg->ReadInt(_T("/environment/aui/sash_size"),
                                                         art->GetMetric(wxAUI_DOCKART_SASH_SIZE)));
    art->SetMetric(wxAUI_DOCKART_CAPTION_SIZE,
                   cfg->ReadInt(_T("/environment/aui/caption_size"),
                                art->GetMetric(wxAUI_DOCKART_CAPTION_SIZE)));
    art->SetColour(wxAUI_DOCKART_ACTIVE_CAPTION_COLOUR,
                   cfg->ReadColour(_T("/environment/aui/active_caption_colour"),
                                   art->GetColour(wxAUI_DOCKART_ACTIVE_CAPTION_COLOUR)));
    art->SetColour(wxAUI_DOCKART_ACTIVE_CAPTION_GRADIENT_COLOUR,
                   cfg->ReadColour(_T("/environment/aui/active_caption_gradient_colour"),
                                   art->GetColour(wxAUI_DOCKART_ACTIVE_CAPTION_GRADIENT_COLOUR)));
    art->SetColour(wxAUI_DOCKART_ACTIVE_CAPTION_TEXT_COLOUR,
                   cfg->ReadColour(_T("/environment/aui/active_caption_text_colour"),
                                   art->GetColour(wxAUI_DOCKART_ACTIVE_CAPTION_TEXT_COLOUR)));
    art->SetColour(wxAUI_DOCKART_INACTIVE_CAPTION_COLOUR,
                   cfg->ReadColour(_T("/environment/aui/inactive_caption_colour"),
                                   art->GetColour(wxAUI_DOCKART_INACTIVE_CAPTION_COLOUR)));
    art->SetColour(wxAUI_DOCKART_INACTIVE_CAPTION_GRADIENT_COLOUR,
                   cfg->ReadColour(_T("/environment/aui/inactive_caption_gradient_colour"),
                                   art->GetColour(wxAUI_DOCKART_INACTIVE_CAPTION_GRADIENT_COLOUR)));
    art->SetColour(wxAUI_DOCKART_INACTIVE_CAPTION_TEXT_COLOUR,
                   cfg->ReadColour(_T("/environment/aui/inactive_caption_text_colour"),
...
{
    EditorBase *ed = Manager::Get()->GetEditorManager()
    ...
    if (ed && ed->IsBuiltinEditor())
    {
        ...
        prj = Manager::Get()->GetProjectManager()
...
void MainFrame::ShowHideStartPage(bool forceHasProject, int forceState)
{
    ...
    bool show =
        !forceHasProject && Manager::Get()->GetProjectManager()->GetProjects()->GetCount() == 0
        && Manager::Get()->GetConfigManager(_T("app"))->ReadBool(_T("/environment/start_here_page"),
                                                                 true);
...
void MainFrame::OnStartHereLink(wxCommandEvent &event)
{
    ...
    else if (link.StartsWith(_T("CB_CMD_OPEN_HISTORY_")))
    {
        ...
        recent = link.StartsWith(_T("CB_CMD_OPEN_HISTORY_PROJECT_")) ? &m_projectsHistory
                                                                     : &m_filesHistory;
        unsigned long item;
    ...
    {
        RecentItemsList *recent;
        recent = link.StartsWith(_T("CB_CMD_DELETE_HISTORY_PROJECT_")) ? &m_projectsHistory
                                                                       : &m_filesHistory;
        unsigned long item;
        link.AfterLast(_T('_')).ToULong(&item);
        --item;
...
void MainFrame::OnPluginsExecuteMenu(wxCommandEvent &event)
{
    ...
    if (!pluginName.IsEmpty())
        ...
        Manager::Get()->GetLogManager()->DebugLog(
            F(_T("No plugin found for ID %d"), event.GetId()));
}
...
void MainFrame::OnFileNewWhat(wxCommandEvent &event)
{
    ...
    if (ed)
        ed->GetControl()->SetChangeCollection(Manager::Get()
                                                  ->GetConfigManager(_T("editor"))
                                                  ->ReadBool(_T("/margin/use_changebar"), true));
    ...
    if (cbMessageBox(
            _("Do you want to add this new file in the active project (has to be saved first)?"),
            _("Add file to project"), wxYES_NO | wxICON_QUESTION)
            == wxID_YES
        && ed->SaveAs() && ed->IsOK())
    {
        wxArrayInt targets;
        if (Manager::Get()->GetProjectManager()->AddFileToProject(ed->GetFilename(), project,
                                                                  targets)
            != 0)
        {
...
void MainFrame::DoOnFileOpen(bool bProject)
{
    ...
    if (mgr)
    {
        if (!bProject)
        {
            ...
            FileFilters::GetFilterIndexFromName(Filters, _("Code::Blocks project/workspace files"),
                                                StoredIndex);
    }
    wxFileDialog dlg(this, _("Open file"), Path, wxEmptyString, Filters,
...
void MainFrame::OnFileSave(cb_unused wxCommandEvent &event)
{
    ...
    {
        wxString msg;
        msg.Printf(_("File %s could not be saved..."),
...
void MainFrame::OnFileSaveAll(cb_unused wxCommandEvent &event)
{
    ...
    if (prjManager->GetWorkspace()->GetModified() && !prjManager->GetWorkspace()->IsDefault()
...
{
    TemplateManager::Get()->SaveUserTemplate(
        Manager::Get()->GetProjectManager()->GetActiveProject());
}
...
{
    OpenGeneric(ShowOpenFileDialog(_("Import Dev-C++ project"),
                                   FileFilters::GetFilterString(_T('.') + FileFilters::DEVCPP_EXT)),
                false);
}

void MainFrame::OnFileImportProjectMSVC(cb_unused wxCommandEvent &event)
{
    OpenGeneric(ShowOpenFileDialog(_("Import MS Visual C++ 6.0 project"),
                                   FileFilters::GetFilterString(_T('.') + FileFilters::MSVC6_EXT)),
                false);
}

void MainFrame::OnFileImportProjectMSVCWksp(cb_unused wxCommandEvent &event)
{
    OpenGeneric(ShowOpenFileDialog(
                    _("Import MS Visual C++ 6.0 workspace"),
                    FileFilters::GetFilterString(_T('.') + FileFilters::MSVC6_WORKSPACE_EXT)),
                false);
}

void MainFrame::OnFileImportProjectMSVS(cb_unused wxCommandEvent &event)
{
    OpenGeneric(ShowOpenFileDialog(_("Import MS Visual Studio 7.0+ project"),
                                   FileFilters::GetFilterString(_T('.') + FileFilters::MSVC7_EXT)),
                false);
}
...
{
    OpenGeneric(ShowOpenFileDialog(
                    _("Import MS Visual Studio 7.0+ solution"),
                    FileFilters::GetFilterString(_T('.') + FileFilters::MSVC7_WORKSPACE_EXT)),
                false);
}
...
void MainFrame::OnFileOpenDefWorkspace(cb_unused wxCommandEvent &event)
{
    ...
    if (!pman->GetWorkspace()->IsDefault() && !pman->LoadWorkspace())
    {
        ...
        cbMessageBox(_("Can't open default workspace (file exists?)"), _("Warning"),
                     wxICON_WARNING);
    }
}
...
void MainFrame::OnFilePrint(cb_unused wxCommandEvent &event)
{
    ...
    PlaceWindow(&dlg);
    if (dlg.ShowModal() == wxID_OK)
        Manager::Get()->GetEditorManager()->Print(dlg.GetPrintScope(), dlg.GetPrintColourMode(),
                                                  dlg.GetPrintLineNumbers());
}
...
void MainFrame::OnApplicationClose(wxCloseEvent &event)
{
...

    {
        ...
        {
            int result =
                cbMessageBox(_("Currently compiling. Stop compilation and exit?"), _("Question"),
    ...
    // Disconnect the mouse right click event handler for toolbars, this should be done before the
    // plugin is unloaded in Manager::Shutdown().
    PluginToolbarsMap::iterator it;
    for (it = m_PluginsTools.begin(); it != m_PluginsTools.end(); ++it)
    {
        wxToolBar *toolbar = it->second;
        if (toolbar) // Disconnect the mouse right click event handler before the toolbar is
                     // destroyed
        {
            bool result =
                toolbar->Disconnect(wxID_ANY, wxEVT_COMMAND_TOOL_RCLICKED,
                                    wxCommandEventHandler(MainFrame::OnToolBarRightClick));
            cbAssert(result);
        }
    }
...
void MainFrame::OnEditLineMove(wxCommandEvent &event)
{
    ...
    // TODO (mortenmacfly##): Exclude rectangle selection here or not? What behaviour the majority
    // of users expect here???
    ...
    if (event.GetId() == idEditLineUp)
    {
        ...
        stc->BeginUndoAction();
        const int offset =
            (isLastLine ? startPos - stc->GetLineEndPosition(stc->LineFromPosition(startPos) - 1)
                        : 0);
        const int lineLength =
            startPos - stc->PositionFromLine(stc->LineFromPosition(startPos) - 1);
    ...
    else // event.GetId() == idEditLineDown
    {
        ...
        stc->BeginUndoAction();
        const int lineLength =
            stc->PositionFromLine(stc->LineFromPosition(endPos + 1) + 1) - endPos - 1;
...
bool SelectNext(cbStyledTextCtrl *control, const wxString &selectedText, long selectionEnd,
                bool reversed)
{
...
void MainFrame::OnEditSelectNext(cb_unused wxCommandEvent &event)
{
    ...
    if (!selection.Empty())
    {
        const wxString &selectedText(
            control->GetTextRange(selection.GetStart(), selection.GetEnd()));
        ...
        */
        if (startLine != endLine && // selection is more than one line
            stc->GetColumn(stc->GetSelectionEnd())
                == 0) // and the caret is at the start of the line
        {
        ...
        stc->SetSelectionVoid(stc->PositionFromLine(startLine),
                              stc->PositionFromLine(endLine) + stc->LineLength(endLine));
    }
    stc->EndUndoAction();
}
    ...
    if (wxSCI_INVALID_POSITION != stc->GetSelectionStart())
    {
        ...
        */
        ...
        while (curLine <= endLine)
        {
            ...
            startsWithComment =
                strLine.Strip(wxString::leading)
                    .StartsWith(comment.streamCommentStart); // check for stream comment start
            endsWithComment =
                strLine.Strip(wxString::trailing)
                    .EndsWith(comment.streamCommentEnd); // check for stream comment end
            if (startsWithComment && endsWithComment)
            {
                ...
                Pos = strLine.rfind(comment.streamCommentEnd, strLine.npos)
                      - comment.streamCommentStart.Length();
                start = stc->PositionFromLine(curLine) + Pos;
    ...
    if (wxSCI_INVALID_POSITION != stc->GetSelectionStart())
    {
        ...
        else
        {
            /**
                ...
                This fixes it.
            */
            if (stc->GetColumn(stc->GetSelectionEnd())
                == 0) // and the caret is at the start of the line
            {
                // don't take into account the line the caret is on,
                // because it contains no selection (caret_column == 0)...
    ...
    if (wxSCI_INVALID_POSITION != stc->GetSelectionStart())
    {
        ...
        */
        ...
        else // selection is more than one line
        {
            ...
            stc->InsertText(stc->PositionFromLine(curLine),
                            comment.boxCommentEnd + GetEOLStr(stc->GetEOLMode()));
        } // end if
        stc->SetSelectionVoid(stc->PositionFromLine(startLine),
                              stc->PositionFromLine(endLine) + stc->LineLength(endLine));
    }
...
void MainFrame::OnViewLayoutSave(cb_unused wxCommandEvent &event)
{
    ...
    wxString name = cbGetTextFromUser(_("Enter the name for this perspective"),
                                      _("Save current perspective"), def);
    if (!name.IsEmpty())
    {
        DoFixToolbarsLayout();
...
void MainFrame::OnViewLayoutDelete(cb_unused wxCommandEvent &event)
{
    ...
    {
        if (cbMessageBox(
                _("The default perspective cannot be deleted. It can always be reverted to "
    ...
    if (cbMessageBox(
            wxString::Format(_("Are you really sure you want to delete the perspective '%s'?"),
                             m_LastLayoutName.wx_str()),
            _("Confirmation"), wxICON_QUESTION | wxYES_NO | wxNO_DEFAULT)
        == wxID_YES)
    {
        ...
        cbMessageBox(
            wxString::Format(_("Perspective '%s' deleted.\nWill now revert to perspective '%s'..."),
                             m_LastLayoutName.wx_str(), gDefaultLayout.wx_str()),
...
{
    if (m_LastLayoutName == gMinimalLayout)
        LoadViewLayout(m_PreviousLayoutName.IsEmpty()
                           ? Manager::Get()->GetConfigManager(_T("app"))->Read(
                                 _T("/environment/view/layout_to_toggle"), gDefaultLayout)
...
void MainFrame::OnViewHideEditorTabs(cb_unused wxCommandEvent &event)
{
    ...
    if (nb)
    {
        ...
        Manager::Get()->GetConfigManager(_T("app"))->Write(_T("/environment/hide_editor_tabs"),
                                                           hide_editor_tabs);
    }
}
...
void MainFrame::OnFileMenuUpdateUI(wxUpdateUIEvent &event)
{
    ...
    else
    {
        ...
        if (id == idFileReopenProject)
            event.Enable(true);
        else if (id == idFileCloseProject || id == idFileSaveProjectAs
...
void MainFrame::OnEditMenuUpdateUI(wxUpdateUIEvent &event)
{
    ...
    mbar->Enable(idEditFolding, ed
                                    && Manager::Get()
                                           ->GetConfigManager(_T("editor"))
                                           ->ReadBool(_T("/folding/show_folds"), false));
    ...
    if (ed)
    {
        // OK... this was the strangest/silliest/most-frustrating bug ever in the computer programs
        // history... Under wxGTK it seems that if you try to Check() a menu item if its container
        // Menu is disabled, you enter an endless message loop eating 100% CPU... DARN! This fixes
        // the dreaded 'linux-hang-on-close-project' bug.
        ...
        switch (ed->GetControl()->GetEOLMode())
        {
            ...
            case wxSCI_EOL_LF:
                ...
                break;
            default:
                (platform::windows ? mbar->Check(idEditEOLCRLF, true)
                                   : mbar->Check(idEditEOLLF, true));
                break;
        }

        bool defenc = ed
                      && (ed->GetEncoding() == wxFONTENCODING_SYSTEM
...
void MainFrame::OnViewMenuUpdateUI(wxUpdateUIEvent &event)
{
    ...
    mbar->Check(idViewStartPage,
                Manager::Get()->GetEditorManager()->GetEditor(g_StartHereTitle) != NULL);
    ...
    mbar->Check(idViewHideEditorTabs,
...
void MainFrame::OnEditorUpdateUI(CodeBlocksEvent &event)
{
    ...
    if (Manager::Get()->GetEditorManager()
        && event.GetEditor() == Manager::Get()->GetEditorManager()->GetActiveEditor())
    {
...
static void FitToolbars(wxAuiManager &layoutManager, wxWindow *mainFrame)
{
    ...
    for (std::vector<wxWindow *>::iterator it = nonFitingToolbars.begin();
         it != nonFitingToolbars.end(); ++it)
    {
...
void MainFrame::OnSwitchTabs(cb_unused wxCommandEvent &event)
{
    ...
    { // Switch tabs editor with tab order
        for (size_t i = 0; i < nb->GetPageCount(); ++i)
        {
            ...
            items
                .AddItem(title, title, GetEditorDescription(static_cast<EditorBase *>(window)), i,
                         nb->GetPageBitmap(i))
                .SetWindow(window);
        }
    ...
    { // Switch tabs editor with last used order
        ...
        for (body = Manager::Get()->GetEditorManager()->GetNotebookStack(); body != NULL;
             body = body->next)
        {
            ...
            items
                .AddItem(title, title,
                         GetEditorDescription(static_cast<EditorBase *>(body->window)), index,
                         nb->GetPageBitmap(index))
        ...
        if (items.GetItemCount() > 2)
            items.SetSelection(
                2); // CTRL + TAB directly select the last editor, not the current one
        else
...
void MainFrame::OnPluginUnloaded(CodeBlocksEvent &event)
{
    ...
    if (m_PluginsTools[plugin])
    {
        ...
        bool result = m_PluginsTools[plugin]->Disconnect(
            wxID_ANY, wxEVT_COMMAND_TOOL_RCLICKED,
            wxCommandEventHandler(MainFrame::OnToolBarRightClick));
        cbAssert(result);
...
void MainFrame::OnSettingsEnvironment(cb_unused wxCommandEvent &event)
{
    ...
    if (dlg.ShowModal() == wxID_OK)
    {
        ...
        m_SmallToolBar = Manager::Get()->GetConfigManager(_T("app"))->ReadBool(
            _T("/environment/toolbar_size"), true);
    ...
    if (needRestart)
        cbMessageBox(_("Code::Blocks needs to be restarted for the changes to take effect."),
                     _("Information"), wxICON_INFORMATION);
}
...
void MainFrame::PopupToggleToolbarMenu()
{
    ...
    for (size_t ii = 0; ii < toolbarMenu->GetMenuItemCount(); ++ii)
    {
        ...
        if (!old)
            continue;
        wxMenuItem *item;
        item = new wxMenuItem(&menu, old->GetId(), old->GetItemLabelText(), old->GetHelp(),
                              old->GetKind());
        menu.Append(item);
    }
...
void MainFrame::SetChecksForViewToolbarsMenu(wxMenu &menu)
{
    ...
    menu.Check(idViewToolDebugger,
               m_LayoutManager.GetPane(m_debuggerToolbarHandler->GetToolbar(false)).IsShown());
}
