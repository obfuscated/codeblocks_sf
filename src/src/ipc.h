...
public:
     ...
     *   which the caller is still holding, so another process can write to the shared memory area
     * again. It does not release the reader semaphore, since it should block on it on the next
     * iteration.
     *
     * Unlock(writer) locks "as writer", i.e. it
     *   1. unlocks the reader semaphore, waking up the Server thread
     *   2. unlocks the writer mutex, so the Server thread can acquire it and prevent other
