...
DebuggerSettingsPanel::DebuggerSettingsPanel(wxWindow *parent, DebuggerSettingsDlg *dialog,
                                             cbDebuggerPlugin *plugin)
  : m_dialog(dialog), m_plugin(plugin)
{
    ...
    butCreate = new wxButton(this, ID_BUTTON_CREATE, _("Create Config"), wxDefaultPosition,
                             wxDefaultSize, 0, wxDefaultValidator, _T("ID_BUTTON_CREATE"));
    buttonSizer->Add(butCreate, 1, wxALIGN_BOTTOM, 5);
    butDelete = new wxButton(this, ID_BUTTON_DELETE, _("Delete Config"), wxDefaultPosition,
                             wxDefaultSize, 0, wxDefaultValidator, _T("ID_BUTTON_DELETE"));
    buttonSizer->Add(butDelete, 1, wxLEFT | wxALIGN_BOTTOM, 5);
    butReset = new wxButton(this, ID_BUTTON_RESET, _("Reset defaults"), wxDefaultPosition,
                            wxDefaultSize, 0, wxDefaultValidator, _T("ID_BUTTON_RESET"));
    ...
    textInfo = new wxTextCtrl(this, ID_TEXTCTRL_INFO, wxEmptyString, wxDefaultPosition,
                              wxSize(186, 243), 0, wxDefaultValidator, _T("ID_TEXTCTRL_INFO"));
    textInfo->Disable();
    ...
    Connect(ID_BUTTON_CREATE, wxEVT_COMMAND_BUTTON_CLICKED,
            (wxObjectEventFunction)&DebuggerSettingsPanel::OnButtonCreate);
    Connect(ID_BUTTON_DELETE, wxEVT_COMMAND_BUTTON_CLICKED,
            (wxObjectEventFunction)&DebuggerSettingsPanel::OnButtonDelete);
    Connect(ID_BUTTON_RESET, wxEVT_COMMAND_BUTTON_CLICKED,
            (wxObjectEventFunction)&DebuggerSettingsPanel::OnButtonReset);
    //*)
...
{
    wxTextEntryDialog dialog(this, _("Please specify a name for the configuration"),
                             _("Create config"));
    PlaceWindow(&dialog);
...
void DebuggerSettingsPanel::OnButtonReset(cb_unused wxCommandEvent &event)
{
    if (cbMessageBox(_("Are you sure you want to reset the settings for the selected plugin?"),
                     _("Reset settings"), wxICON_QUESTION | wxYES)
        == wxID_NO)
    {
        return;
    }
