...
bool DoCheckAssociation(const wxString &ext, const wxString &descr, const wxString &exe,
                        int icoNum); // TODO: descriptions are not checked at all, and actually it's
                                     // not necessary
                                     //       (extension + executable is actually enough)
                                     //        ---> decide whether to remove parameter from "Check"
                                     //        function or implement
