...
DebuggerSettingsCommonPanel::DebuggerSettingsCommonPanel(wxWindow *parent)
{
    ...
    m_autoBuild =
        new wxCheckBox(this, ID_AUTOBUILD, _("Auto-build project if it is not up-to-date"),
                       wxDefaultPosition, wxDefaultSize, 0, wxDefaultValidator, _T("ID_AUTOBUILD"));
    m_autoBuild->SetValue(false);
    flexSizer->Add(m_autoBuild, 1, wxTOP | wxLEFT | wxRIGHT | wxALIGN_TOP, 5);
    m_autoSwitch = new wxCheckBox(
        this, ID_AUTOSWITCH,
        _("When stopping, auto-switch to the first frame with valid source info"),
        wxDefaultPosition, wxDefaultSize, 0, wxDefaultValidator, _T("ID_AUTOSWITCH"));
    m_autoSwitch->SetValue(false);
    flexSizer->Add(m_autoSwitch, 1, wxTOP | wxLEFT | wxRIGHT | wxALIGN_TOP, 5);
    m_debuggersLog =
        new wxCheckBox(this, ID_DEBUGGERS_LOG, _("Full (Debug) log"), wxDefaultPosition,
                       wxDefaultSize, 0, wxDefaultValidator, _T("ID_DEBUGGERS_LOG"));
    m_debuggersLog->SetValue(false);
    flexSizer->Add(m_debuggersLog, 1, wxTOP | wxLEFT | wxRIGHT | wxALIGN_TOP, 5);
    m_jumpOnDoubleClick = new wxCheckBox(
        this, ID_JUMP_ON_DOUBLE_CLICK, _("Jump on Double-click in Stack trace window"),
        wxDefaultPosition, wxDefaultSize, 0, wxDefaultValidator, _T("ID_JUMP_ON_DOUBLE_CLICK"));
    m_jumpOnDoubleClick->SetValue(false);
    flexSizer->Add(m_jumpOnDoubleClick, 1, wxTOP | wxLEFT | wxRIGHT | wxALIGN_TOP, 5);
    m_requireCtrlForTooltips = new wxCheckBox(
        this, ID_REQUIRE_CTRL_FOR_TOOLTIPS,
        _("Require Control key to show the \'Evaluate expression\' tooltips"), wxDefaultPosition,
        wxDefaultSize, 0, wxDefaultValidator, _T("ID_REQUIRE_CTRL_FOR_TOOLTIPS"));
    ...
    m_valueTooltipLabel =
        new wxStaticText(this, ID_VALUE_TOOLTIP_LABEL, _("This is a sample text"),
                         wxDefaultPosition, wxDefaultSize, 0, _T("ID_VALUE_TOOLTIP_LABEL"));
    valueTooltipSizer->Add(m_valueTooltipLabel, 1, wxALIGN_CENTER_VERTICAL, 5);
    chooseFont = new wxButton(this, ID_BUTTON_CHOOSE_FONT, _("Choose"), wxDefaultPosition,
                              wxDefaultSize, 0, wxDefaultValidator, _T("ID_BUTTON_CHOOSE_FONT"));
    ...
    labelPerspective = new wxStaticText(this, wxID_ANY, _("Perspective:"), wxDefaultPosition,
                                        wxDefaultSize, 0, _T("wxID_ANY"));
    BoxSizer1->Add(labelPerspective, 0, wxALIGN_CENTER_VERTICAL, 5);
    m_perspective = new wxChoice(this, ID_CHOICE_PERSPECTIVE, wxDefaultPosition, wxDefaultSize, 0,
                                 0, 0, wxDefaultValidator, _T("ID_CHOICE_PERSPECTIVE"));
    m_perspective->Append(_("Only one perspective"));
    m_perspective->Append(_("One perspective per Debugger"));
    m_perspective->SetSelection(
        m_perspective->Append(_("One perspective per Debugger configuration")));
    ...
    Connect(ID_BUTTON_CHOOSE_FONT, wxEVT_COMMAND_BUTTON_CLICKED,
            (wxObjectEventFunction)&DebuggerSettingsCommonPanel::OnChooseFontClick);
    //*)
    ...
    m_autoSwitch->SetValue(
        cbDebuggerCommonConfig::GetFlag(cbDebuggerCommonConfig::AutoSwitchFrame));
    m_autoSwitch->SetToolTip(
        _("When stopping, auto-switch to first frame with valid source info."));

    m_debuggersLog->SetValue(
        cbDebuggerCommonConfig::GetFlag(cbDebuggerCommonConfig::ShowDebuggersLog));
    m_debuggersLog->SetToolTip(
        _("If enabled, the debugger's raw input/output will be logged in a separate log page."));

    m_jumpOnDoubleClick->SetValue(
        cbDebuggerCommonConfig::GetFlag(cbDebuggerCommonConfig::JumpOnDoubleClick));

    m_requireCtrlForTooltips->SetValue(
        cbDebuggerCommonConfig::GetFlag(cbDebuggerCommonConfig::RequireCtrlForTooltips));
    m_requireCtrlForTooltips->SetToolTip(
        _("If enabled, show 'Evaluate expression' tooltips only if the Control key is pressed."));
...
void DebuggerSettingsCommonPanel::SaveChanges()
{
    ...
    cbDebuggerCommonConfig::SetFlag(cbDebuggerCommonConfig::AutoSwitchFrame,
                                    m_autoSwitch->GetValue());
    cbDebuggerCommonConfig::SetFlag(cbDebuggerCommonConfig::ShowDebuggersLog,
                                    m_debuggersLog->GetValue());
    cbDebuggerCommonConfig::SetFlag(cbDebuggerCommonConfig::JumpOnDoubleClick,
                                    m_jumpOnDoubleClick->GetValue());
