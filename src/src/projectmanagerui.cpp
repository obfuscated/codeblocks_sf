...
bool ProjectNodeDragged(cbProject *project, wxTreeCtrl *tree, wxArrayTreeItemIds &fromArray,
                        wxTreeItemId to);
...
bool ProjectVirtualFolderRenamed(cbProject *project, wxTreeCtrl *tree, wxTreeItemId node,
                                 const wxString &new_name);
bool ProjectVirtualFolderDragged(cbProject *project, wxTreeCtrl *tree, wxTreeItemId from,
                                 wxTreeItemId to);
...
ProjectManagerUI::ProjectManagerUI()
  : m_pTree(nullptr), m_TreeFreezeCounter(0), m_isCheckingForExternallyModifiedProjects(false)
{
    m_pNotebook = new cbAuiNotebook(Manager::Get()->GetAppWindow(), idNB, wxDefaultPosition,
                                    wxDefaultSize, wxAUI_NB_WINDOWLIST_BUTTON);
    if (Manager::Get()->GetConfigManager(_T("app"))->ReadBool(
            _T("/environment/project_tabs_bottom"), false))
    ...
    m_TreeVisualState |=
        (cfg->ReadBool(_T("/hide_folder_name"), false) ? ptvsHideFolderName : ptvsNone);
    ...
    Manager::Get()->GetColourManager()->RegisterColour(
        _("Project Tree"), _("Not-compiled files (headers/resources)"),
        wxT("project_tree_non_source_files"), wxSystemSettings::GetColour(wxSYS_COLOUR_GRAYTEXT));
...
void ProjectManagerUI::RebuildTree()
{
    ...
    for (int i = 0; i < count; ++i)
    {
        if (cbProject *prj = pa->Item(i))
        {
            BuildProjectTree(prj, m_pTree, m_TreeRoot, m_TreeVisualState,
                             pm->GetFilesGroupsAndMasks());
...
void ProjectManagerUI::FinishLoadingProject(cbProject *project, bool newAddition,
                                            cb_unused FilesGroupsAndMasks *fgam)
{
    if (newAddition)
    {
        ProjectManager *pm = Manager::Get()->GetProjectManager();
        BuildProjectTree(project, m_pTree, m_TreeRoot, m_TreeVisualState,
                         pm->GetFilesGroupsAndMasks());
    }
    else
        RebuildTree();
...
void ProjectManagerUI::CreateMenu(wxMenuBar *menuBar)
{
    ...
    if (menuBar)
    {
        ...
        if (menu)
        {
            ...
            menu->Insert(menu->GetMenuItemCount() - 1, wxID_SEPARATOR,
                         _T("")); // instead of AppendSeparator();
        }
        ...
        if (menu)
        {
            ...
            menu->Append(idMenuAddFilesRecursively, _("Add files recursively..."),
                         _("Add files recursively to the project"));
            menu->Append(idMenuRemoveFile, _("Remove files..."),
                         _("Remove files from the project"));

            menu->AppendSeparator();
            CreateMenuTreeProps(menu, false);

            menu->Append(idMenuExecParams, _("Set &programs' arguments..."),
                         _("Set execution parameters for the targets of this project"));
...
void ProjectManagerUI::CreateMenuTreeProps(wxMenu *menu, bool popup)
{
    ...
    bool do_hide_folder_name =
        !do_use_folders
        && cfg->ReadBool(_T("/hide_folder_name"), false); // "use folders" has precedence
    cfg->Write(_T("/hide_folder_name"),
               do_hide_folder_name); // make sure that configuration is consistent
...
void ProjectManagerUI::ShowMenu(wxTreeItemId id, const wxPoint &pt)
{
    ...
    if (pa && ftd
        && (ftd->GetKind() == FileTreeData::ftdkProject || ftd->GetKind() == FileTreeData::ftdkFile
    ...
    if (ftd)
    {
            ...
            for (unsigned int i = 0; i < mimes.GetCount() && i < MAX_OPEN_WITH_ITEMS; ++i)
            {
                ...
                if (plugin && plugin->CanHandleFile(m_pTree->GetItemText(id)))
                {
                    const PluginInfo *info =
                        Manager::Get()->GetPluginManager()->GetPluginInfo(plugin);
                    openWith->Append(idOpenWith[i],
                                     info ? info->title : wxString(_("<Unknown plugin>")));
                }
            }
        ...
        else if (ftd->GetKind() == FileTreeData::ftdkFolder)
        {
            ...
            menu.Append(idMenuRemoveFolderFilesPopup,
                        wxString::Format(_("Remove %s*"), f.GetFullPath().c_str()));
            menu.Enable(idMenuRemoveFolderFilesPopup, PopUpMenuOption);
            menu.Append(idMenuOpenFolderFilesPopup,
                        wxString::Format(_("Open %s*"), f.GetFullPath().c_str()));
        }
        ...
        else if (is_vfolder)
        {
            ...
            menu.Append(idMenuRemoveFile, _("Remove files..."));
            menu.Append(idMenuRemoveFolderFilesPopup,
                        wxString::Format(_("Remove %s*"), ftd->GetFolder().c_str()));
            menu.Append(idMenuOpenFolderFilesPopup,
                        wxString::Format(_("Open %s*"), ftd->GetFolder().c_str()));
            menu.AppendSeparator();
            menu.Append(idMenuFindFile, _("Find file..."));
        }
    ...
    else
    {
        ...
        else if (plugin->OpenFile(filename) != 0)
        {
            ...
            msg.Printf(
                ...
                filename.c_str(),
                info ? info->title.c_str() : wxString(_("<Unknown plugin>")).c_str());
            Manager::Get()->GetLogManager()->LogError(msg);
        }
    }
}
...
void ProjectManagerUI::OnProjectFileActivated(wxTreeEvent &event)
{
    ...
    if (ftd && ftd->GetKind() == FileTreeData::ftdkProject)
    {
        ...
        m_pTree->IsExpanded(m_TreeRoot) ? m_pTree->Collapse(m_TreeRoot)
                                        : m_pTree->Expand(m_TreeRoot);
    else
        DoOpenSelectedFile();
}
...
void ProjectManagerUI::OnRightClick(cb_unused wxCommandEvent &event)
{
    ...
    bool do_hide_folder_name =
        !do_use_folders && (m_TreeVisualState & ptvsHideFolderName); // "use folders" has precedence
...
void ProjectManagerUI::OnTreeItemRightClick(wxTreeEvent &event)
{
    ...
    // We have a popup menu, so we will use the right-click item instead of the first tree
    // selection.
...
void ProjectManagerUI::OnRenameWorkspace(cb_unused wxCommandEvent &event)
{
    ...
    if (wkspc)
    {
        wxString text = cbGetTextFromUser(_("Please enter the new name for the workspace:"),
                                          _("Rename workspace"), wkspc->GetTitle());
        if (!text.IsEmpty())
        {
            wkspc->SetTitle(text);
...
void ProjectManagerUI::OnAddFilesToProjectRecursively(wxCommandEvent &event)
{
    ...
    wxString dir = ChooseDirectory(m_pTree, _("Add files recursively..."), basePath, wxEmptyString,
                                   false, false);
    if (dir.IsEmpty())
        return;
    ...
    while (i < array.GetCount())
    {
        ...
        if (wxDirExists(array[i]) || array[i].Contains(_T("/.git/"))
            || array[i].Contains(_T("\\.git\\")) || array[i].Contains(_T("\\.hg\\"))
            || array[i].Contains(_T("/.hg/")) || array[i].Contains(_T("\\.svn\\"))
            || array[i].Contains(_T("/.svn/")) || array[i].Contains(_T("\\CVS\\"))
            || array[i].Contains(_T("/CVS/")) || array[i].Lower().Matches(_T("*.cbp")))
        {
            array.RemoveAt(i);
        }
        else
...
void ProjectManagerUI::OnAddFileToProject(wxCommandEvent &event)
{
    ...
    wxFileDialog dlg(Manager::Get()->GetAppWindow(), _("Add files to project..."), basePath,
                     wxEmptyString, FileFilters::GetFilterString(),
...
void FindFiles(wxArrayString &resultFiles, wxTreeCtrl &tree, wxTreeItemId item)
{
    ...
    switch (ftd->GetKind())
    {
        ...
        default:
            for (FilesList::iterator it = ftd->GetProject()->GetFilesList().begin();
                 it != ftd->GetProject()->GetFilesList().end(); ++it)
...
void ProjectManagerUI::OnRemoveFileFromProject(wxCommandEvent &event)
{
    ...
    if (event.GetId() == idMenuRemoveFile)
    {
        ...
        if (files.Count() == 0)
        {
            cbMessageBox(_("This project does not contain any files to remove."), _("Error"),
                         wxICON_WARNING);
            return;
        }
        files.Sort();
    ...
    else if (event.GetId() == idMenuRemoveFolderFilesPopup)
    {
        ...
        if (cbMessageBox(_("Are you sure you want to recursively remove from the project all the "
...
void ProjectManagerUI::OnCloseProject(wxCommandEvent &WXUNUSED(event))
{
    ...
    for (std::set<cbProject *>::iterator it = projectsToClose.begin(); it != projectsToClose.end();
         ++it)
        pm->CloseProject(*it);
...
void ProjectManagerUI::OnOpenFolderFiles(wxCommandEvent &event)
{
    ...
    if (cbMessageBox(_("Are you sure you want to recursively open from the project all the files "
                       "under this folder?"),
                     _("Confirmation"), wxICON_QUESTION | wxYES_NO | wxNO_DEFAULT)
        != wxID_YES)
    {
...
void ProjectManagerUI::OnFileOptions(wxCommandEvent &event)
{
    ...
    else if (event.GetId() == idMenuTreeOptionsEnableBoth
...
struct ProjectFileRelativePathCmp
{
    ...
    {
        if (pf1->GetParentProject() == m_pActiveProject
            && pf2->GetParentProject() != m_pActiveProject)
            return true;
        else if (pf1->GetParentProject() != m_pActiveProject
                 && pf2->GetParentProject() == m_pActiveProject)
            return false;
        else
        {
...
void ProjectManagerUI::OnGotoFile(cb_unused wxCommandEvent &event)
{
    ...
    for (size_t prjIdx = 0; prjIdx < pa->GetCount(); ++prjIdx)
    {
        ...
        for (FilesList::iterator it = prj->GetFilesList().begin(); it != prj->GetFilesList().end();
             ++it)
        {
    ...
    struct Iterator : IncrementalSelectIteratorIndexed
    {
        ...
        int GetTotalCount() const override { return m_pfiles.size(); }
        ...
        int GetColumnWidth(int column) const override { return m_ColumnWidth; }
    ...
    private:
        ...
        {
            if (m_ShowProject)
                return pf.relativeFilename + wxT(" (") + pf.GetParentProject()->GetTitle()
                       + wxT(")");
            else
    ...
    GotoFile dlg(Manager::Get()->GetAppWindow(), &iterator, _("Select file..."),
                 _("Please select file to open:"));
    PlaceWindow(&dlg);
    if (dlg.ShowModal() == wxID_OK)
    {
...
void ProjectManagerUI::OnViewCategorize(wxCommandEvent &event)
{
    ...
    Manager::Get()
        ->GetConfigManager(_T("project_manager"))
        ->Write(_T("/categorize_tree"), do_categorise);
...
void ProjectManagerUI::OnViewUseFolders(wxCommandEvent &event)
{
    ...
    Manager::Get()
        ->GetConfigManager(_T("project_manager"))
        ->Write(_T("/use_folders"), do_use_folders);
    ...
    if (do_use_folders)
    {
        ...
        Manager::Get()
            ->GetConfigManager(_T("project_manager"))
            ->Write(_T("/hide_folder_name"), false);
    }
...
void ProjectManagerUI::OnViewHideFolderName(wxCommandEvent &event)
{
    ...
    Manager::Get()
        ->GetConfigManager(_T("project_manager"))
        ->Write(_T("/hide_folder_name"), do_hide_folder_name);
...
void ProjectManagerUI::OnFindFile(cb_unused wxCommandEvent &event)
{
    ...
    struct Iterator : IncrementalSelectIteratorIndexed
    {
        ...
        int GetTotalCount() const override { return m_files.size(); }
        const wxString &GetItemFilterString(int index) const override { return m_files[index]; }
        wxString GetDisplayText(int index, int column) const override
        {
    ...
    if (item.IsOk())
    {
        ...
        if (ftd && chkOpen->IsChecked())
        {
            ...
            else if (ftd->GetKind() == FileTreeData::ftdkProject && ftd->GetProject())
            {
...
{
    wxString fld =
        cbGetTextFromUser(_("Please enter the new virtual folder path:"), _("New virtual folder"));
    if (fld.IsEmpty())
        return;
...
void ProjectManagerUI::OnRenameVirtualFolder(wxCommandEvent &event)
{
    ...
    wxTextEntryDialog dlg(Manager::Get()->GetAppWindow(),
                          ...
                          _("Rename Virtual Folder"), oldName, wxOK | wxCANCEL | wxCENTRE);
    if (dlg.ShowModal() == wxID_OK)
    {
...
void ProjectManagerUI::OnRenameFile(cb_unused wxCommandEvent &event)
{
    ...
    wxTextEntryDialog dlg(Manager::Get()->GetAppWindow(), _("Please enter the new name:"),
                          _("Rename file"), name, wxOK | wxCANCEL | wxCENTRE);
    PlaceWindow(&dlg);
    if (dlg.ShowModal() == wxID_OK)
    {
...
void ProjectManagerUI::OnKeyDown(wxTreeEvent &event)
{
    ...
    if (prj && (prj->GetCurrentlyCompilingTarget() == nullptr)
...
bool ProjectManagerUI::QueryCloseWorkspace()
{
    ...
    bool blankWorkspace = Manager::Get()->GetConfigManager(_T("app"))->ReadBool(
        _T("/environment/blank_workspace"), true);
    if (!(wkspc->IsDefault() && blankWorkspace))
    {
        ...
        if (wkspc->GetModified())
        {
            ...
            wxString msg;
            msg.Printf(_("Workspace '%s' is modified. Do you want to save it?"),
                       wkspc->GetTitle().c_str());
            switch (cbMessageBox(msg, _("Save workspace"), wxYES_NO | wxCANCEL | wxICON_QUESTION))
            {
                case wxID_YES:
...
int ProjectManagerUI::AskForBuildTargetIndex(cbProject *project)
{
    ...
    int target = cbGetSingleChoiceIndex(_("Select the target:"), _("Project targets"), array,
                                        m_pTree, wxSize(300, 400));
...
{
    if (m_isCheckingForExternallyModifiedProjects) // for some reason, a mutex locker does not
                                                   // work???
        return;
            ...
            { // was modified -> reload
                ...
                if (!reloadAll)
                {
                    ...
                    msg.Printf(_("Project %s is modified outside the IDE...\nDo you want to reload "
                                 "it (you will lose any unsaved work)?"),
...
wxTreeItemId ProjectFindNodeToInsertAfter(wxTreeCtrl *tree, const wxString &text,
                                          const wxTreeItemId &parent, bool in_folders)
{
    ...
    if (tree && parent.IsOk())
    {
        ...
        {
            bool is_folder =
                tree->GetItemImage(child) == fldIdx || tree->GetItemImage(child) == vfldIdx;
...
wxTreeItemId ProjectAddTreeNode(cbProject *project, wxTreeCtrl *tree, const wxString &text,
    ...
    if (useFolders && pos >= 0)
    {
        ...
        if (!newparent)
        {
            ...
            if (folders_kind != FileTreeData::ftdkVirtualFolder)
                ftd->SetFolder(project->GetCommonTopLevelPath()
                               + GetRelativeFolderPath(tree, parent) + folder + wxFILE_SEP_PATH);
            else
        ...
        ret = ProjectAddTreeNode(project, tree, path, newparent, true, folders_kind, compiles,
                                 image, data);
    }
    else
    {
...
void ProjectCopyTreeNodeRecursively(wxTreeCtrl *tree, const wxTreeItemId &item,
                                    const wxTreeItemId &new_parent)
{
    ...
    wxTreeItemId insert =
        ProjectFindNodeToInsertAfter(tree, tree->GetItemText(item), new_parent,
                                     ftd && ftd->GetKind() == FileTreeData::ftdkVirtualFolder);
    wxTreeItemId target =
        tree->InsertItem(new_parent, insert, tree->GetItemText(item), idx, idx, ftd_moved);
                                 ...
                                 wxTreeItemId to)
{
        ...
        && ftdTo->GetKind() == FileTreeData::ftdkVirtualFolder)
    {
        ...
        for (size_t i = 0; i < oldArray.GetCount(); ++i)
        {
            ...
            else
            {
                ...
                {
                    cbMessageBox(_("Another Virtual folder with same name exists in the "
                                   "destination folder!"),
    ...
    else if (ftdFrom->GetKind() == FileTreeData::ftdkVirtualFolder
                        ...
                        wxTreeItemId to)
{
    ...
    wxTreeItemId parentTo =
        ftdTo->GetKind() == FileTreeData::ftdkFile ? tree->GetItemParent(to) : to;
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        // allow only if a file or vfolder was dragged on a file, another vfolder or the project
        // itself
        ...
        wxTreeItemId parentFrom =
            ftdFrom->GetKind() == FileTreeData::ftdkFile ? tree->GetItemParent(from) : from;
...
bool ProjectHasVirtualFolder(const wxString &folderName, const wxArrayString &virtualFolders)
{
    for (size_t i = 0; i < virtualFolders.GetCount(); ++i)
    {
        ...
        {
            cbMessageBox(_("A virtual folder with the same name already exists."), _("Error"),
                         wxICON_WARNING);
            return false;
        }
    }
                               ...
                               const wxString &virtual_folder)
{
    ...
    //    Manager::Get()->GetLogManager()->DebugLog(F(_T("VirtualFolderAdded: %s: %s"),
    //    foldername.c_str(), GetStringFromArray(m_VirtualFolders, _T(";")).c_str()));
    return true;
}
...
void ProjectVirtualFolderDeleted(cbProject *project, wxTreeCtrl *tree, wxTreeItemId node)
{
    ...
    //    Manager::Get()->GetLogManager()->DebugLog(F(_T("VirtualFolderDeleted: %s: %s"),
    //    foldername.c_str(), GetStringFromArray(m_VirtualFolders, _T(";")).c_str()));
}
                                 ...
                                 const wxString &new_name)
{
    ...
    wxString new_foldername =
        GetRelativeFolderPath(tree, tree->GetItemParent(node)) + new_name + wxFILE_SEP_PATH;
    ...
    //    Manager::Get()->GetLogManager()->DebugLog(F(_T("VirtualFolderRenamed: %s to %s: %s"),
    //    old_foldername.c_str(), new_foldername.c_str(), GetStringFromArray(m_VirtualFolders,
    //    _T(";")).c_str()));
    return true;
}
                                        ...
                                        FilesGroupsAndMasks *fgam)
{
    ...
    if (do_categorise)
    {
        ...
        for (unsigned int i = 0; i < fgam->GetGroupsCount(); ++i)
        {
            ...
            pGroupNodes[i] = tree->AppendItem(project->GetProjectNode(), fgam->GetGroupName(i),
                                              fldIdx, fldIdx, ftd);
        }
        // add a default category "Generated" for all auto-generated file types
        ftd = new FileTreeData(project, FileTreeData::ftdkVirtualGroup);
        generated =
            tree->AppendItem(project->GetProjectNode(), _("Auto-generated"), fldIdx, fldIdx, ftd);
    ...
    for (FilesList::iterator it = fileList.begin(); it != fileList.end(); ++it)
    {
        ...
        if (!pf)
        {
            Manager::Get()->GetLogManager()->DebugLogError(
                _T("Looks like the project's file list is broken?!"));
            continue;
        }
        ...
        // by default, the parent node is the project node (in case of no grouping, no virtual
        // folders)
        ...
        if (!pf->virtual_path.IsEmpty())
        {
            ...
            wxString slash =
                pf->virtual_path.Last() == wxFILE_SEP_PATH ? _T("") : wxString(wxFILE_SEP_PATH);
        ...
        bool useFolders =
            (ptvs & ptvsUseFolders) || (folders_kind == FileTreeData::ftdkVirtualFolder);
        pf->SetTreeItemId(ProjectAddTreeNode(project, tree, nodetext, parentNode, useFolders,
                                             folders_kind, pf->compile, (int)pf->GetFileState(),
                                             ftd));
...
#ifdef fileload_measuring
    Manager::Get()->GetLogManager()->DebugLogError(
        F(_T("%s::%s:%d  took : %d ms"), cbC2U(__FILE__).c_str(),
          cbC2U(__PRETTY_FUNCTION__).c_str(), __LINE__, (int)sw.Time()));
#endif
}
