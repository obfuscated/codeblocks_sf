...
struct ToolbarInfo
{
    ...
    ToolbarInfo(wxToolBar *toolbar_in, const wxAuiPaneInfo &paneInfo_in, int priority_in)
      : paneInfo(paneInfo_in), toolbar(toolbar_in), priority(priority_in)
    {
    }

    bool operator<(const ToolbarInfo &b) const { return priority < b.priority; }
...
protected:
    ...
    void CreateToolbars();
    void ScanForPlugins();
    void AddToolbarItem(int id, const wxString &title, const wxString &shortHelp,
    ...
    wxMenuItem *AddPluginInMenus(wxMenu *menu, cbPlugin *plugin, wxObjectEventFunction callback,
                                 int pos = -1, bool checkable = false);

    void LoadViewLayout(const wxString &name, bool isTemp = false);
    void SaveViewLayout(const wxString &name, const wxString &layout, const wxString &layoutMP,
                        bool select = false);
...
protected:
    ...
    int m_LastCtrlAltTabWindow; //!< Last window focussed in the cycle 1 = Mgmt. panel, 2 = Editor,
                                //!< 3 = Logs & others
    ...
    typedef std::map<int, const wxString>
        MenuIDToScript; // script menuitem ID -> script function name
