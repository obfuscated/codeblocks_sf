...
#if wxCHECK_VERSION(3, 0, 0)
bool DDEConnection::OnExecute(cb_unused const wxString &topic, const void *data,
                              cb_unused size_t size, cb_unused wxIPCFormat format)
#else
bool DDEConnection::OnExecute(cb_unused const wxString &topic, wxChar *data, cb_unused int size,
                              cb_unused wxIPCFormat format)
#endif
{
    wxString strData((wxChar *)data);

    if (strData.StartsWith(_T("[IfExec_Open(\"")))
        return false; // let Shell Open handle the request as we *know* that we have registered the
                      // Shell Open command, too
    ...
    if (strData.StartsWith(_T("[Open(\"")))
    {
        ...
        if (reCmd.Matches(strData))
        {
            ...
            // always put files in the delayed queue, the will either be loaded in OnDisconnect, or
            // after creating of MainFrame if we open the files directly it can lead to an
            // applicaton hang (at least when opening C::B's project-file on linux)
    ...
    else if (strData.StartsWith(_T("[CmdLine({")))
    {
        ...
        if (pos != wxNOT_FOUND)
        {
            ...
            {
                // Manager::Get()->GetLogManager()->Log(wxString::Format(_T("DDEConnection::OnExecute
                // line = ") + line));
    ...
    wxSafeShowMessage(wxT("Warning"),
                      wxString::Format(wxT("DDE topic %s not handled."), strData.wx_str()));
    return false;
}
...
const wxCmdLineEntryDesc cmdLineDesc[] = {
    ...
    {wxCMD_LINE_SWITCH, CMD_ENTRY("?"), CMD_ENTRY("?"),
     CMD_ENTRY("show this help message (alias for help)"), wxCMD_LINE_VAL_NONE,
     wxCMD_LINE_OPTION_HELP},
    {wxCMD_LINE_SWITCH, CMD_ENTRY(""), CMD_ENTRY("safe-mode"),
     CMD_ENTRY("load in safe mode (all plugins will be disabled)"), wxCMD_LINE_VAL_NONE,
     wxCMD_LINE_PARAM_OPTIONAL},
#ifdef __WXMSW__
    {wxCMD_LINE_SWITCH, CMD_ENTRY("na"), CMD_ENTRY("no-check-associations"),
     CMD_ENTRY("don't perform any association checks"), wxCMD_LINE_VAL_NONE,
     wxCMD_LINE_PARAM_OPTIONAL},
    {wxCMD_LINE_SWITCH, CMD_ENTRY("nd"), CMD_ENTRY("no-dde"), CMD_ENTRY("don't start a DDE server"),
     wxCMD_LINE_VAL_NONE, wxCMD_LINE_PARAM_OPTIONAL},
#endif
#ifndef __WXMSW__
    {wxCMD_LINE_SWITCH, CMD_ENTRY("ni"), CMD_ENTRY("no-ipc"),
     CMD_ENTRY("don't start an IPC server"), wxCMD_LINE_VAL_NONE, wxCMD_LINE_PARAM_OPTIONAL},
#endif
    {wxCMD_LINE_SWITCH, CMD_ENTRY("ns"), CMD_ENTRY("no-splash-screen"),
     CMD_ENTRY("don't display a splash screen while loading"), wxCMD_LINE_VAL_NONE,
     wxCMD_LINE_PARAM_OPTIONAL},
    {wxCMD_LINE_SWITCH, CMD_ENTRY(""), CMD_ENTRY("multiple-instance"),
     CMD_ENTRY("allow running multiple instances"), wxCMD_LINE_VAL_NONE, wxCMD_LINE_PARAM_OPTIONAL},
    {wxCMD_LINE_SWITCH, CMD_ENTRY("d"), CMD_ENTRY("debug-log"),
     CMD_ENTRY("display application's debug log"), wxCMD_LINE_VAL_NONE, wxCMD_LINE_PARAM_OPTIONAL},
    {wxCMD_LINE_SWITCH, CMD_ENTRY("nc"), CMD_ENTRY("no-crash-handler"),
     CMD_ENTRY("don't use the crash handler (useful for debugging C::B)"), wxCMD_LINE_VAL_NONE,
     wxCMD_LINE_PARAM_OPTIONAL},
    {wxCMD_LINE_SWITCH, CMD_ENTRY("v"), CMD_ENTRY("verbose"),
     CMD_ENTRY("show more debugging messages"), wxCMD_LINE_VAL_NONE, wxCMD_LINE_PARAM_OPTIONAL},
    ...
    {wxCMD_LINE_OPTION, CMD_ENTRY(""), CMD_ENTRY("user-data-dir"),
     CMD_ENTRY("set a custom location for user settings and plugins"), wxCMD_LINE_VAL_STRING,
     wxCMD_LINE_NEEDS_SEPARATOR},
    {wxCMD_LINE_OPTION, CMD_ENTRY("p"), CMD_ENTRY("personality"),
     CMD_ENTRY("the personality to use: \"ask\" or <personality-name>"), wxCMD_LINE_VAL_STRING,
     wxCMD_LINE_NEEDS_SEPARATOR},
    {wxCMD_LINE_SWITCH, CMD_ENTRY(""), CMD_ENTRY("no-log"),
     CMD_ENTRY("turn off the application log"), wxCMD_LINE_VAL_NONE, wxCMD_LINE_PARAM_OPTIONAL},
    {wxCMD_LINE_SWITCH, CMD_ENTRY(""), CMD_ENTRY("log-to-file"),
     CMD_ENTRY("redirect application log to a file"), wxCMD_LINE_VAL_NONE,
     wxCMD_LINE_PARAM_OPTIONAL},
    {wxCMD_LINE_SWITCH, CMD_ENTRY(""), CMD_ENTRY("debug-log-to-file"),
     CMD_ENTRY("redirect application debug log to a file"), wxCMD_LINE_VAL_NONE,
     wxCMD_LINE_PARAM_OPTIONAL},
    {wxCMD_LINE_OPTION, CMD_ENTRY(""), CMD_ENTRY("profile"), CMD_ENTRY("synonym to personality"),
     wxCMD_LINE_VAL_STRING, wxCMD_LINE_NEEDS_SEPARATOR},
    {wxCMD_LINE_SWITCH, CMD_ENTRY(""), CMD_ENTRY("rebuild"),
     CMD_ENTRY("clean and then build the project/workspace"), wxCMD_LINE_VAL_NONE,
     wxCMD_LINE_PARAM_OPTIONAL},
    {wxCMD_LINE_SWITCH, CMD_ENTRY(""), CMD_ENTRY("build"),
     CMD_ENTRY("just build the project/workspace"), wxCMD_LINE_VAL_NONE, wxCMD_LINE_PARAM_OPTIONAL},
    {wxCMD_LINE_SWITCH, CMD_ENTRY(""), CMD_ENTRY("clean"), CMD_ENTRY("clean the project/workspace"),
     wxCMD_LINE_VAL_NONE, wxCMD_LINE_PARAM_OPTIONAL},
    {wxCMD_LINE_OPTION, CMD_ENTRY(""), CMD_ENTRY("target"),
     CMD_ENTRY("the target for the batch build"), wxCMD_LINE_VAL_STRING,
     wxCMD_LINE_NEEDS_SEPARATOR},
    {wxCMD_LINE_SWITCH, CMD_ENTRY(""), CMD_ENTRY("no-batch-window-close"),
     CMD_ENTRY("do not auto-close log window when batch build is done"), wxCMD_LINE_VAL_NONE,
     wxCMD_LINE_PARAM_OPTIONAL},
    {wxCMD_LINE_SWITCH, CMD_ENTRY(""), CMD_ENTRY("batch-build-notify"),
     CMD_ENTRY("show message when batch build is done"), wxCMD_LINE_VAL_NONE,
     wxCMD_LINE_PARAM_OPTIONAL},
    {wxCMD_LINE_OPTION, CMD_ENTRY(""), CMD_ENTRY("script"), CMD_ENTRY("execute script file"),
     wxCMD_LINE_VAL_STRING, wxCMD_LINE_NEEDS_SEPARATOR},
    {wxCMD_LINE_OPTION, CMD_ENTRY(""), CMD_ENTRY("file"),
     CMD_ENTRY("open file and optionally jump to specific line (file[:line])"),
     wxCMD_LINE_VAL_STRING, wxCMD_LINE_NEEDS_SEPARATOR},
...
public:
    Splash(const bool show) : m_pSplash(nullptr)
    {
        ...
        {
            wxBitmap bmp =
                cbLoadBitmap(ConfigManager::ReadDataPath() + _T("/images/splash_1312.png"));
            m_pSplash = new cbSplashScreen(bmp, -1, nullptr, -1,
                                           wxNO_BORDER | wxFRAME_NO_TASKBAR | wxFRAME_SHAPED);
            Manager::Yield();
        }
    }
    ~Splash() { Hide(); }
...
IMPLEMENT_APP(CodeBlocksApp) // TODO: This gives a "redundant declaration" warning, though I think
                             // it's false. Dig through macro and check.
...
bool CodeBlocksApp::InitXRCStuff()
{
    if (!Manager::LoadResource(_T("resources.zip")))
    {
        ...
        msg.Printf(_T("Cannot find resources...\n"
                   ...
                   appglobals::AppName.wx_str(), ConfigManager::ReadDataPath().wx_str(),
...
{
    // This is a remnant from early 2006 (Windows only), but keep the revision tag for possible
    // future use
    ConfigManager *cfg = Manager::Get()->GetConfigManager(_T("app"));
...
void CodeBlocksApp::InitLocale()
{
    ...
    if (!lang.IsEmpty()) // Note: You can also write this line of code as !(!lang) from wx-2.9
                         // onwards
...
bool CodeBlocksApp::OnInit()
{
    ...
    try
    {
        ...
        delete wxMessageOutput::Set(new cbMessageOutputNull); // No output. (suppress warnings about
                                                              // unknown options from plugins)
        ...
        if (m_DDE && !m_Batch
            && Manager::Get()->GetConfigManager(_T("app"))->ReadBool(_T("/environment/use_ipc"),
                                                                     true))
        {
            // Create a new client
            ...
            connection = (DDEConnection *)client->MakeConnection(
                _T("localhost"), F(DDE_SERVICE, wxGetUserId().wx_str()), DDE_TOPIC);
            ...
            if (connection)
            {
                ...
                if (!cmdLine.IsEmpty())
                {
                    // escape openings and closings so it is easily possible to find the end on the
                    // rx side
                ...
                if (Manager::Get()->GetConfigManager(_T("app"))->ReadBool(
                        _T("/environment/raise_via_ipc"), true))
            ...
            // free memory DDE-/IPC-clients, if we are here connection could not be established and
        ...
        if (Manager::Get()->GetConfigManager(_T("app"))->ReadBool(
                _T("/environment/single_instance"), true)
            && !parser.Found(_T("multiple-instance")))
        {
            ...
            if (m_pSingleInstance->IsAnotherRunning())
            {
                ...
                cbMessageBox(_("Another program instance is already running.\nCode::Blocks is "
                               "currently configured to only allow one running instance.\n\nYou "
                               "can access this Setting under the menu item 'Environment'."),
                             _T("Code::Blocks"), wxOK | wxICON_ERROR);
                return false;
            }
        }
        // Splash screen moved to this place, otherwise it would be short visible, even if we only
        // pass filenames via DDE/IPC we also don't need it, if only a single instance is allowed
        Splash splash(!m_Batch && m_Script.IsEmpty() && m_Splash
                      && Manager::Get()->GetConfigManager(_T("app"))->ReadBool(
                             _T("/environment/show_splash"), true));
        InitDebugConsole();
        ...
        if (m_Batch)
        {
            ...
            Manager::Get()->RegisterEventSink(cbEVT_COMPILER_FINISHED,
                                              new cbEventFunctor<CodeBlocksApp, CodeBlocksEvent>(
                                                  this, &CodeBlocksApp::OnBatchBuildDone));
            s_Loading = false;
        ...
        try
        {
            wxString startup = ConfigManager::LocateDataFile(_T("startup.script"),
    ...
    catch (...)
    {
        wxSafeShowMessage(
            _T("Exception"),
            _T("Unknown exception was raised. The application will terminate immediately..."));
    }
...
inline void EnableLFH()
{
    ...
    HeapSetInformation_t HeapSetInformation_func =
        (HeapSetInformation_t)GetProcAddress(kh, "HeapSetInformation");
    GetProcessHeaps_t GetProcessHeaps_func =
        (GetProcessHeaps_t)GetProcAddress(kh, "GetProcessHeaps");
    ...
    if (GetProcessHeaps_func && HeapSetInformation_func)
    {
        ...
        for (int i = 0; i < n; ++i)
            HeapSetInformation_func(h[i], HeapCompatibilityInformation, &HeapFragValue,
...
int CodeBlocksApp::OnRun()
{
    ...
    catch (...)
    {
        wxSafeShowMessage(
            _("Exception"),
            _("Unknown exception was raised. The application will terminate immediately..."));
    }
    // if we reached here, return error
    return -1;
}
...
#else
    cbMessageBox(
        wxString::Format(_("Something has gone wrong inside %s and it will terminate immediately.\n"
...
int CodeBlocksApp::BatchJob()
{
    ...
    wxString title = _("Building '") + wxFileNameFromPath(wxString(argv[argc - 1]))
                     + _("' (target '") + m_BatchTarget + _T("')");
...
void CodeBlocksApp::OnBatchBuildDone(CodeBlocksEvent &event)
{
    ...
    if (m_BatchNotify)
    {
        ...
        cbMessageBox(msg, appglobals::AppName,
                     m_BatchExitCode == 0 ? wxICON_INFORMATION : wxICON_WARNING,
                     m_pBatchBuildDialog);
    }
    else
        wxBell();
    ...
    else
    {
        if (handlerFrame)
        {
            ...
            for (int param = 0; param < count; ++param)
            {
                ...
                fn.Normalize(); // really important so that two same files with different names are
                                // not loaded twice
                ...
                else if (wxFile::Exists(fn.GetFullPath())) // also try to open non source, header
                                                           // and resource files
                {
        ...
        else
        {
            ...
            if (parser.Found(_T("log-to-file")))
                Manager::Get()->GetLogManager()->SetLog(new FileLogger(_T("codeblocks.log")),
                                                        LogManager::app_log);
            if (m_HasDebugLog)
                Manager::Get()->GetLogManager()->SetLog(new TextCtrlLogger, LogManager::debug_log);
            if (parser.Found(_T("debug-log-to-file")))
                Manager::Get()->GetLogManager()->SetLog(new FileLogger(_T("codeblocks-debug.log")),
                                                        LogManager::debug_log);
        }
    }
#endif // wxUSE_CMDLINE_PARSER
...
void CodeBlocksApp::SetupPersonality(const wxString &personality)
{
    if (personality.CmpNoCase(_T("ask")) == 0)
    {
        ...
        wxSingleChoiceDialog dlg(nullptr, _("Please choose which personality (profile) to load:"),
                                 _("Personalities (profiles)"), items);
...
void CodeBlocksApp::LoadDelayedFiles(MainFrame *const frame)
{
    ...
    for (std::set<wxString>::const_iterator it = uniqueFilesToOpen.begin();
         it != uniqueFilesToOpen.end(); ++it)
        frame->Open(*it, true);
    ...
    if (!m_AutoFile.IsEmpty())
    {
        ...
        if (linePos != wxNOT_FOUND)
        {
            ...
            {
                // on windows, if :line is omitted: c:\foo\bar.h -> \foo\bar.h is not the line
                // number!
                filePart = m_AutoFile;
            }
        }
        wxFileName fn(filePart);
        fn.Normalize(); // really important so that two same files with different names are not
                        // loaded twice
...
void CodeBlocksApp::OnAppActivate(wxActivateEvent &event)
{
    ...
    if (!m_Batch && Manager::Get()->GetEditorManager()
        && Manager::Get()->GetConfigManager(_T("app"))->ReadBool(
               _T("/environment/check_modified_files"), true))
    {
        ...
        // move it around
        // so : idEditorManagerCheckFiles, EditorManager::OnCheckForModifiedFiles just exist for
        // this workaround
