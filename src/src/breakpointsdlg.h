...
private:
    ...
    struct Item
    {
        Item(const cb::shared_ptr<cbBreakpoint> &breakpoint_in, cbDebuggerPlugin *plugin_in,
             wxString const &pluginName_in)
          : breakpoint(breakpoint_in), plugin(plugin_in), pluginName(pluginName_in)
        {
        }
