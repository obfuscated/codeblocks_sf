...
static const int infopane_flags = wxAUI_NB_MIDDLE_CLICK_CLOSE | wxAUI_NB_WINDOWLIST_BUTTON
...
class InfoPane : public cbAuiNotebook
{
    ...
    struct Page
    {
        Page()
          : icon(nullptr),
            window(nullptr),
            logger(nullptr),
            indexInNB(std::numeric_limits<int>::min()),
            eventID(0),
            islogger(0){};
        wxString title;
        wxBitmap *icon;
    ...
    bool InsertPagePrivate(wxWindow *p, const wxString &title, wxBitmap *icon = nullptr,
                           int index = -1);
    using wxWindow::Show;
...
public:
    ...
    void UpdateEffectiveTabOrder(); // refreshes the tab effective order, needed, because tabs might
                                    // have moved with drag and drop
    void LoadTabOrder(wxString layout);
    wxString SaveTabOrder();

    /*
     *  You should not need to call these functions under normal conditions. The application
     * initialises everything at startup, after the main window has been created and the plugins
     * have been loaded. AddLogger: Add the GUI control obtained by a logger's CreateControl()
     * function. DeleteLogger: Remove (delete) a tab and its logger. All log output directed to the
     * corresponding log index will be redirected to the null log thereafter. To prove that you are
     * serious, you must know the logger belonging to the tab to delete.
     */
    bool AddLogger(Logger *logger, wxWindow *p, const wxString &title, wxBitmap *icon = nullptr);
    bool DeleteLogger(Logger *l);

    /*
     *  If something that is not a logger should be placed in the info pane (think twice about this
     * before doing it), use AddNonLogger()/DeleteNonLogger() for that purpose. An example of
     * something that is not a logger but might still show up in the info pane is the list of
     * search results.
     */
