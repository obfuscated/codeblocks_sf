...
{
    Associations::DoSetAssociation(knownTypes[index].ext, knownTypes[index].descr, executable,
                                   knownTypes[index].index);
};

inline bool DoCheckAssociation(const wxString &executable, int index)
{
    return Associations::DoCheckAssociation(knownTypes[index].ext, knownTypes[index].descr,
                                            executable, knownTypes[index].index);
};
...
void Associations::DoSetAssociation(const wxString &ext, const wxString &descr, const wxString &exe,
                                    int icoNum)
{
    ...
    {
        wxString batchbuildargs = Manager::Get()->GetConfigManager(_T("app"))->Read(
...
bool Associations::DoCheckAssociation(const wxString &ext, cb_unused const wxString &descr,
                                      const wxString &exe, int icoNum)
{
    wxString BaseKeyName(_T("HKEY_CURRENT_USER\\Software\\Classes\\"));
    ...
    if (ext.IsSameAs(FileFilters::CODEBLOCKS_EXT) || ext.IsSameAs(FileFilters::WORKSPACE_EXT))
    {
        ...
        key.SetName(BaseKeyName + _T("CodeBlocks.") + ext
                    + _T("\\shell\\Rebuild (clean)\\command"));
        if (!key.Open())
            return false;
        if (!key.QueryValue(wxEmptyString, strVal))
            return false;
...
ManageAssocsDialog::ManageAssocsDialog(wxWindow *parent)
{
    ...
    for (unsigned int i = 0; i < Associations::CountAssocs(); ++i)
    {
        ...
        list->Check(i, Associations::DoCheckAssociation(knownTypes[i].ext, knownTypes[i].descr, exe,
                                                        knownTypes[i].index));
    }
