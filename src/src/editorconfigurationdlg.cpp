...
const wxString base_imgs[] = {
    _T("editor"), _T("folding"), _T("gutter-margin"), _T("syntax-highlight"), _T("default-code"),
};
...
{
    wxXmlResource::Get()->LoadObject(this, parent, _T("dlgConfigureEditor"),
    ...
    XRCCTRL(*this, "chkSmartIndent", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/smart_indent"), true));
    XRCCTRL(*this, "chkBraceCompletion", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/brace_completion"), true));
    XRCCTRL(*this, "chkDetectIndent", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/detect_indent"), false));
    ...
    XRCCTRL(*this, "chkShowIndentGuides", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/show_indent_guides"), false));
    XRCCTRL(*this, "chkBraceSmartIndent", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/brace_smart_indent"), true));
    XRCCTRL(*this, "chkSelectionBraceCompletion", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/selection_brace_completion"), false));
    XRCCTRL(*this, "chkTabIndents", wxCheckBox)->SetValue(cfg->ReadBool(_T("/tab_indents"), true));
    XRCCTRL(*this, "chkBackspaceUnindents", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/backspace_unindents"), true));
    XRCCTRL(*this, "chkWordWrap", wxCheckBox)->SetValue(cfg->ReadBool(_T("/word_wrap"), false));
    XRCCTRL(*this, "chkWordWrapStyleHomeEnd", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/word_wrap_style_home_end"), true));
    XRCCTRL(*this, "chkPosixRegex", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/use_posix_style_regexes"), false));
#ifdef wxHAS_REGEX_ADVANCED
    XRCCTRL(*this, "chkAdvancedRegex", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/use_advanced_regexes"), false));
#else
    XRCCTRL(*this, "chkAdvancedRegex", wxCheckBox)->SetValue(false);
    XRCCTRL(*this, "chkAdvancedRegex", wxCheckBox)->Enable(false);
#endif
    XRCCTRL(*this, "chkShowLineNumbers", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/show_line_numbers"), true));
    XRCCTRL(*this, "chkHighlightCaretLine", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/highlight_caret_line"), false));
    XRCCTRL(*this, "chkSimplifiedHome", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/simplified_home"), false));
    ...
    XRCCTRL(*this, "chkSyncEditorWithProjectManager", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/sync_editor_with_project_manager"), false));
    ...
    XRCCTRL(*this, "rbTabText", wxRadioBox)
        ->SetSelection(cfg->ReadBool(_T("/tab_text_relative"), false) ? 1 : 0);

    XRCCTRL(*this, "chkTrackPreprocessor", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/track_preprocessor"), true));
    XRCCTRL(*this, "chkCollectPrjDefines", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/collect_prj_defines"), true));
    XRCCTRL(*this, "chkPlatDefines", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/platform_defines"), false));
    XRCCTRL(*this, "chkColoursWxSmith", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/highlight_wxsmith"), true));
    ...
    XRCCTRL(*this, "chkStripTrailings", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/eol/strip_trailing_spaces"), true));
    XRCCTRL(*this, "chkEnsureFinalEOL", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/eol/ensure_final_line_end"), true));
    XRCCTRL(*this, "chkEnsureConsistentEOL", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/eol/ensure_consistent_line_ends"), false));
    // NOTE: duplicate line in cbeditor.cpp (CreateEditor)
    XRCCTRL(*this, "cmbEOLMode", wxChoice)
        ->SetSelection(cfg->ReadInt(
            _T("/eol/eolmode"),
            platform::windows ? wxSCI_EOL_CRLF
                              : wxSCI_EOL_LF)); // Windows takes CR+LF, other platforms LF only
    ...
    // whitespace colour
    wxColour whiteSpaceColour =
        Manager::Get()->GetColourManager()->GetColour(wxT("editor_whitespace"));
    XRCCTRL(*this, "btnWSColour", wxButton)->SetBackgroundColour(whiteSpaceColour);

    // selections
    XRCCTRL(*this, "chkEnableVirtualSpace", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/selection/use_vspace"), false));
    XRCCTRL(*this, "chkEnableRectVirtualSpace", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/selection/use_rect_vspace"), false));
    ...
    XRCCTRL(*this, "chkEnableAdditionalSelectionTyping", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/selection/multi_typing"), false));
    XRCCTRL(*this, "chkEnableAdditionalSelectionTyping", wxCheckBox)->Enable(multiSelectEnabled);

    // folding
    XRCCTRL(*this, "chkEnableFolding", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/folding/show_folds"), true));
    XRCCTRL(*this, "chkFoldOnOpen", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/folding/fold_all_on_open"), false));
    XRCCTRL(*this, "chkFoldPreprocessor", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/folding/fold_preprocessor"), false));
    XRCCTRL(*this, "chkFoldComments", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/folding/fold_comments"), true));
    XRCCTRL(*this, "chkFoldXml", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/folding/fold_xml"), true));
    XRCCTRL(*this, "chkUnderlineFoldedLine", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/folding/underline_folded_line"), true));
    XRCCTRL(*this, "lstIndicators", wxChoice)
        ->SetSelection(cfg->ReadInt(_T("/folding/indicator"), 2));
    XRCCTRL(*this, "chkFoldLimit", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/folding/limit"), false));
    XRCCTRL(*this, "spnFoldLimitLevel", wxSpinCtrl)
        ->SetValue(cfg->ReadInt(_T("/folding/limit_level"), 1));
    ...
    // margin
    XRCCTRL(*this, "spnMarginWidth", wxSpinCtrl)
        ->SetValue(cfg->ReadInt(_T("/margin/width_chars"), 6));
    XRCCTRL(*this, "chkDynamicWidth", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/margin/dynamic_width"), false));
    XRCCTRL(*this, "spnMarginWidth", wxSpinCtrl)
        ->Enable(!cfg->ReadBool(_T("/margin/dynamic_width"), false));
    XRCCTRL(*this, "chkAddBPByLeftClick", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/margin_1_sensitive"), true));
    XRCCTRL(*this, "chkImageBP", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/margin_1_image_bp"), true));
    ...
    {
        cmbEnc->Clear();
        wxString def_enc_name =
            cfg->Read(_T("/default_encoding"), wxLocale::GetSystemEncodingName());
        int sel = 0;
    ...
    XRCCTRL(*this, "rbEncodingUseOption", wxRadioBox)
        ->SetSelection(cfg->ReadInt(_T("/default_encoding/use_option"), 0));
    XRCCTRL(*this, "chkEncodingFindLatin2", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/default_encoding/find_latin2"), false));
    XRCCTRL(*this, "chkEncodingUseSystem", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/default_encoding/use_system"), true));
    ...
    XRCCTRL(*this, "chkCodeCompletion", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/code_completion"), true));
    XRCCTRL(*this, "chkCCCaseSensitive", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/case_sensitive"), false));
    XRCCTRL(*this, "chkAutoselectSingle", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/auto_select_single"), false));
    XRCCTRL(*this, "spnAutolaunchCount", wxSpinCtrl)
        ->SetValue(cfg->ReadInt(_T("/auto_launch_count"), 3));
    XRCCTRL(*this, "chkDocumentationPopup", wxCheckBox)
        ->SetValue(cfg->ReadBool(_T("/documentation_popup"), true));
    ...
    // the former commented out code leads to problems with wxGTK (parts of long static texts not
    // shown) seems to work without the comments on wx2.8.9 on linux and windows
...
void EditorConfigurationDlg::UpdateListbookImages()
{
    ...
    if (SettingsIconsStyle(Manager::Get()->GetConfigManager(_T("app"))->ReadInt(
            _T("/environment/settings_size"), 0)))
    {
...
void EditorConfigurationDlg::UpdateColourControls(const OptionColour *opt)
{
    if (opt)
    {
        ...
        if (c == wxNullColour)
        {
            XRCCTRL(*this, "btnColoursFore", wxButton)
        ...
        if (c == wxNullColour)
        {
            XRCCTRL(*this, "btnColoursBack", wxButton)
                ->SetBackgroundColour(wxSystemSettings::GetColour(wxSYS_COLOUR_BTNFACE));
...
{
    XRCCTRL(*this, "spnCaretWidth", wxSpinCtrl)
        ->Enable(XRCCTRL(*this, "lstCaretStyle", wxChoice)->GetSelection()
...
void EditorConfigurationDlg::ChangeTheme()
{
    ...
    if (sel == -1)
    {
        wxString lang = Manager::Get()
                            ->GetConfigManager(_T("editor"))
                            ->Read(_T("/colour_sets/active_lang"), _T("C/C++"));
...
bool CheckColourThemeName(const wxString &name, wxWindow *parent)
{
    ...
    else
    {
        cbMessageBox(_("You've entered invalid characters for the name of the theme. "
                       "Only alphanumeric characters and '_' are allowed! The first character "
                       "should be a letter. "
                       "Please try again."),
                     _("Error"), wxOK, parent);
        return false;
...
{
    wxTextEntryDialog dlg(this, _("Please enter the name of the new colour theme:"),
                          _("New theme name"));
    PlaceWindow(&dlg);
    if (dlg.ShowModal() != wxID_OK)
        return;
...
{
    if (cbMessageBox(_("Are you sure you want to delete this theme?"), _("Confirmation"), wxYES_NO,
                     this)
        == wxID_YES)
    {
        Manager::Get()
            ->GetConfigManager(_T("editor"))
            ->DeleteSubPath(_T("/colour_sets/") + m_Theme->GetName());
        wxChoice *cmbThemes = XRCCTRL(*this, "cmbThemes", wxChoice);
        int idx = cmbThemes->FindString(m_Theme->GetName());
        if (idx != wxNOT_FOUND)
...
{
    wxTextEntryDialog dlg(this, _("Please enter the new name of the new colour theme:"),
                          _("New theme name"), m_Theme->GetName());
    PlaceWindow(&dlg);
    if (dlg.ShowModal() != wxID_OK)
        return;
...
void EditorConfigurationDlg::OnEditKeywords(cb_unused wxCommandEvent &event)
{
    if (m_Theme && m_Lang != HL_NONE)
    {
        ...
        if (m_TextColourControl)
            descriptions =
                GetArrayFromString(m_TextColourControl->DescribeKeyWordSets(), wxT("\n"));
...
void EditorConfigurationDlg::OnEditFilemasks(cb_unused wxCommandEvent &event)
{
    ...
    {
        wxString masks =
            cbGetTextFromUser(_("Edit filemasks (use commas to separate them - case insensitive):"),
...
void EditorConfigurationDlg::OnColoursCopyFrom(cb_unused wxCommandEvent &event)
{
    ...
    for (MenuIDToLanguageOption::const_iterator it = m_MenuIDToLanguageOption.begin();
         it != m_MenuIDToLanguageOption.end(); ++it)
    {
...
void EditorConfigurationDlg::OnChooseColour(wxCommandEvent &event)
{
    ...
    if (event.GetId() == XRCID("btnColoursFore") || event.GetId() == XRCID("btnColoursBack"))
        WriteColours();
}
...
void EditorConfigurationDlg::EndModal(int retCode)
{
    if (retCode == wxID_OK)
    {
        ...
        cfg->Write(
            _T("/font"),
            XRCCTRL(*this, "lblEditorFont", wxStaticText)->GetFont().GetNativeFontInfoDesc());
        ...
        cfg->Write(_T("/brace_completion"),
                   XRCCTRL(*this, "chkBraceCompletion", wxCheckBox)->GetValue());
        ...
        cfg->Write(_T("/show_indent_guides"),
                   XRCCTRL(*this, "chkShowIndentGuides", wxCheckBox)->GetValue());
        cfg->Write(_T("/brace_smart_indent"),
                   XRCCTRL(*this, "chkBraceSmartIndent", wxCheckBox)->GetValue());
        cfg->Write(_T("/selection_brace_completion"),
                   XRCCTRL(*this, "chkSelectionBraceCompletion", wxCheckBox)->GetValue());
        cfg->Write(_T("/tab_indents"), XRCCTRL(*this, "chkTabIndents", wxCheckBox)->GetValue());
        cfg->Write(_T("/backspace_unindents"),
                   XRCCTRL(*this, "chkBackspaceUnindents", wxCheckBox)->GetValue());
        cfg->Write(_T("/word_wrap"), XRCCTRL(*this, "chkWordWrap", wxCheckBox)->GetValue());
        cfg->Write(_T("/word_wrap_style_home_end"),
                   XRCCTRL(*this, "chkWordWrapStyleHomeEnd", wxCheckBox)->GetValue());
        cfg->Write(_T("/use_posix_style_regexes"),
                   XRCCTRL(*this, "chkPosixRegex", wxCheckBox)->GetValue());
#ifdef wxHAS_REGEX_ADVANCED
        cfg->Write(_T("/use_advanced_regexes"),
                   XRCCTRL(*this, "chkAdvancedRegex", wxCheckBox)->GetValue());
#endif

        cfg->Write(_T("/show_line_numbers"),
                   XRCCTRL(*this, "chkShowLineNumbers", wxCheckBox)->GetValue());
        cfg->Write(_T("/highlight_caret_line"),
                   XRCCTRL(*this, "chkHighlightCaretLine", wxCheckBox)->GetValue());
        cfg->Write(_T("/simplified_home"),
                   XRCCTRL(*this, "chkSimplifiedHome", wxCheckBox)->GetValue());
        cfg->Write(_T("/camel_case"), XRCCTRL(*this, "chkCamelCase", wxCheckBox)->GetValue());

        cfg->Write(_T("/track_preprocessor"),
                   XRCCTRL(*this, "chkTrackPreprocessor", wxCheckBox)->GetValue());
        cfg->Write(_T("/collect_prj_defines"),
                   XRCCTRL(*this, "chkCollectPrjDefines", wxCheckBox)->GetValue());
        cfg->Write(_T("/platform_defines"),
                   XRCCTRL(*this, "chkPlatDefines", wxCheckBox)->GetValue());
        cfg->Write(_T("/highlight_wxsmith"),
                   XRCCTRL(*this, "chkColoursWxSmith", wxCheckBox)->GetValue());
        ...
        cfg->Write(_T("/sync_editor_with_project_manager"),
                   XRCCTRL(*this, "chkSyncEditorWithProjectManager", wxCheckBox)->GetValue());
        ...
        cfg->Write(_T("/tab_text_relative"),
                   XRCCTRL(*this, "rbTabText", wxRadioBox)->GetSelection() ? true : false);
        ...
        cfg->Write(_T("/folding/fold_all_on_open"),
                   XRCCTRL(*this, "chkFoldOnOpen", wxCheckBox)->GetValue());
        cfg->Write(_T("/folding/fold_preprocessor"),
                   XRCCTRL(*this, "chkFoldPreprocessor", wxCheckBox)->GetValue());
        cfg->Write(_T("/folding/fold_comments"),
                   XRCCTRL(*this, "chkFoldComments", wxCheckBox)->GetValue());
        cfg->Write(_T("/folding/fold_xml"), XRCCTRL(*this, "chkFoldXml", wxCheckBox)->GetValue());
        cfg->Write(_T("/folding/underline_folded_line"),
                   XRCCTRL(*this, "chkUnderlineFoldedLine", wxCheckBox)->GetValue());
        cfg->Write(_T("/folding/indicator"),
                   XRCCTRL(*this, "lstIndicators", wxChoice)->GetSelection());
        cfg->Write(_T("/folding/limit"), XRCCTRL(*this, "chkFoldLimit", wxCheckBox)->GetValue());
        cfg->Write(_T("/folding/limit_level"),
                   XRCCTRL(*this, "spnFoldLimitLevel", wxSpinCtrl)->GetValue());

        // eol
        cfg->Write(_T("/show_eol"), XRCCTRL(*this, "chkShowEOL", wxCheckBox)->GetValue());
        cfg->Write(_T("/eol/strip_trailing_spaces"),
                   XRCCTRL(*this, "chkStripTrailings", wxCheckBox)->GetValue());
        cfg->Write(_T("/eol/ensure_final_line_end"),
                   XRCCTRL(*this, "chkEnsureFinalEOL", wxCheckBox)->GetValue());
        cfg->Write(_T("/eol/ensure_consistent_line_ends"),
                   XRCCTRL(*this, "chkEnsureConsistentEOL", wxCheckBox)->GetValue());
        ...
        // margin
        cfg->Write(_T("/margin/width_chars"),
                   XRCCTRL(*this, "spnMarginWidth", wxSpinCtrl)->GetValue());
        cfg->Write(_T("/margin/dynamic_width"),
                   XRCCTRL(*this, "chkDynamicWidth", wxCheckBox)->GetValue());
        cfg->Write(_T("/margin_1_sensitive"),
                   (bool)XRCCTRL(*this, "chkAddBPByLeftClick", wxCheckBox)->GetValue());
        cfg->Write(_T("/margin_1_image_bp"),
                   (bool)XRCCTRL(*this, "chkImageBP", wxCheckBox)->GetValue());

        // selections
        cfg->Write(_T("/selection/use_vspace"),
                   (bool)XRCCTRL(*this, "chkEnableVirtualSpace", wxCheckBox)->GetValue());
        cfg->Write(_T("/selection/use_rect_vspace"),
                   (bool)XRCCTRL(*this, "chkEnableRectVirtualSpace", wxCheckBox)->GetValue());
        cfg->Write(_T("/selection/multi_select"),
                   (bool)XRCCTRL(*this, "chkEnableMultipleSelections", wxCheckBox)->GetValue());
        cfg->Write(
            _T("/selection/multi_typing"),
            (bool)XRCCTRL(*this, "chkEnableAdditionalSelectionTyping", wxCheckBox)->GetValue());

        // scrollbar
        cfg->Write(_T("/margin/scroll_width_tracking"),
                   XRCCTRL(*this, "chkScrollWidthTracking", wxCheckBox)->GetValue());
        ...
        if (enableChangebar != m_EnableChangebar)
        {
            ...
            for (int idx = 0; idx < em->GetEditorsCount(); ++idx)
            {
                ...
                if (ed)
                {
                    // if we enable changeCollection, we also have to empty Undo-Buffer, to avoid
                    // inconsistences, if we disable it, there is no need to do that
                    enableChangebar ? ed->ClearHistory() : ed->SetChangeCollection(false);
                }
            }
        }
        ...
        cfg->Write(_T("/colour_sets/active_lang"),
                   XRCCTRL(*this, "cmbLangs", wxChoice)->GetStringSelection());
        ...
        cfg->Write(_T("/default_encoding/use_option"),
                   XRCCTRL(*this, "rbEncodingUseOption", wxRadioBox)->GetSelection());
        cfg->Write(_T("/default_encoding/find_latin2"),
                   XRCCTRL(*this, "chkEncodingFindLatin2", wxCheckBox)->GetValue());
        cfg->Write(_T("/default_encoding/use_system"),
                   XRCCTRL(*this, "chkEncodingUseSystem", wxCheckBox)->GetValue());

        // code completion
        cfg = Manager::Get()->GetConfigManager(_T("ccmanager"));
        cfg->Write(_T("/code_completion"),
                   XRCCTRL(*this, "chkCodeCompletion", wxCheckBox)->GetValue());
        cfg->Write(_T("/case_sensitive"),
                   XRCCTRL(*this, "chkCCCaseSensitive", wxCheckBox)->GetValue());
        cfg->Write(_T("/auto_select_single"),
                   XRCCTRL(*this, "chkAutoselectSingle", wxCheckBox)->GetValue());
        cfg->Write(_T("/auto_launch_count"),
                   XRCCTRL(*this, "spnAutolaunchCount", wxSpinCtrl)->GetValue());
        cfg->Write(_T("/documentation_popup"),
                   XRCCTRL(*this, "chkDocumentationPopup", wxCheckBox)->GetValue());
