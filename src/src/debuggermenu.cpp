...
wxMenu *GetMenuById(long menuId, bool recreate = false)
{
    ...
    if (recreate)
    {
        ...
        if (pos != wxNOT_FOUND)
        {
            ...
            wxMenuItem *newItem = new wxMenuItem(menu, item->GetId(), item->GetItemLabelText(),
                                                 item->GetHelp(), item->GetKind(), newSubMenu);
...
BEGIN_EVENT_TABLE(DebuggerMenuHandler, wxEvtHandler)
    ...
    EVT_MENU(idMenuDebugActiveTargetsDefault,
             DebuggerMenuHandler::OnActiveDebuggerTargetsDefaultClick)
END_EVENT_TABLE()

DebuggerMenuHandler::DebuggerMenuHandler() : m_activeDebugger(nullptr), m_disableContinue(false) {}
...
struct CommonItem : cbDebuggerWindowMenuItem
{
    ...
    CommonItem(cbDebuggerFeature::Flags enableFeature, cbDebuggerPlugin::DebugWindows requestUpdate,
               GetWindowFunc func)
      : m_enableFeature(enableFeature), m_requestUpdate(requestUpdate), m_getWindowFunc(func)
    {
    }
...
void DebuggerMenuHandler::RegisterDefaultWindowItems()
{
    ...
    {
        Watches()
          : CommonItem<cbWatchesDlg>(cbDebuggerFeature::Watches, cbDebuggerPlugin::Watches,
                                     &DebuggerManager::GetWatchesDialog)
        {
        }
        virtual bool IsEnabled()
        {
...
bool DebuggerMenuHandler::RegisterWindowMenu(const wxString &name, const wxString &help,
                                             cbDebuggerWindowMenuItem *item)
{
    for (WindowMenuItemsMap::iterator it = m_windowMenuItems.begin(); it != m_windowMenuItems.end();
         ++it)
    {
        if (it->second.name == name)
        {
            wxString msg = wxString::Format(_("Duplicate debugger window name '%s'. Igrnoring it."),
                                            name.wx_str());
    ...
    Connect(id, wxEVT_UPDATE_UI,
            wxObjectEventFunction(&DebuggerMenuHandler::OnWindowMenuItemUpdateUI));
    Connect(id, wxEVT_COMMAND_MENU_SELECTED,
            wxCommandEventHandler(DebuggerMenuHandler::OnWindowMenuItemClicked));
    return true;
}
...
void DebuggerMenuHandler::RebuildMenus()
{
    ...
    for (DebuggerManager::RegisteredPlugins::const_iterator it = allDebugger.begin();
         it != allDebugger.end(); ++it)
    {
        const DebuggerManager::ConfigurationVector &configs = it->second.GetConfigurations();
        for (DebuggerManager::ConfigurationVector::const_iterator itConf = configs.begin();
             itConf != configs.end(); ++itConf)
        {
            ...
            Connect(id, -1, wxEVT_COMMAND_MENU_SELECTED,
                    (wxObjectEventFunction)(wxEventFunction)(
                        wxCommandEventFunction)&DebuggerMenuHandler::OnActiveDebuggerClick);
        }
    }
...
void DebuggerMenuHandler::BuildContextMenu(wxMenu &menu, const wxString &word_at_caret,
                                           bool is_running)
{
    ...
    if (is_running)
    {
        ...
        if (!word_at_caret.empty())
        {
            if (plugin->SupportsFeature(cbDebuggerFeature::Watches))
                menu.Insert(item++, idMenuDebuggerAddWatch,
                            wxString::Format(_("Watch '%s'"), word_at_caret.c_str()));
            // data breakpoint
            if (plugin->SupportsFeature(cbDebuggerFeature::Breakpoints))
            {
                menu.Insert(
                    item++, idMenuAddDataBreakpoint,
...
void DebuggerMenuHandler::OnRunToCursor(cb_unused wxCommandEvent &event)
{
    ...
    if (manager->GetIsRunning() == nullptr || manager->GetIsRunning() == m_activeDebugger)
    {
        ...
        HideValueTooltip();
        if (!m_activeDebugger->RunToCursor(ed->GetFilename(),
                                           ed->GetControl()->GetCurrentLine() + 1, line_text))
            manager->SetIsRunning(nullptr);
    }
}
...
{
    cbAssert(m_activeDebugger);
    wxString cmd = cbGetTextFromUser(_("Enter command for Debugger:"),
                                     _("Send command to Debugger:"), m_lastCommand);
    if (cmd.IsEmpty())
        return;
...
DebuggerToolbarHandler::DebuggerToolbarHandler(DebuggerMenuHandler *menuHandler)
  : m_Toolbar(nullptr), m_menuHandler(menuHandler)
{
}
