 ...
 *
 * example:
 * EditorHooks::HookFunctorBase* myhook = new EditorHooks::HookFunctor<MyClass>(this,
 * &MyClass::OnHookCalled); int id = EditorHooks::RegisterHook(myhook);
 * ...
...
public:
    ...
    virtual const char *GetTypeName() const { return typeid(m_pFunc).name(); }
#endif // EDITOR_HOOK_PERFORMANCE_MEASURE
...
public:
     ...
     * @param event  The wxScintilla event fired to react accordingly (see cbEditor::CreateEditor,
     * namely scintilla_events)
     */
    virtual void Call(cbEditor *editor, wxScintillaEvent &event) const;

#ifdef EDITOR_HOOK_PERFORMANCE_MEASURE
    virtual const char *GetTypeName() const { return typeid(m_plugin).name(); }
#endif // EDITOR_HOOK_PERFORMANCE_MEASURE
