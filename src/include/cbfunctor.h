...
class IEventFunctorBase : public IFunctorBase
{
public:
...
class cbEventFunctor : public IEventFunctorBase<EventType>
{
private:
...
public:
    ...
    cbEventFunctor(const cbEventFunctor<ClassType, EventType> &rhs)
      : m_pThis(rhs.m_pThis), m_pMember(rhs.m_pMember)
    {
    }
    ...
    virtual void Call(EventType &event)
    {
        if (m_pThis)
