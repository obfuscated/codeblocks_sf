 ...
 *  ConfigManager can save arbitrary objects and even sets/maps of objects, provided they implement
 * Serializable.
 *
 *  Usage:
 *  ------
...
class DLLIMPORT ConfigManager
{
    ...
    wxString InvalidNameMessage(const wxString &what, const wxString &sub,
...
public:
     ...
     *
     * You should always use this function if you are looking for "some arbitrary file that belongs
     * to Code::Blocks", as it works across platforms without any additional effort from your side,
     * and it has some builtin redundancy.
     * @par
     * So, code that looked like this in the old days:
     * @code
     * wxString some_file = ConfigManager::GetScriptsFolder() + wxFILE_SEP_PATH +
     * _T("startup.script");
     * @endcode
     * should be converted to this:
     * @code
     * wxString some_file = ConfigManager::LocateDataFile(_T("startup.script"), sdScriptsUser |
     * sdScriptsGlobal);
     * @endcode
     * This would try to locate the file named "startup.script" in the global and also in the user's
     * scripts folders.
     ...
     * NEVER hard-code a path like "C:\CodeBlocks\share\data". Always use one of the following
     * functions to compose a path.
     */
    ...
    } // use instead of cfg->Read("data_path");
    static inline wxString ReadAppPath()
    {
        return GetExecutableFolder();
    } // use instead of cfg->Read("app_path");
    ...
    /* -----------------------------------------------------------------------------------------------------
     *  Clear all nodes from your namespace or delete the namespace (removing it from the config
     * file). WARNING: After Delete() returns, the pointer to your instance is invalid. Before you
     * can call ANY member function of this class, you have to call
     * Manager::Get()->GetConfigManager() for a valid reference again. Note that Delete() is
     * inherently thread-unsafe. You delete an entire namespace of data as well as the object
    ...
    /* -----------------------------------------------------------------------------------------------------
     *  Set and unset keys, or test for existence. Note that these functions cannot be used to
     * remove paths or test existence of paths (it may be used to implicitly create paths, though).
     */
...
public:
    ...
    ConfigManagerWrapper(wxString namespace_, wxString basepath)
      : m_namespace(namespace_), m_basepath(basepath)
    {
...
/// Read the list of batch build plugins and return them. The list won't be empty; at least the
/// compiler plugin will be added to the to it. The list contain the library names of the plugins
/// and differs on every OS.
