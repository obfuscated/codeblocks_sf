...
public:
     ...
     * @param ed input editor, uses active editor if one is not passed; has (minimal) cache
     * optimization
     */
    ...
    /** Let CCManager know that new results are available from
     * cbCodeCompletionPlugin::GetDocumentation(). */
    void NotifyDocumentation();
    /** Let CCManager know that a change (e.g. active lexer is switched) may invalidate cached
     * active ccPlugin. */
    void NotifyPluginStatus();

    /** If for some reason you *need* to use wxScintilla::AutoCompShow(), call this instead so
     * CCManager does not step on you. */
...
private:
    ...
    void DoShowTips(const wxStringVec &tips, cbStyledTextCtrl *stc, int pos, int argsPos,
                    int hlStart, int hlEnd);

    /** Code completion as just insert some text in the editor, ask the smart indent plugin to
     * adjust the indent. */
    ...
    int m_EditorHookID;
    int m_AutocompPosition; //!< Location of caret when autocomplete timer starts, if caret is still
                            //!< there, launch autocomplete.
