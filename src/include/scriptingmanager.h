...
public:
     ...
     * @param exception A pointer to the exception object containing the error. Can be NULL
     * (default).
     ...
     * @return True on success, false on failure.
     */
    bool RegisterScriptMenu(const wxString &menuPath, const wxString &scriptOrFunc,
                            bool isFunction);
    ...
    ScriptingManager &
        operator=(cb_unused const ScriptingManager &rhs) // prevent assignment operator
    {
...
typedef char
    SQChar; // HACK, MUST match with the type as defined for the dedicated platform in squirrel.h
