...
public:
     ...
     * @param pExtensions A pointer of type TiXmlElement. This will be added as
     * the \<Extensions\> element.
     * @param onlyTarget The target name. If empty, it's like saving the project under a different
     * name (i.e. all targets are exported to the new project). */
    ...
    /** @return True if the file was modified while loading, false if not. This is usually true if
     * FileUpgraded() returned true. */
...
private:
    ...
    TiXmlElement *AddElement(TiXmlElement *parent, const char *name, const char *attr = nullptr,
                             const wxString &attribute = wxEmptyString);
    TiXmlElement *AddElement(TiXmlElement *parent, const char *name, const char *attr,
                             int attribute);
    void AddArrayOfElements(TiXmlElement *parent, const char *name, const char *attr,
    ...
    bool m_Upgraded;
    bool m_OpenDirty; // set this to true if the project is loaded but modified (like the case when
                      // setting another compiler, if invalid)
    bool m_IsPre_1_2;
