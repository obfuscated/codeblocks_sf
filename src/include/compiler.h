...
struct RegExStruct
{
    ...
    RegExStruct(const wxString &_desc, CompilerLineType _lt, const wxString &_regex, int _msg,
                int _filename = 0, int _line = 0, int _msg2 = 0, int _msg3 = 0)
    ...
    bool operator!=(const RegExStruct &other) { return !(*this == other); }
    bool operator==(const RegExStruct &other)
    {
        return (desc == other.desc && lt == other.lt && regex == other.regex
                && msg[0] == other.msg[0] && msg[1] == other.msg[1] && msg[2] == other.msg[2]
...
{
    ctCompileObjectCmd =
        0, ///< Compile object command, e.g. "$compiler $options $includes -c $file -o $object"
    ctGenDependenciesCmd, ///< Generate dependencies command
    ctCompileResourceCmd, ///< Compile Win32 resources command, e.g. "$rescomp -i $file -J rc -o
                          ///< $resource_output -O coff $includes"
    ctLinkExeCmd, ///< Link executable command, e.g. "$linker $libdirs -o $exe_output $link_objects
                  ///< $libs -mwindows"
    ctLinkConsoleExeCmd, ///< Link console executable command, e.g. "$linker $libdirs -o $exe_output
                         ///< $link_objects $libs"
    ctLinkDynamicCmd, ///< Link dynamic (dll) lib command, e.g. "$linker -shared
                      ///< -Wl,--output-def=$def_output -Wl,--out-implib=$static_output -Wl,--dll
                      ///< $libdirs $link_objects $libs -o $dynamic_output"
...
struct DLLIMPORT CompilerSwitches
{
    ...
    bool forceFwdSlashes; // force use forward slashes in file/path names (used by
                          // CompilerCommandGenerator)
    bool forceLinkerUseQuotes; // use quotes for filenames in linker command line (needed or not)?
    bool forceCompilerUseQuotes; // use quotes for filenames in compiler command line (needed or
                                 // not)?
    ...
    bool UseFullSourcePaths; // This is mainly a workaround for the GDB debugger, apparently I
                             // doesn't deal well with relative paths, therefore for GCC it is
                             // better to specify the source full to the compiler in a full path
                             // notation, for all other compilers it is suggested to keep this
                             // switch at false
    bool Use83Paths; // This is mainly a workaround for the resource compiler under Windows,
                     // apparently it doesn't deal well with spaces in the (include) path even if
                     // the path is quoted, therefore use 8.3 notation without spaces on Windows.
                     // However, this will apply to all include path's as other tools might have the
                     // same issue and it won't hurt to apply it to all include directories, if
                     // enabled.
    wxChar includeDirSeparator; // space
    wxChar libDirSeparator; // space
    wxChar objectSeparator; // space
    int statusSuccess; // 0    - treat exit-codes >= 0 and <= statusSuccess as success (do not set
                       // negative!)

    CompilerSwitches(); // constructor initializing the members, specific compilers should overrule
                        // if needed
};
...
struct CompilerTool
{
    ...
    // (;) as separator
    CompilerTool(const wxString &command_in = wxEmptyString,
                 const wxString &extensions_in = wxEmptyString,
                 const wxString &generatedFiles_in = wxEmptyString)
      : command(command_in),
        extensions(GetArrayFromString(extensions_in)),
        generatedFiles(GetArrayFromString(generatedFiles_in))
    {
    }
    ...
    {
        return command == rhs.command && extensions == rhs.extensions
               && generatedFiles == rhs.generatedFiles;
    }
    bool operator!=(const CompilerTool &rhs) const { return !(*this == rhs); }

    wxString command; ///< command to execute
    wxArrayString
        extensions; ///< file extensions for which the command will be invoked (no leading dot)
    wxArrayString generatedFiles; ///< the native language files this command generates that should
                                  ///< be further compiled
};
...
public:
    ...
    /** @brief Returns warning/error line number (as a string). Use it after a call to
     * CheckForWarningsAndErrors() */
    virtual wxString GetLastErrorLine() { return m_ErrorLine; }
    /** @brief Returns warning/error actual string. Use it after a call to
     * CheckForWarningsAndErrors() */
     ...
     */
    virtual const wxString &GetCommand(CommandType ct,
                                       const wxString &fileExtension = wxEmptyString) const;
    /** @brief Get a compiler tool based on CommandType */
    virtual const CompilerTool *
        GetCompilerTool(CommandType ct, const wxString &fileExtension = wxEmptyString) const;
...
protected:
    ...
    wxString
        m_SortOptions[2]; // m_SortOptions[0] == C-only flags; m_SortOptions[1] == C++-only flags
...
private:
    wxString m_ID;
    wxString m_ParentID; // -1 for builtin compilers, the builtin compiler's ID to derive from for
                         // user compilers...
    ...
    // "mirror" default settings for comparing when saving (to save only those that differ from
    // defaults)
    struct MirrorSettings
    {
        ...
        // below are the settings that the user is asked to revert to defaults (if defaults have
        // changed)
