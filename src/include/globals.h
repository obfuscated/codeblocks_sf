...
enum ProjectTreeVisualState
{
    ...
    ptvsUseFolders = 0x02, //!< If true, create folders as needed. If false, the list is flat (not
                           //!< compatible with "hie folder name")
    ptvsHideFolderName = 0x04, //!< If true, the folder name will be hidden and only the file name
                               //!< will be shown (not compatible with "use folders")
...
extern DLLIMPORT wxFontEncoding DetectEncodingAndConvert(
    const char *strIn, wxString &strOut, wxFontEncoding possibleEncoding = wxFONTENCODING_SYSTEM);
...
extern DLLIMPORT wxString GetStringFromArray(const wxArrayString &array,
                                             const wxString &separator = DEFAULT_ARRAY_SEP,
                                             bool SeparatorAtEnd = true);
extern DLLIMPORT wxArrayString GetArrayFromString(const wxString &text,
                                                  const wxString &separator = DEFAULT_ARRAY_SEP,
...
extern DLLIMPORT wxString MakeUniqueString(const wxString &text,
                                           const wxString &separator = DEFAULT_ARRAY_SEP,
                                           bool caseSens = true);
...
extern DLLIMPORT void SaveTreeState(wxTreeCtrl *tree, const wxTreeItemId &parent,
                                    wxArrayString &nodePaths, wxArrayString &selectedItemPaths);
extern DLLIMPORT void RestoreTreeState(wxTreeCtrl *tree, const wxTreeItemId &parent,
                                       wxArrayString &nodePaths, wxArrayString &selectedItemPaths);
...
extern DLLIMPORT void PlaceWindow(wxTopLevelWindow *w, cbPlaceDialogMode mode = pdlBest,
                                  bool enforce = false);
 ...
 */
extern DLLIMPORT int cbMessageBox(const wxString &message, const wxString &caption = wxEmptyString,
                                  int style = wxOK, wxWindow *parent = NULL, int x = -1,
                                  int y = -1);
 ...
 * Use this instead of wxMessageBox(), as this uses PlaceWindow() to show it in the correct monitor.
 */
extern DLLIMPORT wxArrayInt cbGetMultiChoiceDialog(
    const wxString &message, const wxString &caption, const wxArrayString &choices,
    wxWindow *parent = nullptr, const wxSize &size = wxSize(300, 300),
...
extern DLLIMPORT wxString cbGetTextFromUser(const wxString &message,
                                            ...
                                            wxWindow *parent = NULL, int x = wxDefaultCoord,
