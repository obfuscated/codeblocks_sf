...
enum TargetType
{
    ...
    ttConsoleOnly = 1, /**< Target produces a console executable (without GUI) (distinction between
                          ttExecutable and ttConsoleOnly happens only under Win32) */
...
public:
    ...
    virtual void
        GetTargetFilenameGenerationPolicy(TargetFilenameGenerationPolicy &prefixOut,
    ...
    virtual void SetImportLibraryFilename(
        const wxString &filename); ///< Set the target's import library filename
    virtual void SetDefinitionFileFilename(
        const wxString &filename); ///< Set the target's definition file filename
    virtual void SetWorkingDir(const wxString &dirname); ///< Set the target's working dir on
                                                         ///< execution (valid only for executable
                                                         ///< targets)
    virtual void SetObjectOutput(const wxString &dirname); ///< Set the target's objects output dir
    virtual void
        SetDepsOutput(const wxString &dirname); ///< Set the target's dependencies output dir
    virtual OptionsRelation GetOptionRelation(
        OptionsRelationType type) const; ///< Read the target's options relation for \c type
    virtual void SetOptionRelation(
        OptionsRelationType type,
        OptionsRelation rel); ///< Set the target's options relation for \c type to \c rel
    virtual wxString GetWorkingDir(); ///< Read the target's working dir for execution (valid only
    ...
    virtual wxString GetExecutableFilename()
        const; ///< Read the target's executable filename (produced if target type is ttExecutable)
    virtual wxString GetDynamicLibFilename(); ///< Read the target's dynamic library filename
                                              ///< (produced if target type is ttDynamicLib)
    virtual wxString GetDynamicLibImportFilename(); ///< Read the target's dynamic library import
                                                    ///< filename (produced if target type is
                                                    ///< ttDynamicLib)
    virtual wxString GetDynamicLibDefFilename(); ///< Read the target's dynamic library definition
                                                 ///< file filename (produced if target type is
                                                 ///< ttDynamicLib)
    virtual wxString GetStaticLibFilename(); ///< Read the target's static library filename
                                             ///< (produced if target type is ttStaticLib)
    virtual wxString GetNativeFilename(); ///< Read the target's native filename (produced if target
                                          ///< type is ttNative)
    virtual wxString GetBasePath() const; ///< Read the target's base path, e.g. if GetFilename()
                                          ///< returns "/usr/local/bin/xxx", base path will return
                                          ///< "/usr/local/bin"
    ...
    virtual const wxString &
        GetExecutionParameters() const; ///< Read the target's execution parameters
    virtual void SetExecutionParameters(
        const wxString &params); ///< Set the target's execution parameters to \c params
    virtual const wxString &GetHostApplication() const; ///< Read the target's host application
    virtual void
        SetHostApplication(const wxString &app); ///< Set the target's host application to \c app
    virtual bool GetRunHostApplicationInTerminal()
        const; ///< Get the flag if the host app should be run in terminal
    virtual void SetRunHostApplicationInTerminal(
        bool in_terminal); ///! Set the flag if the host app should be run in terminal
    ...
    {
        return m_MakeCommands[cmd];
    } ///< Get the "make" command used for @c cmd
    virtual void
        SetMakeCommandFor(MakeCommand cmd,
                          const wxString &make); ///< Set the "make" command used for @c cmd
    virtual bool MakeCommandsModified() const
    {
        return m_MakeCommandsModified;
