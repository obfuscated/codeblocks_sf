...
static const int cgContribPlugin =
    0x08; ///< One of the contrib plugins (or any third-party plugin for that matter).
...
public:
     ...
     * @return A pointer to the plugin's cbConfigurationPanel. It is deleted by the caller.
     */
    virtual cbConfigurationPanel *GetProjectConfigurationPanel(cb_optional wxWindow *parent,
     ...
     */
    virtual void BuildModuleMenu(cb_optional const ModuleType type, cb_optional wxMenu *menu,
...
public:
    ...
    virtual void BuildModuleMenu(const ModuleType type, wxMenu *menu,
...
public:
    ...
    /** @brief Execute the next instruction and return control to the debugger, if the instruction
     * is a function call step into it. */
    virtual void StepIntoInstruction() = 0;

    /** @brief Execute the next instruction, stepping into function calls if needed, and return
     * control to the debugger. */
    virtual void Step() = 0;

    /** @brief Execute the next instruction, stepping out of function calls if needed, and return
     * control to the debugger. */
    ...
    /**
     * @param[out] disabledMenus A combination of WatchesDisabledMenuItems, which controls which of
     * the default menu items are disabled
     */
...
protected:
    ...
    /** @brief Called when the compilation has finished. The compilation is started when
     * EnsureBuildUpToDate is called.
     * @param compilerFailed the compilation failed for some reason.
     * @param startType it is the same value given to the Debug method, when the debugger session
     * was started.
     * @return True if debug session is start, false if there are any errors or the users canceled
     * the session.
     */
...
protected:
    ...
    bool GetDebuggee(wxString &pathToDebuggee, wxString &workingDirectory,
...
private:
    ...
    void BuildModuleMenu(cb_unused const ModuleType type, cb_unused wxMenu *menu,
...
public:
    ...
    struct CCToken
    {
         ...
         *
         * @param _id Internal identifier for a CC plugin to reference the token in its data
         * structure.
         * @param dispNm The string CCManager will use to display this token.
         * @param categ The category corresponding to the index of the registered image (during
         * autocomplete). Negative values are reserved for CCManager.
         */
         ...
         * @param nm Minimal name of the token that CCManager may choose to display in restricted
         * circumstances.
         * @param _weight Lower numbers are placed earlier in listing, 5 is default; try to keep
         * 0-10.
         * @param categ The category corresponding to the index of the registered image (during
         * autocomplete). Negative values are reserved for CCManager.
         */
        ...
        int id; //!< CCManager will pass this back unmodified. Use it as an internal identifier for
                //!< the token.
        int category; //!< The category corresponding to the index of the registered image (during
                      //!< autocomplete).
        int weight; //!< Lower numbers are placed earlier in listing, 5 is default; try to keep
                    //!< 0-10.
        wxString displayName; //!< Verbose string representing the token.
        wxString name; //!< Minimal name of the token that CCManager may choose to display in
    ...
    struct CCCallTip
    {
         ...
         */
        CCCallTip(const wxString &tp) : hlStart(-1), hlEnd(-1), tip(tp) {}
     ...
     *               character such as '<tt>&gt;</tt>' (for '<tt>-&gt;</tt>'). It is the plugin's
     * job to filter out incorrect calls of this.
     * @param ed The context of this codecompletion call.
     * @param[in,out] tknStart The assumed beginning of the token to be autocompleted. Change this
     * variable if the plugin calculates a different starting location.
     * @param[in,out] tknEnd The current position/end of the known part of the token to be
     * completed. The plugin is allowed to change this (but it is not recommended).
     * @return Completable tokens, or empty vector to cancel autocompletion.
     */
    virtual std::vector<CCToken> GetAutocompList(bool isAuto, cbEditor *ed, int &tknStart,
                                                 int &tknEnd) = 0;
     ...
     * @param[out] argsPos The location in the editor of the beginning of the argument list. @em
     * Required.
     * @return Each entry in this vector is guaranteed either a new line or a separate page in the
     * calltip. CCManager will decide if lines should be further split (for formatting to fit the
     * monitor).
     */
     ...
     * @param[out] allowCallTip Allow CCManager to consider displaying a calltip if the results from
     * this function are unsuitable/empty. True by default.
     ...
     *
     * Handle a link command by, for example, showing the definition of a member function, or
     * opening an editor to the location of the declaration.
     *
     ...
     * @c ed->GetControl()->AutoCompCancel() for different @c wxEVT_SCI_AUTOCOMP_SELECTION
     * behaviour.
     *
     ...
     * selection (which should never happen). The default implementation creates a CCToken and
     * passes it to <tt>DoAutocomplete(const CCToken&, cbEditor*)</tt><br> Override for different
     * behaviour.
     *
...
public:
    ...
    /** @param index the wizard index.
     * @return the template's category (GUI, Console, etc; free-form text). Try to adhere to
     ...
     * @return a pointer to the generated cbProject or ProjectBuildTarget. NULL for everything else
     * (failure too). You should dynamic-cast this to the correct type based on GetOutputType() 's
     * value. */
    virtual CompileTargetBase *Launch(int index,
                                      wxString *createdFilename = nullptr) = 0; // do your work ;)
private:
...
protected:
    ...
    wxString GetLastNonCommentWord(cbEditor *ed, int position = -1,
                                   unsigned int NumberOfWords = 1) const;
    /** (reverse) search for the last characters, which are not whitespace and not comment **/
    wxString GetLastNonWhitespaceChars(cbEditor *ed, int position = -1,
                                       unsigned int NumberOfChars = 1) const;
    ...
    wxChar GetNextNonWhitespaceCharOnLine(cbStyledTextCtrl *stc, int position = -1,
                                          int *pos = nullptr) const;

    int FindBlockStart(cbStyledTextCtrl *stc, int position, wxChar blockStart, wxChar blockEnd,
                       bool skipNested = true) const;
    int FindBlockStart(cbStyledTextCtrl *stc, int position, wxString blockStart, wxString blockEnd,
    ...
    wxChar GetNextNonWhitespaceCharOfLine(cbStyledTextCtrl *stc, int position = -1,
                                          int *pos = nullptr) const;
...
public:
    ...
    static cbPlugin *CreatePlugin() { return new T; }

    static void FreePlugin(cbPlugin *plugin) { delete plugin; }
