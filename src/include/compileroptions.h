...
struct CompOption
{
    ...
    wxString checkAgainst; // "-O -O1 -O2 -O3 -Os" (will check for these options and if any of them
                           // is found, will display the following message)
    wxString checkMessage; // "You have optimizations enabled. This is Not A Good Thing(tm) when
                           // producing debugging symbols..."
    wxString supersedes; // "-O -O1 -O2" (will check for these options and disable any of them that
                         // are found)
...
public:
    ...
    void AddOption(const wxString &name, const wxString &option,
                   ...
                   const wxString &supersedes = wxEmptyString, bool exclusive = false,
                   int index = -1);
