...
public:
    ...
    template<typename type, bool requires_filename = false>
    struct Instantiator : public InstantiatorBase
    {
...
public:
     ...
     * SetLog() transfers ownership of the Logger object to the LogManager. Loggers must be heap
     * allocated. On error, SetLog() returns invalid_log
     */
     ...
     *   - Log(), LogWarning(), and LogError() output "info", "warning", or "error" messages to a
     * log. Log() is almost certainly the function that you want to use, if you don't know what to
     * pick. By default, logging is directed to the application log.
     *   - DebugLog() and DebugLogError() direct their output into the debug log.
     *   - LogToStdOut() outputs a message on stdout. Normally you will not want to use this
     * function, it exists solely for some special cases.
     *   - Panic() signals a condition that does not allow proper continuation of the application or
     * significant parts of it, but it is slightly less harsh than simply bailing out with an
     * unhandled exception. Currently, Panic() is simply a wrapper around wxSafeShowMessage(),
     * but it might do something else, too. When signalling panic, you will usually want to shut
     * down the application as soon as appropriate. Plugins should call Panic() with the
     * plugin's name as the component argument.
     */
     ...
     * ------------------------------------------------------------------------------------------------------
     * These functions allow to obtain a list of names for all generic Loggers that are presently
     * available and to create a new Logger by name without knowing the type at compile time.
     *
     *   logptr = LogManager::Get()->New(_T("stdout"));  // does exactly the same as
     *   logptr = new StdoutLogger();
     *
     * You normally do not need to worry about creating Loggers. Only ever consider using these
     * functions if you really have to (want to) change the global behaviour of Code::Blocks for
     * some reason.
     */
    ...
    /*
     * Add a logger to the registry of "available Logger types". Unless you are adding a general
     * Logger which should be accessible by name, you will not need this function. Having said that,
     * you probably NEVER need this function.
     */
     ...
     *  If you use it, and your name is not "main.cpp", then you better come up with a good excuse,
     * if someone asks.
     * ------------------------------------------------------------------------------------------------------
     */
    void NotifyUpdate();
