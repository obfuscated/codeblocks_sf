...
public:
    ...
    FileTreeData(cbProject *project, FileTreeDataKind kind = ftdkUndefined)
      : m_Index(-1), m_Project(project), m_file(nullptr), m_kind(kind)
    {
    }
...
{
    pchSourceDir =
        0, /// In a dir (named by the PCH) on the same level as the source header (default).
...
public:
     ...
     * @param index The index of the file. Must be greater or equal than zero and less than
     * GetFilesCount().
     ...
     * @param isUnixFilename True if @c filename is already normalized with UnixFilename(), false if
     * not.
     * @return A pointer to the file or NULL if not found.
     */
    ProjectFile *GetFileByFilename(const wxString &filename, bool isRelative = true,
     ...
     * @param custom If true, use a custom Makefile for compilation. If false, use direct C::B build
     * mode.
     */
    ...
    /** Allow the specification of specific execution directory if the project use a custom
     * Makefile. Defaults to the projects base path, if no custom makefile is used.
     ...
     * @param index The build target index. Must be greater or equal to zero and less than
     ...
     * @param newName The name for the new build target. If empty, it will be named like "Copy of
     * <base_target_name>".
     ...
     * @return The new build target if @c index was valid, NULL if not (or something went wrong).
     */
    ProjectBuildTarget *DuplicateBuildTarget(const wxString &targetName,
     ...
     * @param filename The file's filename. This *must* be a filename relative to the project's
     * path.
     ...
     * @param weight A value between zero and 100 (defaults to 50). Smaller weight, makes the file
     * compile earlier than those with larger weight.
     * @return The newly added file or NULL if something went wrong.
     */
    ProjectFile *AddFile(const wxString &targetName, const wxString &filename, bool compile = true,
                         bool link = true, unsigned short int weight = 50);
     ...
     * @return The newly added file or NULL if something went wrong.
     */
    ProjectFile *AddFile(int targetIndex, const wxString &filename, bool compile = true,
                         bool link = true, unsigned short int weight = 50);
    ...
    struct Glob
    {
        ...
        Glob(const wxString &path, const wxString &wildCard, bool recursive)
          : m_Path(path), m_WildCard(wildCard), m_Recursive(recursive)
        {
        }
    };

    /** Set the globs to the project, this are directory paths do retrieve files from to be added to
     * the project, the path can be searched recursively
     ...
     * Defaults to none (-1).
     * @param evenIfOne If true, the dialog is still shown even if the project contains only one
     * target. The default behaviour is to not show the dialog if the project has only one target.
    ...
    /** Returns the last modification time for the file. Used to detect modifications outside the
     * Program. */
    wxDateTime GetLastModificationTime() const { return m_LastModified; }

    /** Sets the last modification time for the project to 'now'. Used to detect modifications
     * outside the Program. */
    void Touch();
...
private:
    ...
    void NotifyPlugins(wxEventType type, const wxString &targetName = wxEmptyString,
