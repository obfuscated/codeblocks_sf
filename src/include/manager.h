...
public:
     ...
     * During application startup as well as during runtime, these functions will always return a
     * valid pointer. During application shutdown, these functions will continue to return a valid
     * pointer until the requested manager shuts down. From that point, the below functions will
     * return null. If there is any chance that your code might execute during application shutdown,
     * you MUST check for a null pointer. The one notable exception to this rule is ConfigManager,
     ...
     *
     * For plugin developers, this means that most managers (except for the ones you probably don't
     * use anyway) will be available throughout the entire lifetime of your plugins.
     */
    ...
    void RegisterEventSink(wxEventType eventType,
