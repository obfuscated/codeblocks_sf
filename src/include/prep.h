...
/*  ---------------------------------------------------------------------------------------------------------
    ...
    Example:
        if(!wxMinimumVersion<2.8>::eval && (foo_version < Version<1.2>::eval))
            ErrorMessage("This feature is only supported under wxWidgets 2.8, or with Foo
   Component 1.2 or higher.");
*/
...
struct Version
{
    enum
    {
        eval = (major << 25) + (minor << 15) + revision
    };
};
...
template<int major, int minor, int rel = 0>
struct wxMinimumVersion
{
    enum
    {
        eval = ((unsigned int)Version<wxMAJOR_VERSION, wxMINOR_VERSION, wxRELEASE_NUMBER>::eval
...
/*  ---------------------------------------------------------------------------------------------------------
    ...
    Example:
        typedef TernaryCondTypedef<wxMinimumVersion<2,5>::eval, wxTreeItemIdValue, long int>::eval
   tree_cookie_t;
*/
template<bool cond, class true_t, class false_t>
...
*/
template<typename T>
unsigned int array_size(const T &array)
{
    enum
    {
...
/*  ---------------------------------------------------------------------------------------------------------
    ...
    In class destructors, please continue using the normal C++ delete operator (it is unnecessary
   overhead to set a pointer to zero in the destructor, as it can never be used again). In _all_
   other cases, use Delete(), which prevents accidential double-deletes.
*/
template<typename T>
...
/*  ---------------------------------------------------------------------------------------------------------
    ...
    platform::darwin,  platform::solaris, platform::Unix
        Boolean value that evaluates to true if the target platform is the same as the variable's
   name, false otherwise. Using the platform booleans is equivalent to using platform::id, but
   results in nicer code.

    platform::unicode
        Boolean value that evaluates to true if this application was built in Unicode mode. Of
   course this does not tell anything about the host system's actual capabilities.

    platform::gtk
    platform::carbon
    platform::cocoa
        Boolean values showing the presence of very specific toolkits. Use only for workarounds to
   specific problems with these.
...
//
// IMPORTANT: Do not lie to the compiler when marking functions pure or const, or you will cause
// great evil.
//
...
//                         ALSO: No external memory, including memory referenced by parameters, may
//                         be touched. ALSO: function does not throw (makes sense with the above
//                         requirement).
//
// cb_force_inline         What the name says. This is the strongest available inline hint (the
// compiler may still ignore it).
//
//
// b) Usage hints:
//
// cb_must_consume_result  The return value of a function must be consumed (usually because of
// taking ownership), i.e.
//                         ObjType* result = function();  ---> valid
//                         function();                    ---> will raise a warning
//
// cb_deprecated_function  The function is deprecated and may be removed in a future revision of the
// API.
//                         New code should not call the function. Doing so will work, but will raise
//                         a warning.
//
// cb_optional             No warning will be raised if the parameter is not used. Identical to
// "unused",
//                         but the wording sounds more like "you may omit using this" rather than
//                         "we are not using this" Used for interfaces or base classes to convey the
//                         message that a (generally useful) parameter is passed, but it is
//                         allowable to ignore the parameter (and maybe a base class implementation
//                         does just that).
//
// cb_unused               No warning will be raised if the parameter is not used.
//                         Use this if you want to convey that you are aware of a parameter but you
//                         are intentionally not using it.
//
...
/*  ---------------------------------------------------------------------------------------------------------
    The compatibility namespace is intended for workarounds that try to cope with incompatibilities
   in different wxWidgets versions. Since these often involve missing functions or constants, #ifdef
   is explicitly allowed and not frowned upon in this namespace.

    wxHideReadonly
        use in file selector dialog to keep wxWidgets 2.4/2.6 users happy (this flag is important
   for normal operation!), without breaking the 2.8 build.
*/
...
/*  ---------------------------------------------------------------------------------------------------------
    Utility function for an incrementing (unique per instance) unsigned integer ID.
        ...
        - if it _must_ start with 1 for some odd reason, simply add one extra call to
   GetID()<YourClass>
    ...
    Example:
        ...
        In this example, A::X() will return a counter that is globally unique throughout the
   lifetime of the application. A::Y() and B::Y() will return counters that increment independently
   for A and B. In other words, B does not know about A's counter, nor can it influence it. A::Z()
   and B::Z() will return a shared counter that increments if either A or B is asked to return a
   value.
*/
...
// Add std::shared_ptr in a namespace, so different implementations can be used with different
// compilers
namespace cb
{
