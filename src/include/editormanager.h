...
{
    cbNotebookStack(wxWindow *a_pWindow = nullptr) : window(a_pWindow), next(nullptr) {}
...
public:
    ...
    cbEditor *Open(LoaderBase *fileLdr, const wxString &filename, int pos = 0,
    ...
    {
        return IsBuiltinOpen(filename);
    } // synonym of IsBuiltinOpen()
    ...
    /** Closes all editors in the same tab control as the active editor, except the editor passed as
     * parameter. */
...
private:
    ...
    bool IsHeaderSource(const wxFileName &candidateFile, const wxFileName &activeFile,
                        FileType ftActive, bool &isCandidate);
    wxFileName FindHeaderSource(const wxArrayString &candidateFilesArray,
    ...
    /** Returns a vector with pointers to the editors that are in the same tab control as the passed
     * editor. */
