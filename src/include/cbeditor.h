...
protected:
    ...
    cbEditor(wxWindow *parent, LoaderBase *fileLdr, const wxString &filename,
...
public:
    ...
    /** Save fold states within a new cbStyledTextCtrl. This saves the whole document, thus saving
     * the fold states before the Fold Options Change*/
    bool SaveFoldState();

    /** Fix fold states by comparing foldBackup with m_pControl. This is a temp fix for the
     * Scintilla bug*/
    bool FixFoldState();
    ...
    /** Sets the type of folding indicator where id is one of the following: 0->Arrow, 1->Circle,
     * 2->Square, 3->simple */
    ...
    /** Unfolds the block containing \c line. If \c line is -1, unfolds the block containing the
     * caret. */
    void UnfoldBlockFromLine(int line = -1);

    /** Toggles folding of the block containing \c line. If \c line is -1, toggles folding of the
    ...
    /** Returns the specified line's (0-based) indentation (whitespace) in spaces. If line is -1, it
     * uses the current line */
    int GetLineIndentInSpaces(int line = -1) const;

    /** Returns the specified line's (0-based) indentation (whitespace) string. If line is -1, it
     * uses the current line */
    ...
    /** Sets the last modification time for the file to 'now'. Used to detect modifications outside
     * the editor. */
    void Touch();
     ...
     * @param centerOnScreen If true (default), tries to bring the specified line to the centre of
     * the editor.*/
    ...
    /** Refresh all markers for the breakpoints (only the markers for the current debugger will be
     * shown) */
    ...
    virtual void AddToContextMenu(wxMenu *popup, ModuleType type,
                                  bool pluginsdone); // pecan 2006/03/22
