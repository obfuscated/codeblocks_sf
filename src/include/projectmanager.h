...
WX_DECLARE_HASH_MAP(cbProject *, ProjectsArray *, wxPointerHash, wxPointerEqual,
                    DepsMap); // for project dependencies
...
public:
    ...
    virtual void UpdateActiveProject(cbProject *oldProject, cbProject *newProject,
                                     bool refresh) = 0;
    virtual void RemoveProject(cbProject *project) = 0;
    virtual void BeginLoadingWorkspace() = 0;
    virtual void CloseWorkspace() = 0;
    virtual void FinishLoadingProject(cbProject *project, bool newAddition,
                                      FilesGroupsAndMasks *fileGroups) = 0;
    virtual void FinishLoadingWorkspace(cbProject *activeProject,
...
public:
    ...
    /** Reloads a project and tries to keep everything the same (project order, dependencies, active
     * project)
     ...
     */
    int AddMultipleFilesToProject(const wxArrayString &filelist, cbProject *project,
                                  int target = -1);
    /** Add multiple files to a project. This function comes in two versions. This version,
     ...
     * @return true if Loading,closing a project/workspace, or if the app is shutting down. False
     * otherwise
     */
    static bool IsBusy();
    ...
    /** Begins the project loading process. Only to be used by code that needs it (e.g. project
     * importers).
    ...
    /** Ends the project loading process. Only to be used by code that needs it (e.g. project
     * importers).
    ...
    /** Begins the workspace loading process. Only to be used by code that needs it (e.g. workspace
     * importers).
     * @return True on success, false on failure.
    ...
    /** Ends the workspace loading process. Only to be used by code that needs it (e.g. workspace
     * importers).
    ...
    /** Return the project which has the file in it, also return the pointer to the ProjectFile
     * object. */
    cbProject *FindProjectForFile(const wxString &file, ProjectFile **resultFile, bool isRelative,
                                  bool isUnixFilename);
