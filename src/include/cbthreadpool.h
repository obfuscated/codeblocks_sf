...
public:
    /** cbThreadPool ctor
     *
     * @param owner Event handler to receive cbEVT_THREADTASK_ENDED and cbEVT_THREADTASK_ALLDONE
     * events
...
private:
    ...
    public:
         ...
         * @param semaphore Used to synchronize the Worker Threads, it is a reference to the
         * CountedPtr object
         */
    ...
    struct cbThreadedTaskElement
    {
        cbThreadedTaskElement(cbThreadedTask *_task = nullptr, bool _autodelete = false)
          : task(_task), autodelete(_autodelete)
        {
            // empty
        }
    ...
    void _SetConcurrentThreads(
        int concurrentThreads); // like SetConcurrentThreads, but non-thread safe
...
protected:
    ...
    /** Mechanism for the threads to tell the Pool they're done and will go to idle, so we can
     * assign another task to this thread. this function will be called in the worker thread, it
     * just say: hey, I have finished one task thus, decrease the running thread number by one, and
     * let me go to idle mode
     *
    ...
    /** Called by a Worker Thread to inform a single task has finished, this will send a
     * cbEVT_THREADTASK_ENDED event
     *
...
inline void cbThreadPool::Broadcast()
{
    ...
    if (m_concurrentThreads == -1)
        return;
    // let the idle(pending) worker thread to execute tasks, those worker threads are waiting for
    // semaphore
    for (std::size_t i = 0; i < static_cast<std::size_t>(m_concurrentThreads - m_workingThreads);
         ++i)
        m_semaphore->Post();
}
...
inline void cbThreadPool::AwakeNeeded()
{
    ...
    // the thread number to awake should be less than the idle thread number and the tasks queue's
    // size
    std::size_t awakeThreadNumber =
        std::min<std::size_t>(m_tasksQueue.size(), (m_concurrentThreads - m_workingThreads));
