...
wxObject *wxToolBarAddOnXmlHandler::DoCreateResource()
{
    ...
    if (m_class == _T("tool"))
    {
        wxCHECK_MSG(m_toolbar, NULL,
                    _("Incorrect syntax of XRC resource: tool not within a toolbar!"));
...
#if !wxCHECK_VERSION(3, 0, 0)
                               GetBool(_T("toggle")), GetPosition().x, GetPosition().y, NULL,
#else
                               wxITEM_NORMAL,
#endif
                               GetText(_T("tooltip")), GetText(_T("longhelp")));
        ...
        else
        {
            ...
            m_toolbar->AddTool(GetID(), GetText(_T("label")),
                               GetCenteredBitmap(_T("bitmap"), wxART_TOOLBAR, bitmapSize),
                               GetCenteredBitmap(_T("bitmap2"), wxART_TOOLBAR, bitmapSize), kind,
                               GetText(_T("tooltip")), GetText(_T("longhelp")));
            if (GetBool(_T("disabled")))
            {
                m_toolbar->Realize();
    ...
    {
        wxCHECK_MSG(m_toolbar, NULL,
                    _("Incorrect syntax of XRC resource: separator not within a toolbar!"));
    ...
    else /*<object class="wxToolBar">*/
    {
        ...
        else
        {
            ...
            toolbar->Create(m_parentAsWindow, GetID(), GetPosition(), GetSize(), style, GetName());
        ...
        while (n)
        {
                ...
                && (n->GetName() == _T("object") || n->GetName() == _T("object_ref")))
            {
                ...
                if (!IsOfClass(n, _T("tool")) && !IsOfClass(n, _T("separator")) && control != NULL
                    && control != toolbar)
                {
                    // Manager::Get()->GetLogManager()->DebugLog(F(_T("control=%p, parent=%p,
                    // toolbar=%p"), control, control->GetParent(), toolbar));
...
// Don't ask me why... >:-|
    ...
    return ((!m_isInside && istbar) || (m_isInside && istool) || (m_isInside && issep));
}
