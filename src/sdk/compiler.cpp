...
{
    if (ConfigManager::LocateDataFile(wxT("compilers/options_") + GetID() + wxT(".xml"),
                                      sdDataUser | sdDataGlobal)
            .IsEmpty())
...
wxString Compiler::MakeInvalidCompilerMessages() const
{
    ...
    {
        triedPathsMsgs +=
            F(_T("Tried to run compiler executable '%s', but failed!\n"), tmp.wx_str());
...
void Compiler::SaveSettings(const wxString &baseKey)
{
    ...
    if (m_Mirror.ExtraPaths != m_ExtraPaths)
        cfg->Write(tmp + _T("/extra_paths"),
                   GetStringFromArray(MakeUniqueArray(m_ExtraPaths, true), _T(";")), true);
    ...
    for (int i = 0; i < ctCount; ++i)
    {
        for (size_t n = 0; n < m_Commands[i].size(); ++n)
        {
            ...
            {
                wxString key = wxString::Format(_T("%s/macros/%s/tool%lu/"), tmp.c_str(),
                                                CommandTypeDescriptions[i].c_str(),
    ...
    if (m_Mirror.Switches.linkerNeedsLibExtension != m_Switches.linkerNeedsLibExtension)
        cfg->Write(tmp + _T("/switches/linkerNeedsLibExtension"),
                   m_Switches.linkerNeedsLibExtension);
    if (m_Mirror.Switches.linkerNeedsPathResolved != m_Switches.linkerNeedsPathResolved)
        cfg->Write(tmp + _T("/switches/linkerNeedsPathResolved"),
                   m_Switches.linkerNeedsPathResolved);
...
void Compiler::LoadSettings(const wxString &baseKey)
{
    ...
    m_ExtraPaths = MakeUniqueArray(
        GetArrayFromString(cfg->Read(tmp + _T("/extra_paths"), _T("")), _T(";")), true);
    ...
    // set member variable containing the version string with the configuration toolchain
    // executables, not only with the default ones, otherwise we might have an empty version-string
    ...
    SetResourceCompilerOptions(
        GetArrayFromString(cfg->Read(tmp + _T("/resource_compiler_options"), wxEmptyString)));
    ...
    SetResourceIncludeDirs(
        GetArrayFromString(cfg->Read(tmp + _T("/res_include_dirs"), wxEmptyString)));
    ...
    SetCommandsBeforeBuild(
        GetArrayFromString(cfg->Read(tmp + _T("/commands_before"), wxEmptyString)));
    SetCommandsAfterBuild(
        GetArrayFromString(cfg->Read(tmp + _T("/commands_after"), wxEmptyString)));

    for (int i = 0; i < ctCount; ++i)
    {
        wxArrayString keys =
            cfg->EnumerateSubPaths(tmp + _T("/macros/") + CommandTypeDescriptions[i]);
        for (size_t n = 0; n < keys.size(); ++n)
        {
            ...
            if (keys[n].Mid(4).ToULong(&index)) // skip 'tool'
            {
                ...
                wxString key = wxString::Format(_T("%s/macros/%s/tool%lu/"), tmp.c_str(),
                                                CommandTypeDescriptions[i].c_str(), index);
    ...
    m_Switches.objectExtension =
        cfg->Read(tmp + _T("/switches/objectext"), m_Switches.objectExtension);
    m_Switches.needDependencies =
        cfg->ReadBool(tmp + _T("/switches/deps"), m_Switches.needDependencies);
    m_Switches.forceCompilerUseQuotes =
        cfg->ReadBool(tmp + _T("/switches/forceCompilerQuotes"), m_Switches.forceCompilerUseQuotes);
    m_Switches.forceLinkerUseQuotes =
        cfg->ReadBool(tmp + _T("/switches/forceLinkerQuotes"), m_Switches.forceLinkerUseQuotes);
    m_Switches.logging =
        (CompilerLoggingType)cfg->ReadInt(tmp + _T("/switches/logging"), m_Switches.logging);
    m_Switches.libPrefix = cfg->Read(tmp + _T("/switches/libPrefix"), m_Switches.libPrefix);
    m_Switches.libExtension =
        cfg->Read(tmp + _T("/switches/libExtension"), m_Switches.libExtension);
    m_Switches.linkerNeedsLibPrefix =
        cfg->ReadBool(tmp + _T("/switches/linkerNeedsLibPrefix"), m_Switches.linkerNeedsLibPrefix);
    m_Switches.linkerNeedsLibExtension = cfg->ReadBool(
        tmp + _T("/switches/linkerNeedsLibExtension"), m_Switches.linkerNeedsLibExtension);
    m_Switches.linkerNeedsPathResolved = cfg->ReadBool(
        tmp + _T("/switches/linkerNeedsPathResolved"), m_Switches.linkerNeedsPathResolved);
    m_Switches.forceFwdSlashes =
        cfg->ReadBool(tmp + _T("/switches/forceFwdSlashes"), m_Switches.forceFwdSlashes);
    m_Switches.supportsPCH =
        cfg->ReadBool(tmp + _T("/switches/supportsPCH"), m_Switches.supportsPCH);
    m_Switches.PCHExtension =
        cfg->Read(tmp + _T("/switches/pchExtension"), m_Switches.PCHExtension);
    m_Switches.UseFlatObjects =
        cfg->ReadBool(tmp + _T("/switches/UseFlatObjects"), m_Switches.UseFlatObjects);
    m_Switches.UseFullSourcePaths =
        cfg->ReadBool(tmp + _T("/switches/UseFullSourcePaths"), m_Switches.UseFullSourcePaths);
    m_Switches.Use83Paths = cfg->ReadBool(tmp + _T("/switches/Use83Paths"), m_Switches.Use83Paths);
    m_Switches.includeDirSeparator = (wxChar)cfg->ReadInt(tmp + _T("/switches/includeDirSeparator"),
                                                          (int)m_Switches.includeDirSeparator);
    m_Switches.libDirSeparator = (wxChar)cfg->ReadInt(tmp + _T("/switches/libDirSeparator"),
                                                      (int)m_Switches.libDirSeparator);
    m_Switches.objectSeparator = (wxChar)cfg->ReadInt(tmp + _T("/switches/objectSeparator"),
                                                      (int)m_Switches.objectSeparator);
    m_Switches.statusSuccess =
        cfg->ReadInt(tmp + _T("/switches/statusSuccess"), m_Switches.statusSuccess);
    ...
    for (size_t i = 0; i < keys.GetCount(); ++i)
    {
        ...
        RegExStruct rs(cfg->Read(group + _T("/description")),
                       ...
                       cfg->Read(group + _T("/regex")), cfg->ReadInt(group + _T("/msg1"), 0),
                       cfg->ReadInt(group + _T("/filename"), 0),
                       cfg->ReadInt(group + _T("/line"), 0), cfg->ReadInt(group + _T("/msg2"), 0),
    ...
    if (versionMismatch)
    {
        ...
        msg << _("Some compiler settings defaults have changed in this version.\n"
                 ...
                 "Only disallow this if you don't want to lose any customizations you have done to "
                 "this compiler's settings.\n\n"
                 "Note that the only settings that are affected are those found in \"Advanced "
                 "compiler options\"...\n\n"
...
void Compiler::LoadDefaultOptions(const wxString &name, int recursion)
{
    ...
    wxString doc = ConfigManager::LocateDataFile(wxT("compilers/options_") + name + wxT(".xml"),
                                                 sdDataUser | sdDataGlobal);
    if (doc.IsEmpty())
    {
    ...
    if (!options.Load(doc))
    {
        wxString msg(_("Error: Compiler options file '") + doc + _("' not found for compiler '")
                     + name + wxT("'."));
        Manager::Get()->GetLogManager()->Log(msg);
        cbMessageBox(msg, _("Compiler options"), wxICON_ERROR);
        return;
    ...
    while (node)
    {
        ...
        else if (node->GetName() == wxT("Program")) // configuration is read so execution of renamed
                                                    // programs work, m_Mirror is needed to reset
        ...
        else if (node->GetName() == wxT("Option"))
        {
            ...
            m_Options.AddOption(
                wxGetTranslation(node->GetAttribute(wxT("name"), wxEmptyString)),
                node->GetAttribute(wxT("option"), wxEmptyString), wxGetTranslation(category),
                ...
                node->GetAttribute(wxT("supersedes"), wxEmptyString), exclusive == wxT("true"));
        }
        ...
        else if (node->GetName() == wxT("Sort"))
        {
            ...
            if (node->GetAttribute(wxT("CFlags"), &flags))
            {
                ...
                SetCOnlyFlags(MakeUniqueString(GetCOnlyFlags() + wxT(" ") + flags, wxT(" ")));
            }
            else if (node->GetAttribute(wxT("CPPFlags"), &flags))
            {
                flags.Replace(wxT("\n"), wxT(" "));
                flags.Replace(wxT("\r"), wxT(" "));
                SetCPPOnlyFlags(MakeUniqueString(GetCPPOnlyFlags() + wxT(" ") + flags, wxT(" ")));
            }
        }
        else if (node->GetName() == wxT("Common"))
        {
            LoadDefaultOptions(wxT("common_") + node->GetAttribute(wxT("name"), wxEmptyString),
                               recursion + 1);
        }
    ...
    if (recursion == 0) // reset programs to their actual defaults (customized settings are loaded
...
void Compiler::LoadRegExArray(const wxString &name, bool globalPrecedence, int recursion)
{
    ...
    if (doc.IsEmpty())
    {
        Manager::Get()->GetLogManager()->Log(_("Error: file 'options_") + name
                                             + _(".xml' not found"));
        return;
    }
    if (recursion > 5)
    {
        Manager::Get()->GetLogManager()->LogWarning(_("Warning: '") + doc
                                                    + _("' not loaded due to excessive recursion"));
        return;
    }
    if (!options.Load(doc))
    {
    ...
    while (node)
    {
        ...
        else if (node->GetName() == wxT("RegEx"))
        {
            ...
            if (tp == wxT("warning"))
                ...
                clt = cltInfo;
            wxArrayString msg =
                GetArrayFromString(node->GetAttribute(wxT("msg"), wxEmptyString) + wxT(";0;0"));
            m_RegExes.push_back(RegExStruct(
                wxGetTranslation(node->GetAttribute(wxT("name"), wxEmptyString)), clt,
                node->GetNodeContent().Trim().Trim(false), wxAtoi(msg[0]),
                wxAtoi(node->GetAttribute(wxT("file"), wxT("0"))),
                wxAtoi(node->GetAttribute(wxT("line"), wxT("0"))), wxAtoi(msg[1]), wxAtoi(msg[2])));
        }
        else if (node->GetName() == wxT("Common"))
        {
    ...
    else if (node->GetAttribute(wxT("exec"), &test))
    {
        ...
        const wxString origPath = path;
        {
            ...
            if (cfg->Exists(loc + wxT("/name")))
            {
                ...
                extraPaths = MakeUniqueArray(
                    GetArrayFromString(cfg->Read(loc + wxT("/extra_paths"), wxEmptyString)), true);
            }
            for (size_t i = 0; i < extraPaths.GetCount(); ++i)
                path.Prepend(extraPaths[i] + wxPATH_SEP);
            if (!masterPath.IsEmpty())
                path.Prepend(masterPath + wxPATH_SEP + masterPath + wxFILE_SEP_PATH + wxT("bin")
                             + wxPATH_SEP);
        }
