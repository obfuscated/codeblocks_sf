...
template<>
TemplateManager *Mgr<TemplateManager>::instance = nullptr;
template<>
bool Mgr<TemplateManager>::isShutdown = false;
...
void TemplateManager::LoadUserTemplates()
{
    ...
    Manager::Get()->GetLogManager()->DebugLog(
        F(_T("%d user templates loaded"), m_UserTemplates.GetCount()));
}
...
cbProject *TemplateManager::NewProjectFromUserTemplate(NewFromTemplateDlg &dlg, wxString *pFilename)
{
    ...
    if (!dlg.SelectedUserTemplate())
    {
        Manager::Get()->GetLogManager()->DebugLog(_T("TemplateManager::NewProjectFromUserTemplate(")
                                                  _T(") called when no user template was selected ")
                                                  _T("?!?"));
        return NULL;
    }
    ...
    path = ChooseDirectory(nullptr, _("Choose a directory to create the new project"), path, _T(""),
                           false, true);
    if (path.IsEmpty())
        return NULL;
    ...
    if (wxDirExists(path))
    {
        ...
        if (dir.HasFiles() || dir.HasSubDirs())
        {
            if (cbMessageBox(
                    path
                        + _(" already contains other files.\n"
                            "If you continue, files with the same names WILL BE OVERWRITTEN.\n"
                            "Are you sure you want to continue?"),
                    _("Files exist in directory"), wxICON_EXCLAMATION | wxYES_NO | wxNO_DEFAULT)
                != wxID_YES)
            {
                return nullptr;
            }
    ...
    {
        Manager::Get()->GetLogManager()->DebugLog(
            F(_T("Cannot open user-template source path '%s'!"), templ.wx_str()));
        return NULL;
    }
    ...
    for (size_t i = 0; i < files.GetCount(); ++i)
    {
        ...
        //        Manager::Get()->GetLogManager()->DebugLog("dst=%s, dstname=%s", dst.c_str(),
        //        dstname.GetFullPath().c_str());
        ...
        if (wxCopyFile(src, dst, true))
        {
            ...
            Manager::Get()->GetLogManager()->DebugLog(
                F(_T("Failed copying %s to %s"), src.wx_str(), dst.wx_str()));
#else
            Manager::Get()->GetLogManager()->DebugLog(
                F(_T("Failed copying %s to %s"), src.c_str(), dst.c_str()));
#endif
    }
    if (count != total_count)
        cbMessageBox(_("Some files could not be loaded with the template..."), _("Error"),
                     wxICON_ERROR);
    else
    {
        // open new project
        ...
        else
        {
            ...
            wxString newname = cbGetTextFromUser(
                _("If you want, you can change the project's filename here (without extension):"),
                _("Change project's filename"), fname.GetName());
            ...
            if (prj && !newname.IsEmpty())
            {
                ...
                for (int i = 0; i < prj->GetBuildTargetsCount(); ++i)
                {
                    ...
                    if (tgfpPrefix == tgfpPlatformDefault && bt->GetTargetType() == ttStaticLib)
                    {
                        Compiler *projectCompiler =
                            CompilerFactory::GetCompiler(bt->GetCompilerID());
                        if (projectCompiler)
                ...
                Manager::Get()
                    ->GetProjectManager()
                    ->GetUI()
                    .RebuildTree(); // so the tree shows the new name
...
void TemplateManager::SaveUserTemplate(cbProject *prj)
{
    ...
    if (!prj->SaveAllFiles() || !prj->Save())
    {
        cbMessageBox(_("Could not save project and/or all its files. Aborting..."), _("Error"),
                     wxICON_ERROR);
        return;
    }
    ...
    while (true)
    {
        ...
        wxTextEntryDialog dlg(nullptr, _("Enter a title for this template"), _("Enter title"),
                              title);
        PlaceWindow(&dlg);
        if (dlg.ShowModal() != wxID_OK)
            return;
...
#if wxCHECK_VERSION(3, 0, 0)
        Manager::Get()->GetLogManager()->DebugLog(
            F(_T("Copying %s to %s"), src.wx_str(), dst.wx_str()));
#else
        Manager::Get()->GetLogManager()->DebugLog(
            F(_T("Copying %s to %s"), src.c_str(), dst.c_str()));
#endif
        if (!CreateDirRecursively(dst))
            Manager::Get()->GetLogManager()->DebugLog(_T("Failed creating directory for ") + dst);
        if (wxCopyFile(src, dst, true))
    ...
    {
        cbMessageBox(_("Failed to create the directory for the project file!"), _("Error"),
                     wxICON_ERROR);
        ++count;
    }
    else
    {
        ...
        {
            Manager::Get()->GetLogManager()->DebugLog(_T("Failed to copy the project file: ")
                                                      + fname.GetFullPath());
    ...
    if (count == total_count)
        cbMessageBox(_("User-template saved successfully"), _("Information"),
                     wxICON_INFORMATION | wxOK);
    else
        cbMessageBox(_("Some files could not be saved with the template..."), _("Error"),
                     wxICON_ERROR);
}
