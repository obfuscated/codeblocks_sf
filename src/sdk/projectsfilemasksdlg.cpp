...
ProjectsFileMasksDlg::ProjectsFileMasksDlg(wxWindow *parent, FilesGroupsAndMasks *fgam)
  : m_FileGroupsAndMasksCopy(
        *fgam), // store a local copy, so if we press "Cancel", we can revert to the original...
    m_pFileGroupsAndMasks(fgam),
    m_LastListSelection(0)
{
    wxXmlResource::Get()->LoadObject(this, parent, _T("dlgProjectsFileMasks"),
...
{
    wxString groupName = cbGetTextFromUser(_("Enter the new group name:"), _("New group"));
...
void ProjectsFileMasksDlg::OnEdit(cb_unused wxCommandEvent &event)
{
    ...
    wxString groupName = cbGetTextFromUser(_("Rename the group:"), _("Edit group"), oldName);
