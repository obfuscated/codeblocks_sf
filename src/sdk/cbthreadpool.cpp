...
cbThreadPool::~cbThreadPool()
{
    ...
    std::for_each(m_tasksQueue.begin(), m_tasksQueue.end(),
                  std::mem_fun_ref(&cbThreadedTaskElement::Delete));
}
...
void cbThreadPool::_SetConcurrentThreads(int concurrentThreads)
{
    if (!m_workingThreads) // if pool is not running (no thread is running)
    {
        ...
        //    Manager::Get()->GetLogManager()->DebugLog(_T("Concurrent threads for pool set to %d"),
        //    m_concurrentThreads);
    }
    else
...
void cbThreadPool::AddTask(cbThreadedTask *task, bool autodelete)
{
    ...
    // m_workingThreads < m_concurrentThreads means there are some threads in idle mode (no task
    // assigned)
...
bool cbThreadPool::WaitingThread()
{
    ...
    if (m_workingThreads <= 0 && m_tasksQueue.empty())
    {
        ...
        // The last active thread is now waiting and there's a pending new number of threads to
        // assign...
...
cbThreadPool::cbWorkerThread::cbWorkerThread(cbThreadPool *pool, CountedPtr<wxSemaphore> &semaphore)
  : m_abort(false), m_pPool(pool), m_semaphore(semaphore), m_pTask(nullptr)
{
    // empty
}
