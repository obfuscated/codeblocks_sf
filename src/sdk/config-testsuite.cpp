...
c->WriteBinary("/trailing/slash/window-layout",
               wxString("Binary data. This can be anything which fits in a wxString or a pointer "
                        "plus a size_t argument."));
...
c->Read<MySerializableLongIntClass>("boo",
                                    (OldConfigManagerContainer::SerializableObjectMap *)&objMap2);
objMap2["a"]->Print();
...
ConfigManager *cv = Manager::Get()->GetConfigManager(
    "volatile:main"); // volatile offers DOM to store temporary data
...
sset.insert("boing");
sset.insert("boom");
sset.insert("Was würde der Fürst von Thurn & Taxis zu Umlauten sagen?"); // test how good tinyXML is
                                                                         // at quoting special chars
                                                                         // :)
