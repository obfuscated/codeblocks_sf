...
SQInteger ConfigManager_Read(HSQUIRRELVM v)
{
    ...
    if (paramCount == 3)
    {
        ...
        if (sa.GetType(3) == OT_INTEGER)
            return sa.Return((SQInteger)Manager::Get()
                                 ->GetConfigManager(_T("scripts"))
                                 ->ReadInt(key, sa.GetInt(3)));
        else if (sa.GetType(3) == OT_BOOL)
            return sa.Return(
                Manager::Get()->GetConfigManager(_T("scripts"))->ReadBool(key, sa.GetBool(3)));
        else if (sa.GetType(3) == OT_FLOAT)
            return sa.Return((float)Manager::Get()
                                 ->GetConfigManager(_T("scripts"))
                                 ->ReadDouble(key, sa.GetFloat(3)));
        else
        {
...
SQInteger ConfigManager_Write(HSQUIRRELVM v)
{
    ...
    if (paramCount == 3)
    {
        ...
        else
        {
            Manager::Get()
                ->GetConfigManager(_T("scripts"))
                ->Write(key, *SqPlus::GetInstance<wxString, false>(v, 3));
            return SQ_OK;
        }
    }
...
SQInteger EditorManager_GetBuiltinEditor(HSQUIRRELVM v)
{
    ...
    if (paramCount == 2)
    {
        ...
        if (sa.GetType(2) == OT_INTEGER)
            ...
            ed = Manager::Get()->GetEditorManager()->GetBuiltinEditor(
...
SQInteger EditorManager_Open(HSQUIRRELVM v)
{
    ...
    {
        cbEditor *ed =
            Manager::Get()->GetEditorManager()->Open(*SqPlus::GetInstance<wxString, false>(v, 2));
        SqPlus::Push(v, ed);
        return 1;
    }
...
SQInteger EditorManager_Close(HSQUIRRELVM v)
{
    ...
    if (paramCount == 2)
    {
        if (sa.GetType(2) == OT_INTEGER)
            ...
            return sa.Return(Manager::Get()->GetEditorManager()->Close(
                *SqPlus::GetInstance<wxString, false>(v, 2)));
    }
...
SQInteger EditorManager_Save(HSQUIRRELVM v)
{
    ...
    if (paramCount == 2)
    {
        if (sa.GetType(2) == OT_INTEGER)
            ...
            return sa.Return(Manager::Get()->GetEditorManager()->Save(
                *SqPlus::GetInstance<wxString, false>(v, 2)));
    }
...
SQInteger cbProject_RenameBuildTarget(HSQUIRRELVM v)
{
    ...
    if (paramCount == 3)
    {
        ...
        if (sa.GetType(2) == OT_INTEGER)
            return sa.Return(
                prj->RenameBuildTarget(sa.GetInt(2), *SqPlus::GetInstance<wxString, false>(v, 3)));
        else
            return sa.Return(prj->RenameBuildTarget(*SqPlus::GetInstance<wxString, false>(v, 2),
...
SQInteger cbProject_DuplicateBuildTarget(HSQUIRRELVM v)
{
    ...
    if (paramCount == 3)
    {
        ...
        ProjectBuildTarget *bt = nullptr;
        if (sa.GetType(2) == OT_INTEGER)
            bt = prj->DuplicateBuildTarget(sa.GetInt(2),
                                           *SqPlus::GetInstance<wxString, false>(v, 3));
        else
            bt = prj->DuplicateBuildTarget(*SqPlus::GetInstance<wxString, false>(v, 2),
                                           *SqPlus::GetInstance<wxString, false>(v, 3));
        SqPlus::Push(v, bt);
        return 1;
    }
...
SQInteger cbProject_ExportTargetAsProject(HSQUIRRELVM v)
{
    ...
    if (paramCount == 2)
    {
        ...
        if (sa.GetType(2) == OT_INTEGER)
            ...
            return sa.Return(
                prj->ExportTargetAsProject(*SqPlus::GetInstance<wxString, false>(v, 2)));
    }
...
SQInteger ProjectManager_AddFileToProject(HSQUIRRELVM v)
{
    ...
    if (paramCount == 4)
    {
        if (sa.GetType(4) == OT_INTEGER)
        {
            ...
            int idx = sa.GetInt(4);
            return sa.Return(
                (SQInteger)Manager::Get()->GetProjectManager()->AddFileToProject(fname, prj, idx));
        }
    }
...
SQInteger CompilerFactory_GetCompilerIndex(HSQUIRRELVM v)
{
    ...
    int paramCount = sa.GetParamCount();
    if (paramCount == 2)
        return sa.Return((SQInteger)CompilerFactory::GetCompilerIndex(
            *SqPlus::GetInstance<wxString, false>(v, 2)));
...
void RegisterBindings()
{
    ...
    SqPlus::SQClassDef<CompileTargetBase>("CompileTargetBase", "CompileOptionsBase")
        ...
        .
        //                func(&CompileTargetBase::GetTargetFilenameGenerationPolicy,
        //                "GetTargetFilenameGenerationPolicy"). // not exposed because its args are
    ...
    SqPlus::SQClassDef<ProjectManager>("ProjectManager")
        ...
        .
        //                func(&ProjectManager::ConfigureProjectDependencies,
