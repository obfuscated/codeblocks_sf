...
bool SecurityAllows(const wxString &operation, const wxString &descr)
{
    ...
    switch (response)
    {
        ...
        case ssrTrustPermanently:
            Manager::Get()->GetScriptingManager()->TrustCurrentlyRunningScript(
                response == ssrTrustPermanently);
            return true;
...
{
    return ChooseDirectory(nullptr, message,
                           Manager::Get()->GetMacrosManager()->ReplaceMacros(initialPath),
                           wxEmptyString, false, showCreateDirButton);
}
...
bool CopyFile(const wxString &src, const wxString &dst, bool overwrite)
{
    ...
    if (!SecurityAllows(_T("CopyFile"), wxString::Format(_T("%s -> %s"), src.c_str(), dst.c_str())))
        return false;
    if (!wxFileExists(fname1.GetFullPath()))
        return false;
    return wxCopyFile(fname1.GetFullPath(), fname2.GetFullPath(), overwrite);
}
...
bool RenameFile(const wxString &src, const wxString &dst)
{
    ...
    NormalizePath(fname1, wxEmptyString);
    NormalizePath(fname2, wxEmptyString);
    if (!SecurityAllows(_T("RenameFile"),
                        wxString::Format(_T("%s -> %s"), fname1.GetFullPath().c_str(),
                                         fname2.GetFullPath().c_str())))
        return false;
    if (!wxFileExists(fname1.GetFullPath()))
        return false;
    return wxRenameFile(fname1.GetFullPath(), fname2.GetFullPath());
}
...
{
    wxFileDialog dlg(nullptr, title, wxEmptyString,
                     Manager::Get()->GetMacrosManager()->ReplaceMacros(defaultFile), filter,
