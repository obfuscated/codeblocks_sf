...
public:
    XrcDialog(wxWindow *parent, const wxString &dlgName, const wxString &callback)
      : m_CallBack(callback)
    {
        // first try to load dlgName as wxDialog, if that does not work, try to load it as
        // wxScrollingDialog if both does not work, throw an exception
...
void XrcDialog::OnButton(wxCommandEvent &event)
{
    ...
    try
    {
        //            Manager::Get()->GetLogManager()->DebugLog(F(_T("Script dialog event: %d"),
        //            event.GetId()));
...
void EndModal(int retCode)
{
    ...
    cbMessageBox(_("EndModal() only valid while inside a ShowDialog() call..."), _("Error"),
                 wxICON_ERROR);
}
...
SQInteger XrcId(HSQUIRRELVM v)
{
    ...
    if (!s_ActiveDialog)
    {
        cbMessageBox(_("XRCID() only valid while inside a ShowDialog() call..."), _("Error"),
                     wxICON_ERROR);
    ...
    if (sa.GetType(2) == OT_STRING)
        ...
        win =
            wxWindow::FindWindowByName(*SqPlus::GetInstance<wxString, false>(v, 2), s_ActiveDialog);
