...
{
    return cbMessageBox(msg, caption, buttons);
}
...
{
    return Manager::Get()->GetMacrosManager()->ReplaceMacros(buffer);
}
...
bool InstallPlugin(const wxString &pluginName, bool allUsers, bool confirm)
{
    if (cbMessageBox(_("A script is trying to install a Code::Blocks plugin.\n"
                       "Do you wish to allow this?\n\n")
                         + pluginName,
                     _("Security warning"), wxICON_WARNING | wxYES_NO)
        == wxID_NO)
    {
        return false;
    }
...
{
    return 0; /* leaving script binding intact for compatibility, but this is factually not
                 implemented at all */
}
        ...
        else
        {
            if (isLast)
            {
                ...
                if (id != wxNOT_FOUND)
                {
                    ...
                    {
                        wxString msg;
                        msg.Printf(_("Calling the menu '%s' with ID %d failed."), menuPath.wx_str(),
                                   id);
...
SQInteger wx_GetColourFromUser(HSQUIRRELVM v)
{
    StackHandler sa(v);
    const wxColour &c =
        sa.GetParamCount() == 2 ? *SqPlus::GetInstance<wxColour, false>(v, 2) : *wxBLACK;
    return SqPlus::ReturnCopy(v, wxGetColourFromUser(Manager::Get()->GetAppWindow(), c));
}
long wx_GetNumberFromUser(const wxString &message, const wxString &prompt, const wxString &caption,
                          long value)
{
