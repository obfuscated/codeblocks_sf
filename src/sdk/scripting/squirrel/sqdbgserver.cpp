...
void SQDbgServer::InformNewFileLoaded(HSQUIRRELVM v,const SQChar *src)
{
    SQInteger vm_state = sq_getvmstate(v);
    if(vm_state !=  SQ_VMSTATE_SUSPENDED)
        sq_suspendvm(v);

    BeginDocument();
        BeginElement(_SC("file_loaded"));
            Attribute(_SC("src"),src);
        EndElement(_SC("file_loaded"));
    EndDocument();

    if(vm_state !=  SQ_VMSTATE_SUSPENDED)
        sq_wakeupvm(v,false,false,false,false);
}
