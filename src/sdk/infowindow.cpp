...
const char *iBitmap[] = {"20 20 38 1",           "  c #606060",          ". c gray38",
                         "X c #646464",          "o c #656565",          "O c gray40",
                         "+ c gray45",           "@ c gray50",           "# c blue",
                         "$ c #0808FF",          "% c #0A0AFF",          "& c #0B0BFF",
                         "* c #2121FF",          "= c #2828FF",          "- c #3838FF",
                         "; c #4545FF",          ": c #4646FF",          "> c #5656FF",
                         ", c #5959FF",          "< c gray51",           "1 c #8E8E8E",
                         "2 c gray59",           "3 c #9191A7",          "4 c #8888EF",
                         "5 c #8080F7",          "6 c #8282F7",          "7 c #8787FE",
                         "8 c #8A8AFE",          "9 c #9696FE",          "0 c #9797FF",
                         "q c #ABABFF",          "w c #CDCDFF",          "e c #D5D5FF",
                         "r c #E4E4F9",          "t c gray96",           "y c #F6F6F6",
                         "u c gray97",           "i c #F8F8F8",          "p c gray100",
                         "     X+<1221<+O     ", "   X+35,=&&=,63+X   ", "  X@4:########:4@X  ",
                         " X@5*#&>0ww0,&#*5@X ", " +4*#-qppppppq:#*4+ ", "X3:#:eppp##pppe:#:3X",
                         "+6#&qpppp##ppppq&#6+", "<,#,ppppp##ppppp,#,@", "1=#9ppppp##ppppp0#=1",
                         "2&#wppppp##pppppw#&2", "i&#wppppp##pppppw#&u", "i=#9ppppp##ppppp0#=u",
                         "u,#>pppppppppppp>#,y", "y8#&qpppp##ppppq$#8y", "yr:#:eppp##pppe:#:rt",
                         "yy9*#:qppppppq:#*9yt", "yyy7*#$>0ww0>&#*7ytt", "ttyy9:########:9yytt",
                         "ttttyr8,=&&=,8ryyttt", "ttttyyyyiuuuuyyyyttt"};
...
public:
    ForwardingTextControl(wxWindow *parent, wxWindowID id, const wxString &label,
...
InfoWindow::InfoWindow(const wxString &title, const wxString &message, unsigned int delay,
    ...
    ks(2)
{
    ...
    if (platform::gtk)
    {
        ...
        titleC = new ForwardingTextControl(pnl, -1, title, wxDefaultPosition, wxDefaultSize,
                                           wxALIGN_CENTRE);
    ...
    {
        titleC = new ForwardingTextControl(this, -1, title, wxDefaultPosition, wxDefaultSize,
                                           wxALIGN_CENTRE);
    ...
    ForwardingTextControl *text =
        new ForwardingTextControl(this, -1, message, wxDefaultPosition, wxDefaultSize, 0);
...
// static
void InfoWindow::Display(const wxString &title, const wxString &message, unsigned int delay,
                         unsigned int hysteresis)
{
    if (std::find(active_messages.begin(), active_messages.end(), message) != active_messages.end())
    {
        const wxString dups =
            _T("Multiple information windows with the same\nmessage have been suppressed.");
        if (std::find(active_messages.begin(), active_messages.end(), dups)
            == active_messages.end())
