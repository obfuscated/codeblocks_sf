    ...
    while (compiler)
    {
        ...
        if (compiler == newcompiler)
        {
            Manager::Get()->GetLogManager()->DebugLog(
                _T("Compiler circular dependency detected?!?!?"));
            break;
        }
...
Compiler *CompilerFactory::CreateCompilerCopy(Compiler *compiler, const wxString &newName)
{
    ...
    Manager::Get()->GetLogManager()->DebugLog(
        F(_T("Added compiler \"%s\""), newC->GetName().wx_str()));
...
void CompilerFactory::RemoveCompiler(Compiler *compiler)
{
    ...
    Manager::Get()
        ...
        ->DeleteSubPath(_T("/user_sets/") + compiler->GetID());

    Compilers.Remove(compiler);
    Manager::Get()->GetLogManager()->DebugLog(
        F(_T("Compiler \"%s\" removed"), compiler->GetName().wx_str()));
...
void CompilerFactory::LoadSettings()
{
    ...
    for (size_t i = 0; i < Compilers.GetCount(); ++i)
    {
        ...
        {
            Manager::Get()->GetLogManager()->DebugLog(
                F(_T("Master path of compiler ID \"%s\" is empty -> triggers auto-detection."),
                  Compilers[i]->GetID().wx_str()));
...
Compiler *CompilerFactory::SelectCompilerUI(const wxString &message, const wxString &preselectedID)
{
    ...
    wxSingleChoiceDialog dlg(nullptr, message, _("Compiler selection"),
                             CompilerFactory::Compilers.GetCount(), comps);
