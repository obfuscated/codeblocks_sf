        ...
        m_pFileLoader(fileLoader)
    {
        ...
        if (m_pFileLoader)
        {
            ...
            Manager::Get()->GetLogManager()->DebugLog(
                F(_T("Encoding via fileloader took : %d ms"), (int)sw.Time()));
#endif
        }
    }
    ...
    void SetLineNumberColWidth(bool both = true)
    {
        ...
        if (cfg->ReadBool(_T("/show_line_numbers"), true))
        {
            if (m_pOwner->m_pControl2 && both)
            {
                ...
                {
                    m_pOwner->m_pControl->SetMarginWidth(
                        C_LINE_MARGIN,
                        pixelWidth * 0.75
                            + cfg->ReadInt(_T("/margin/width_chars"), 6) * pixelWidth);
                    m_pOwner->m_pControl2->SetMarginWidth(
                        C_LINE_MARGIN,
                        pixelWidth * 0.75
                            + cfg->ReadInt(_T("/margin/width_chars"), 6) * pixelWidth);
                }
            ...
            else
            {
                ...
                {
                    control->SetMarginWidth(C_LINE_MARGIN,
                                            pixelWidth * 0.75
                                                + cfg->ReadInt(_T("/margin/width_chars"), 6)
                                                      * pixelWidth);
                }
            }
        }
        ...
        switch (tabSize)
        {
            case 2:
                if ((lineCount[2] - lineCount[6]) / total < 0.1)
                    return 6;
                if (lineCount[2] > lineCount[4] * 1.8 || lineCount[4] / total < 0.5)
                    return 2;
                // fall through
            case 4:
                if (lineCount[4] > lineCount[8] * 1.8 || lineCount[8] / total < 0.5)
                    return 4;
                // fall through
            case 8:
            ...
            case 3:
                if (lineCount[3] > lineCount[6] * 1.8 || lineCount[6] / total < 0.5)
                    return 3;
...
static int DetectLineEnds(cbStyledTextCtrl *control)
{
    ...
    // voting logic
    // if the file does not contain any line-feed or the most largest counts are equal( e.g.:
    // linesLF=5, linesCRLF=5, linesCR=0 ), then we will use the initial EOL mode
    ...
    if (((linesCR > 0) && (linesCRLF > 0)) || ((linesLF > 0) && (linesCRLF > 0))
        || ((linesCR > 0) && (linesLF > 0)))
    {
        // In mixed EOL file, give the user a beep and InfoWindow notification.
        wxBell();
        InfoWindow::Display(_("Mixed Line Endings"),
                            _("Mixed line endings found, setting mode ") + eolModeStr, delay);
    }
    return eolMode;
}
...
// class constructor
cbEditor::cbEditor(wxWindow *parent, LoaderBase *fileLdr, const wxString &filename,
...
void cbEditor::DoInitializations(const wxString &filename, LoaderBase *fileLdr)
{
    ...
    //    m_Filename.c_str(), m_Shortname.c_str());
    ...
    // by default we show no markers, marginMasks are set explicitly in
    // "InternalSetEditorStyleBeforeFileOpen()" and/or by plugins, that use markers, like
    // browsemarks-plugin
    ...
    if (Manager::Get()
            ...
            ->ReadBool(_T("/folding/fold_all_on_open"), false))
        FoldAll();
...
void cbEditor::NotifyPlugins(wxEventType type, int intArg, const wxString &strArg, int xArg,
                             int yArg)
{
...
cbStyledTextCtrl *cbEditor::GetControl() const
{
    ...
    if (m_pControl2)
    {
        // every time a control gets the focus it stores the actual timestamp, the timestamp
        // defaults to 0 so the greater is the timestamp of the control that had the focus last time
...
void cbEditor::SetModified(bool modified)
{
    if (modified != m_Modified)
    {
        ...
        // visual state
        if (m_pProjectFile)
            m_pProjectFile->SetFileState(
                m_pControl->GetReadOnly() ? fvsReadOnly : (m_Modified ? fvsModified : fvsNormal));
    }
}
...
void cbEditor::SetProjectFile(ProjectFile *project_file, bool preserve_modified)
{
    ...
    if (m_pProjectFile)
    {
        ...
        if (Manager::Get()
                ->GetConfigManager(_T("editor"))
                ->ReadBool(_T("/folding/show_folds"), true))
        {
        ...
        if (Manager::Get()
                ->GetConfigManager(_T("editor"))
                ->ReadBool(_T("/tab_text_relative"), true))
...
{
    // avoid gtk-critical because of sizes less than -1 (can happen with
    // wxAuiNotebook/cbAuiNotebook)
...
void cbEditor::ConnectEvents(cbStyledTextCtrl *stc)
{
    ...
    Connect(stcID, wxEVT_SCI_USERLISTSELECTION,
            wxScintillaEventHandler(cbEditor::OnUserListSelection));
    ...
    int scintilla_events[] = {
        ...
        //        wxEVT_SCI_CHARADDED,
        wxEVT_SCI_SAVEPOINTREACHED, wxEVT_SCI_SAVEPOINTLEFT, wxEVT_SCI_ROMODIFYATTEMPT,
        wxEVT_SCI_KEY, wxEVT_SCI_DOUBLECLICK,
        //        wxEVT_SCI_UPDATEUI,
        //        wxEVT_SCI_MODIFIED,
        wxEVT_SCI_MACRORECORD,
        //        wxEVT_SCI_MARGINCLICK,
        wxEVT_SCI_NEEDSHOWN, wxEVT_SCI_PAINTED,
        //        wxEVT_SCI_USERLISTSELECTION,
        wxEVT_SCI_URIDROPPED,
        //        wxEVT_SCI_DWELLSTART,
        //        wxEVT_SCI_DWELLEND,
        wxEVT_SCI_START_DRAG, wxEVT_SCI_FINISHED_DRAG, wxEVT_SCI_DRAG_OVER, wxEVT_SCI_DO_DROP,
        wxEVT_SCI_ZOOM, wxEVT_SCI_HOTSPOT_CLICK, wxEVT_SCI_HOTSPOT_DCLICK, wxEVT_SCI_CALLTIP_CLICK,
        wxEVT_SCI_AUTOCOMP_SELECTION,
        //        wxEVT_SCI_INDICATOR_CLICK,
        //        wxEVT_SCI_INDICATOR_RELEASE,
        wxEVT_SCI_AUTOCOMP_CANCELLED, wxEVT_SCI_TAB, wxEVT_SCI_ESC,
...
void cbEditor::Split(cbEditor::SplitType split)
{
    ...
    m_pSplitter = new wxSplitterWindow(this, wxNewId(), wxDefaultPosition, wxDefaultSize,
    ...
    // we do not call "SetEditorStyleAfterFileOpen" here because it calls SetLanguage for the
    // already loaded text inside the left control and slows down loading of large files a lot.
...
// on wxGTK > 2.9 we need to thaw before reparent and refreeze the editor here or the whole app
// stays frozen
...
// stays frozen
    ...
    // add it in the sizer
    m_pSizer->Add(m_pControl, 1, wxEXPAND);
    // notify the plugin when the right splitter window is not destroyed and the left window is
    // reparented to cbEditor
...
void cbEditor::SetEditorStyleBeforeFileOpen()
{
    ...
    m_pData->m_ensure_consistent_line_ends =
        mgr->ReadBool(_T("/eol/ensure_consistent_line_ends"), false);
...
void cbEditor::InternalSetEditorStyleBeforeFileOpen(cbStyledTextCtrl *control)
{
    ...
    control->SetCaretLineBackground(
        GetOptionColour(_T("/highlight_caret_line_colour"), wxColour(0xFF, 0xFF, 0x00)));
    ...
    if (mgr->ReadBool(_T("/camel_case"), false))
    {
        ...
        control->CmdKeyAssign(wxSCI_KEY_LEFT, wxSCI_SCMOD_CTRL | wxSCI_SCMOD_SHIFT,
                              wxSCI_CMD_WORDPARTLEFTEXTEND);
        control->CmdKeyAssign(wxSCI_KEY_RIGHT, wxSCI_SCMOD_CTRL | wxSCI_SCMOD_SHIFT,
                              wxSCI_CMD_WORDPARTRIGHTEXTEND);
    }
    else // else set default "none CamelCase" key behavior (also default scintilla behaviour, see
         // scintilla docs)
    {
    ...
    control->SetIndentationGuides(
        mgr->ReadBool(_T("/show_indent_guides"), false) ? wxSCI_IV_LOOKBOTH : wxSCI_IV_NONE);
    ...
    if (mgr->ReadBool(_T("/word_wrap_style_home_end"), true))
    {
        ...
        control->CmdKeyAssign(wxSCI_KEY_END, wxSCI_SCMOD_SHIFT | wxSCI_SCMOD_ALT,
        ...
        if (mgr->ReadBool(_T("/simplified_home"), false))
        {
            ...
            control->CmdKeyAssign(wxSCI_KEY_HOME, wxSCI_SCMOD_SHIFT | wxSCI_SCMOD_ALT,
    ...
    { // in word wrap mode, home/end keys goto start/end of the entire line. alt+home/end goes to
      // wrap points
    ...
    control->SetMarginMask(C_MARKER_MARGIN,
                           control->GetMarginMask(C_MARKER_MARGIN) | (1 << BOOKMARK_MARKER)
                               | (1 << BREAKPOINT_MARKER) | (1 << BREAKPOINT_DISABLED_MARKER)
                               | (1 << BREAKPOINT_OTHER_MARKER) | (1 << DEBUG_MARKER)
                               | (1 << DEBUG_MARKER_HIGHLIGHT) | (1 << ERROR_MARKER));
    ...
    const wxString &basepath =
        ConfigManager::GetDataFolder() + wxT("/manager_resources.zip#zip:/images/12x12/");
    ...
    if (imageBP)
    {
        ...
        wxBitmap iconBPDis =
            cbLoadBitmap(basepath + wxT("breakpoint_disabled.png"), wxBITMAP_TYPE_PNG);
        wxBitmap iconBPOth =
            cbLoadBitmap(basepath + wxT("breakpoint_other.png"), wxBITMAP_TYPE_PNG);
    ...
    if (mgr->ReadBool(_T("/margin/use_changebar"), true))
    {
        ...
        control->SetMarginMask(C_CHANGEBAR_MARGIN, control->GetMarginMask(C_CHANGEBAR_MARGIN)
...
void cbEditor::InternalSetEditorStyleAfterFileOpen(cbStyledTextCtrl *control)
{
    ...
    int eolMode =
        mgr->ReadInt(_T("/eol/eolmode"), platform::windows ? wxSCI_EOL_CRLF : wxSCI_EOL_LF);
    ...
    control->SetProperty(_T("lexer.cpp.track.preprocessor"),
                         mgr->ReadBool(_T("/track_preprocessor"), true) ? _T("1") : _T("0"));

    // code folding
    if (mgr->ReadBool(_T("/folding/show_folds"), true))
    {
        control->SetProperty(_T("fold"), _T("1"));
        control->SetProperty(_T("fold.html"),
                             mgr->ReadBool(_T("/folding/fold_xml"), true) ? _T("1") : _T("0"));
        control->SetProperty(_T("fold.comment"), mgr->ReadBool(_T("/folding/fold_comments"), false)
                                                     ? _T("1")
                                                     : _T("0"));
        control->SetProperty(_T("fold.compact"), _T("0"));
        control->SetProperty(_T("fold.preprocessor"),
                             mgr->ReadBool(_T("/folding/fold_preprocessor"), false) ? _T("1")
                                                                                    : _T("0"));
    ...
    control->SetProperty(_T("highlight.wxsmith"),
                         mgr->ReadBool(_T("/highlight_wxsmith"), true) ? _T("1") : _T("0"));
...
bool cbEditor::Open(bool detectEncoding)
{
    ...
    wxStopWatch sw;
#endif
    EncodingDetector enc((wxByte *)m_pData->m_pFileLoader->GetData(),
                         m_pData->m_pFileLoader->GetLength());
    if (detectEncoding)
    {
    ...
    Manager::Get()->GetLogManager()->DebugLog(F(
        _T("cbEditor::Open() => Encoding detection and conversion took : %d ms"), (int)sw.Time()));
    sw.Start();
#endif
...
#ifdef fileload_measuring
    Manager::Get()->GetLogManager()->DebugLog(
        F(_T("loading into editor needs : %d ms"), (int)sw.Time()));
#endif
    return true;
}
...
bool cbEditor::SaveAs()
{
    ...
    SetEditorTitle(m_Shortname);
    // invalidate m_pProjectFile, because if kept, it would point to the ProjectFile with old name
    // and cause ProjectManager::RemoveFileFromProject called via context menu to crash
...
bool cbEditor::FixFoldState()
{
    ...
    if (m_foldBackup)
    {
        ...
        {
            ConfigManager *mgr = Manager::Get()->GetConfigManager(_T("editor"));
            if (mgr->ReadBool(_T("/folding/show_folds"),
                              true)) // Only fix the folds if the folds are enabled
            {
                int count = m_pControl->GetLineCount();
                for (int i = 0; i < count; ++i)
                {
...
void cbEditor::AutoComplete()
{
    Manager::Get()->GetLogManager()->Log(_T("cbEditor::AutoComplete() is obsolete.\nUse ")
                                         _T("AutoComplete(cbEditor &ed) from the Abbreviations ")
                                         _T("plugin instead."));
}
...
bool cbEditor::DoFoldLine(int line, int fold)
{
    ...
    if (level & wxSCI_FOLDLEVELHEADERFLAG)
    {
        ...
        {
            if ((level & wxSCI_FOLDLEVELNUMBERMASK)
                > (wxSCI_FOLDLEVELBASE + m_pData->mFoldingLimitLevel - 1))
                return false;
        }
...
void cbEditor::GotoLine(int line, bool centerOnScreen)
{
    ...
    // when the line is a function declaration (or only contains the opening brace of it [yes, that
    // happens sometimes] ) the body is shown.
    DoFoldLine(line, 0);
    DoFoldLine(line + 1, 0);
...
bool cbEditor::AddBreakpoint(int line, bool notifyDebugger)
{
    ...
    if (dbgManager->GetBreakpointDialog()->AddBreakpoint(dbgManager->GetActiveDebugger(),
                                                         m_Filename, line + 1))
    {
        BreakpointMarkerToggle(line);
        return true;
...
bool cbEditor::RemoveBreakpoint(int line, bool notifyDebugger)
{
    ...
    DebuggerManager *dbgManager = Manager::Get()->GetDebuggerManager();
    if (dbgManager->GetBreakpointDialog()->RemoveBreakpoint(dbgManager->GetActiveDebugger(),
                                                            m_Filename, line + 1))
    {
        BreakpointMarkerToggle(line);
        return true;
...
bool cbEditor::HasBreakpoint(int line) const
{
    ...
    return LineHasMarker(BREAKPOINT_MARKER, line)
           || LineHasMarker(BREAKPOINT_DISABLED_MARKER, line);
}
...
void cbEditor::RefreshBreakpointMarkers()
{
    ...
    const DebuggerManager::RegisteredPlugins &plugins =
...
{
    cbAssert(GetControl());
    GetControl()->EmptyUndoBuffer(Manager::Get()
                                      ->GetConfigManager(_T("editor"))
...
void cbEditor::MarkerNext(int marker)
{
    ...
    if (newLine == -1)
    {
        ...
        if (newLine != -1)
            InfoWindow::Display(_("Find bookmark action"), _("Reached the end of the document"),
                                1000);
    }
...
void cbEditor::GotoMatchingBrace()
{
    ...
    if (matchingBrace == wxSCI_INVALID_POSITION)
    {
        ...
        if (ppIf.Matches(control->GetCurLine()) || ppElse.Matches(control->GetCurLine()))
        {
            ...
            {
                if (control->GetLine(i).Find(wxT('#'))
                    != wxNOT_FOUND) // limit testing due to performance cost
                {
                ...
                if (depth == 0)
                {
                    ...
                    matchingBrace =
                        control->PositionFromLine(i) + pp.GetMatch(control->GetLine(i)).Length();
                    break;
                }
            }
        }
...
void cbEditor::AddToContextMenu(wxMenu *popup, ModuleType type, bool pluginsdone)
{
    ...
    else
    {
        ...
        if (Manager::Get()->GetProjectManager()->GetActiveProject()) // project must be open
        {
            ...
            isAddRemoveEnabled = Manager::Get()
                                     ->GetProjectManager()
                                     ->GetActiveProject()
...
bool cbEditor::OnBeforeBuildContextMenu(const wxPoint &position, ModuleType type)
{
    ...
    if (!noeditor && position != wxDefaultPosition)
    {
        ...
        // but the user right-clicked inside the right control), we find out the active control
        // differently...
        ...
        const int margin = m_pControl->GetMarginWidth(C_LINE_MARGIN) + // numbers, if present
                           ...
                           + // breakpoints, bookmarks... if present
        ...
        if (clientpos.x < margin)
        {
            ...
            if (plugin && plugin->SupportsFeature(cbDebuggerFeature::Breakpoints))
            {
                ...
                bool hasBreakDisabled =
                    LineHasMarker(BREAKPOINT_DISABLED_MARKER, m_pData->m_LastMarginMenuLine);
...
void cbEditor::Print(bool selectionOnly, PrintColourMode pcm, bool line_numbers)
{
    ...
    else
    {
        ...
        Manager::Get()->GetConfigManager(_T("app"))->Write(_T("/printerdialog/paperid"),
                                                           (int)ppd->GetPaperId());
        Manager::Get()->GetConfigManager(_T("app"))->Write(_T("/printerdialog/paperorientation"),
                                                           (int)ppd->GetOrientation());
    }
...
void cbEditor::OnContextMenuEntry(wxCommandEvent &event)
{
    ...
    if (id == idUndo)
        ...
        control->EmptyUndoBuffer(Manager::Get()
                                     ->GetConfigManager(_T("editor"))
                                     ->ReadBool(_T("/margin/use_changebar"), true));
    else if (id == idCut)
        control->Cut();
    ...
    if (!m_autoIndentDone)
    {
        ...
        if ((ch == _T('\n')) || ((control->GetEOLMode() == wxSCI_EOL_CR) && (ch == _T('\r'))))
        {
            ...
            const bool autoIndent =
                Manager::Get()->GetConfigManager(_T("editor"))->ReadBool(_T("/auto_indent"), true);
        ...
        if (!braceCompleted
            && Manager::Get()
                   ->GetConfigManager(_T("editor"))
                   ->ReadBool(_T("/brace_completion"), true))
        {
