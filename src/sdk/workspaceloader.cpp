...
bool WorkspaceLoader::Open(const wxString &filename, wxString &Title)
{
    ...
    Title = cbC2U(
        wksp->Attribute("title")); // Conversion to unicode is automatic (see wxString::operator= )
    ...
    while (proj)
    {
        ...
        if (thisprj)
        {
            ...
            while (dep)
            {
                ...
                fname.MakeAbsolute(wfname.GetPath(wxPATH_GET_VOLUME | wxPATH_GET_SEPARATOR));
                cbProject *depprj =
                    Manager::Get()->GetProjectManager()->IsOpen(fname.GetFullPath());
                if (depprj)
...
bool WorkspaceLoader::Save(const wxString &title, const wxString &filename)
{
    ...
    TiXmlElement *wksp =
        static_cast<TiXmlElement *>(rootnode->InsertEndChild(TiXmlElement("Workspace")));
    ...
    for (unsigned int i = 0; i < arr->GetCount(); ++i)
    {
        ...
        TiXmlElement *node =
            static_cast<TiXmlElement *>(wksp->InsertEndChild(TiXmlElement("Project")));
        node->SetAttribute("filename", cbU2C(UnixFilename(fname.GetFullPath(), wxPATH_UNIX)));

        const ProjectsArray *deps =
            Manager::Get()->GetProjectManager()->GetDependenciesForProject(prj);
        if (deps && deps->GetCount())
        {
            for (size_t j = 0; j < deps->GetCount(); ++j)
            {
                ...
                fname.MakeRelativeTo(wfname.GetPath(wxPATH_GET_VOLUME | wxPATH_GET_SEPARATOR));
                TiXmlElement *dnode =
                    static_cast<TiXmlElement *>(node->InsertEndChild(TiXmlElement("Depends")));
                dnode->SetAttribute("filename",
                                    cbU2C(UnixFilename(fname.GetFullPath(), wxPATH_UNIX)));
            }
        }
    }
...
bool WorkspaceLoader::SaveLayout(const wxString &filename)
{
    ...
    rootnode->FirstChildElement("FileVersion")
        ->SetAttribute("major", WORKSPACE_LAYOUT_FILE_VERSION_MAJOR);
    rootnode->FirstChildElement("FileVersion")
        ->SetAttribute("minor", WORKSPACE_LAYOUT_FILE_VERSION_MINOR);
    ...
    {
        TiXmlElement *el =
            static_cast<TiXmlElement *>(rootnode->InsertEndChild(TiXmlElement("ActiveProject")));
        wxFileName wfname(filename);
    ...
    if (const cbWorkspace *wsp = pm->GetWorkspace())
    {
        ...
        {
            TiXmlElement *el = static_cast<TiXmlElement *>(
            ...
            _T("/environment/enable_editor_layout"), false))
    {
        ...
        el->SetAttribute(
            "layout", cbU2C(Manager::Get()->GetEditorManager()->GetNotebook()->SavePerspective()));
    }
    // else ?!
...
bool WorkspaceLoader::LoadLayout(const wxString &filename)
{
    ...
    {
        GetpMsg()->DebugLog(
            _T("Unable to load Code::Blocks workspace layout file: File is invalid."));
        return false;
    }
    ...
    if (!Manager::IsBatchBuild() && version)
    {
        ...
        if (major >= WORKSPACE_LAYOUT_FILE_VERSION_MAJOR
            && minor > WORKSPACE_LAYOUT_FILE_VERSION_MINOR)
        {
            GetpMsg()->DebugLog(F(_T("Workspace layout file version is > %d.%d. Trying to load..."),
                                  WORKSPACE_LAYOUT_FILE_VERSION_MAJOR,
                                  WORKSPACE_LAYOUT_FILE_VERSION_MINOR));
            AnnoyingDialog dlg(
                _("Workspace layout file format is newer/unknown"),
                F(_("This workspace layout file was saved with a newer version of Code::Blocks.\n"
                    ...
                    "In this case close the workspace, delete %s and reopen the workspace."),
                  filename.wx_str()),
        ...
        else
        {
            ...
            if (!msg.IsEmpty())
            {
                msg.Prepend(wxString::Format(
                    _("Workspace layout file format is older (%d.%d) than the current format "
                      "(%d.%d).\n"
                      ...
                      "List of changes:\n"),
                    major, minor, WORKSPACE_LAYOUT_FILE_VERSION_MAJOR,
                    WORKSPACE_LAYOUT_FILE_VERSION_MINOR));
                AnnoyingDialog dlg(_("Workspace layout file format changed"), msg,
            ...
            if (!warn_msg.IsEmpty())
            {
                ...
                AnnoyingDialog dlg(_("Workspace layout file upgrade warning"), warn_msg,
                                   wxART_WARNING, AnnoyingDialog::OK);
                dlg.ShowModal();
            }
        }
    ...
    if (TiXmlElement *el = root->FirstChildElement("ActiveProject"))
    {
        ...
        if (project)
        {
            ...
            Manager::Get()->GetLogManager()->DebugLog(
                F(_T("Project %s has been activated."), fname.GetFullPath().wx_str()));
        }
        else
            Manager::Get()->GetLogManager()->DebugLog(
                F(_T("Could not activate project: %s"), fname.GetFullPath().wx_str()));
    }
               ...
               _T("/environment/enable_editor_layout"), false)))
    {
        if (TiXmlElement *el = root->FirstChildElement("EditorTabsLayout"))
        {
            if (el->Attribute("layout"))
                Manager::Get()->GetEditorManager()->GetNotebook()->LoadPerspective(
                    cbC2U(el->Attribute("layout")));
        }
    }
