...
void EncodingDetector::Report(const char *aCharset)
{
    ...
    if (m_UseLog)
        Manager::Get()->GetLogManager()->DebugLog(
            F(_T("Mozilla universal detection engine detected '%s'."), m_MozillaResult.wx_str()));
...
bool EncodingDetector::DetectEncoding(const wxByte *buffer, size_t size, bool convert_to_wxstring)
{
    ...
    if (cfgMgr->ReadInt(_T("/default_encoding/use_option"), 0) == 1)
    {
        ...
        if (m_UseLog)
        {
            ...
            msg.Printf(_T("Warning: bypassing C::B's auto-detection!\n"
                       ...
                       wxFontMapper::Get()->GetEncodingDescription(m_Encoding).c_str(), m_Encoding);
    ...
    else
    {
        ...
        // Try our own detection for UTF-16 and UTF-32, the Mozilla-version does not work without
        // BOM
        ...
        else
        {
            ...
            else
            {
                ...
                if (m_UseLog)
                    Manager::Get()->GetLogManager()->DebugLog(
                        F(_T("Mozilla universal detection failed with %d."), res));
            }
    ...
    if (convert_to_wxstring && !ConvertToWxString(buffer, size) && m_UseLog)
        Manager::Get()->GetLogManager()->DebugLog(
            _T("Something seriously went wrong while converting file content to wxString!"));
...
bool EncodingDetector::DetectEncodingEx(const wxByte *buffer, size_t size)
{
    ...
    if (enc == wxFONTENCODING_DEFAULT && size >= 2)
    {
        if (size >= 4 && memcmp(buffer, "\x3C\x00\x00\x00", 4) == 0)
            enc = wxFONTENCODING_UTF32LE;
        else if (size >= 4 && memcmp(buffer, "\x00\x00\x00\x3C", 4) == 0)
            enc = wxFONTENCODING_UTF32BE;
        else if (memcmp(buffer, "\x3C\x00", 2) == 0)
            enc = wxFONTENCODING_UTF16LE;
        else if (memcmp(buffer, "\x00\x3C", 2) == 0)
            enc = wxFONTENCODING_UTF16BE;
    }
    ...
    if (enc == wxFONTENCODING_DEFAULT)
    {
        ...
        if (bad_utf_count == 0)
            enc = wxFONTENCODING_UTF8;
        else if (bad_utf32_count == 0 && nl_utf32le_count > size / 400)
            enc = wxFONTENCODING_UTF32LE;
        else if (bad_utf32_count == 0 && nl_utf32be_count > size / 400)
            enc = wxFONTENCODING_UTF32BE;
        else if (bad_utf16_count == 0 && nl_utf16le_count > size / 200)
            enc = wxFONTENCODING_UTF16LE;
        else if (bad_utf16_count == 0 && nl_utf16be_count > size / 200)
            enc = wxFONTENCODING_UTF16BE;
...
/// Convert the char buffer to wxString and if there are any null-terminating characters at the end
/// - remove them.
...
bool EncodingDetector::ConvertToWxString(const wxByte *buffer, size_t size)
{
    ...
    // if possible use the special conversion-routines, they are much faster than wxCSCov (at least
    // on linux)
    ...
    else
    {
            ...
            && conv.Convert((const char *)buffer, tmp))
        {
            wideBuff = tmp;
            outlen =
                size + 4 - m_BOMSizeInBytes; // should be correct, because Convert has returned true
            if (m_UseLog && outlen > 0)
            {
                logmsg.Printf(_T("Conversion succeeded using wxEncodingConverter "
                                 "(buffer size = %lu, converted size = %lu."),
                              static_cast<unsigned long>(size), static_cast<unsigned long>(outlen));
                logmgr->DebugLog(logmsg);
            }
        }
        else
        {
            ...
            {
                wideBuff =
                    csconv.cMB2WC((const char *)buffer, size + 4 - m_BOMSizeInBytes, &outlen);
                if (m_UseLog && outlen > 0)
                {
                    logmsg.Printf(_T("Conversion succeeded using wxCSConv "
                                     "(buffer size = %lu, converted size = %lu."),
                                  static_cast<unsigned long>(size),
                                  static_cast<unsigned long>(outlen));
                    logmgr->DebugLog(logmsg);
                }
            }
        }
    ...
    if (m_UseLog)
    {
        logmsg.Printf(_T("Encoding conversion using settings has failed!\n"
                      ...
                      wxFontMapper::Get()->GetEncodingDescription(m_Encoding).wx_str(), m_Encoding);
        logmgr->DebugLog(logmsg);
    }
