...
template<>
MacrosManager *Mgr<MacrosManager>::instance = nullptr;
template<>
bool Mgr<MacrosManager>::isShutdown = false;
...
void MacrosManager::RecalcVars(cbProject *project, EditorBase *editor, ProjectBuildTarget *target)
{
             ...
             || (UnixFilename(m_ProjectWxFileName.GetFullName()) != m_ProjectFilename))
    {
        ...
        for (int i = 0; i < project->GetBuildTargetsCount(); ++i)
        {
            ...
            m_Macros[title + _T("_OUTPUT_BASENAME")] =
                wxFileName(it_target->GetOutputFilename()).GetName();
    ...
    temp_str.Printf(_T("%d"), (m_ActiveEditorLine > 0)
                                  ? m_ActiveEditorLine - 1
                                  : -1); // zero based, but keep value for "invalid"
    ...
    temp_str.Printf(_T("%d"), (m_ActiveEditorColumn > 0)
                                  ? m_ActiveEditorColumn - 1
                                  : -1); // zero based, but keep value for "invalid"
    ...
    m_Macros[_T("GET_LOCAL_DATA_DIR")] =
        ((const wxStandardPaths &)wxStandardPaths::Get()).GetLocalDataDir();
    // Unix: ~   Windows: C:\Documents and Settings\username\Documents
    m_Macros[_T("GET_DOCUMENTS_DIR")] =
        ((const wxStandardPaths &)wxStandardPaths::Get()).GetDocumentsDir();
    // Unix: /etc   Windows: C:\Documents and Settings\All Users\Application Data
    m_Macros[_T("GET_CONFIG_DIR")] =
        ((const wxStandardPaths &)wxStandardPaths::Get()).GetConfigDir();
    // Unix: ~   Windows: C:\Documents and Settings\username\Application Data\appname
    m_Macros[_T("GET_USER_CONFIG_DIR")] =
        ((const wxStandardPaths &)wxStandardPaths::Get()).GetUserConfigDir();
    // Unix: ~/.appname   Windows: C:\Documents and Settings\username\Application Data
    m_Macros[_T("GET_USER_DATA_DIR")] =
        ((const wxStandardPaths &)wxStandardPaths::Get()).GetUserDataDir();
    // Unix: ~/.appname   Windows: C:\Documents and Settings\username\Local Settings\Application
    // Data\appname
    m_Macros[_T("GET_USER_LOCAL_DATA_DIR")] =
        ((const wxStandardPaths &)wxStandardPaths::Get()).GetUserLocalDataDir();
...
void MacrosManager::ReplaceMacros(wxString &buffer, ProjectBuildTarget *target, bool subrequest)
{
    ...
    cbProject *project = target ? target->GetParentProject()
    ...
    if (project != m_LastProject || target != m_LastTarget
        || (editor && (editor->GetFilename() != m_ActiveEditorFilename))
    ...
    if (buffer.find(_T("$if")) != wxString::npos)
    {
        while (m_RE_If.Matches(buffer))
        {
            ...
            replace = EvalCondition(m_RE_If.GetMatch(buffer, 1), m_RE_If.GetMatch(buffer, 3),
                                    m_RE_If.GetMatch(buffer, 5), target);
    ...
    while (m_RE_Script.Matches(buffer))
    {
        ...
        replace = Manager::Get()->GetScriptingManager()->LoadBufferRedirectOutput(
            m_RE_Script.GetMatch(buffer, 2));
        buffer.Replace(search, replace, false);
    }
    ...
    while ((index = buffer.Index(toNativePath)) != wxNOT_FOUND)
    {
        ...
        wxString content =
            buffer.Mid(index + toNativePath.Length(), end - index - toNativePath.Length());
    ...
    while ((index = buffer.Index(toUnixPath)) != wxNOT_FOUND)
    {
        ...
        wxString content =
            buffer.Mid(index + toUnixPath.Length(), end - index - toUnixPath.Length());
        ReplaceMacros(content, target, true);
        buffer.Replace(buffer.Mid(index, end - index + 1), UnixFilename(content, wxPATH_UNIX),
                       false);
    }
    ...
    while ((index = buffer.Index(toWindowsPath)) != wxNOT_FOUND)
    {
        ...
        wxString content =
            buffer.Mid(index + toWindowsPath.Length(), end - index - toWindowsPath.Length());
        ReplaceMacros(content, target, true);
        buffer.Replace(buffer.Mid(index, end - index + 1), UnixFilename(content, wxPATH_WIN),
                       false);
    }
    ...
    while (m_RE_RemoveQuotes.Matches(buffer))
    {
        ...
        {
            replace =
                content.Mid(1, content.Len() - 2); // with first and last char (the quotes) removed
            buffer.Replace(search, replace, false);
        }
        else
...
wxString MacrosManager::EvalCondition(const wxString &in_cond, const wxString &true_clause,
                                      const wxString &false_clause, ProjectBuildTarget *target)
{
