...
EditToolDlg::EditToolDlg(wxWindow *parent, cbTool *tool) : m_Tool(tool)
{
    if (!tool)
    ...
    XRCCTRL(*this, "rbLaunchOptions", wxRadioBox)
        ->SetSelection(static_cast<int>(m_Tool->GetLaunchOption()));
} // end of constructor
...
void EditToolDlg::OnBrowseCommand(cb_unused wxCommandEvent &event)
{
    ...
    wxString filename =
        wxFileSelector(_("Select executable"), file.GetPath(wxPATH_GET_VOLUME), file.GetFullName());
    if (!filename.IsEmpty())
...
{
    const wxString dir = ChooseDirectory(this, _("Select working directory"),
                                         XRCCTRL(*this, "txtDir", wxTextCtrl)->GetValue());
    if (!dir.IsEmpty())
...
void EditToolDlg::EndModal(int retCode)
{
    if (retCode == wxID_OK)
    {
        ...
        m_Tool->SetLaunchOption(static_cast<cbTool::eLaunchOption>(
            XRCCTRL(*this, "rbLaunchOptions", wxRadioBox)->GetSelection()));
    }
