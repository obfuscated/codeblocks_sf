...
EditorLexerLoader::EditorLexerLoader(EditorColourSet *target) : m_pTarget(target)
{
    // ctor
}
...
void EditorLexerLoader::DoStyles(HighlightLanguage language, TiXmlElement *node)
{
    ...
    while (style)
    {
        if (style->Attribute("name") && style->Attribute("index"))
        {
            ...
            wxString index =
                wxString(style->Attribute("index"), wxConvUTF8); // comma-separated indices
            ...
            bool italics =
                style->Attribute("italics") ? atol(style->Attribute("italics")) != 0 : false;
            bool underlined =
                style->Attribute("underlined") ? atol(style->Attribute("underlined")) != 0 : false;
            ...
            for (size_t i = 0; i < indices.GetCount(); ++i)
            {
                ...
                //                LOGSTREAM << _("Adding style: ") << name << _T("(") << value <<
                //                _T(")\n");
                m_pTarget->AddOption(language, name, value, fgcolour, bgcolour, bold, italics,
                                     underlined, value >= 0);
            }
        }
...
void EditorLexerLoader::DoSingleKeywordNode(HighlightLanguage language, TiXmlElement *node,
...
void EditorLexerLoader::DoSampleCode(HighlightLanguage language, TiXmlElement *node)
{
    ...
    if (code.IsEmpty())
        return;
    int breakLine =
        sample->Attribute("breakpoint_line") ? atol(sample->Attribute("breakpoint_line")) : -1;
...
void EditorLexerLoader::DoLangAttributes(HighlightLanguage language, TiXmlElement *node)
{
    ...
    bool CaseSensitive = attribs->Attribute("CaseSensitive")
                             ? atol(attribs->Attribute("CaseSensitive")) != 0
                             : false;
    ...
    token.doxygenStreamCommentStart =
        wxString(attribs->Attribute("DoxygenStreamCommentStart"), wxConvUTF8);
    token.doxygenStreamCommentEnd =
        wxString(attribs->Attribute("DoxygenStreamCommentEnd"), wxConvUTF8);
    ...
    std::set<int> CommentLexerStyles, CharacterLexerStyles, StringLexerStyles,
        PreprocessorLexerStyles;
    bool hasLexerStylesSet = false;
    hasLexerStylesSet |=
        DoLangAttributesLexerStyles(attribs, "LexerCommentStyles", CommentLexerStyles);
    hasLexerStylesSet |=
        DoLangAttributesLexerStyles(attribs, "LexerCharacterStyles", CharacterLexerStyles);
    hasLexerStylesSet |=
        DoLangAttributesLexerStyles(attribs, "LexerStringStyles", StringLexerStyles);
    hasLexerStylesSet |=
        DoLangAttributesLexerStyles(attribs, "LexerPreprocessorStyles", PreprocessorLexerStyles);
