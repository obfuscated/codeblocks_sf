...
//    }
    ...
    /* If returned compiler is null, it can be assumed
     * that the user has opted to ignore this target.
     */
    if (compiler == nullptr)
    {
        delete f;
    ...
    if (tool.generatedFiles.GetCount())
    {
        ...
        for (size_t i = 0; i < tool.generatedFiles.GetCount(); ++i)
        {
            ...
            tmps.Replace(_T("$file_basename"), f->file.GetName());
            tmps.Replace(_T("$file"), f->file.GetFullName());
            Manager::Get()->GetMacrosManager()->ReplaceMacros(tmps);

            ProjectFile* pfile = AddFile(-1, UnixFilename(tmps));
            if (!pfile)
