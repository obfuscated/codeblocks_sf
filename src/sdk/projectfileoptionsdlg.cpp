...
inline void AnalyseLine(const CommentToken &language, wxString line, bool &comment, bool &code,
                        bool &multi_line_comment)
{
    ...
    if (multi_line_comment)
    {
        ...
        if (first_multi_line_comment_end > -1)
        {
            ...
            if (first_multi_line_comment_end + language.streamCommentEnd.Length() < line.Length())
                AnalyseLine(
                    language,
                    line.Mid(first_multi_line_comment_end + language.streamCommentEnd.Length()),
    ...
    else if (!multi_line_comment)
    {
        ...
        if ((first_single_line_comment > -1)
            && ((first_multi_line_comment_begin == -1)
                || ((first_multi_line_comment_begin > -1)
                    && (first_single_line_comment < first_multi_line_comment_begin))))
        {
            comment = true;
        ...
        else if (first_multi_line_comment_begin > -1)
        {
            ...
            if (first_multi_line_comment_begin > 0)
                code = true;
            if (first_multi_line_comment_begin + language.streamCommentStart.Length()
                < line.Length())
                AnalyseLine(
                    language,
                    line.Mid(first_multi_line_comment_begin + language.streamCommentStart.Length()),
                    comment, code, multi_line_comment);
        }
        else
        {
...
inline void CountLines(wxFileName filename, const CommentToken &language, long int &code_lines,
                       long int &codecomments_lines, long int &comment_lines, long int &empty_lines,
...
ProjectFileOptionsDlg::ProjectFileOptionsDlg(wxWindow *parent, ProjectFile *pf)
  : m_ProjectFile(pf), m_FileNameStr(wxEmptyString), m_FileName(), m_LastBuildStageCompilerSel(-1)
{
    wxXmlResource::Get()->LoadObject(this, parent, _T("dlgProjectFileOptions"),
    ...
    if (pf)
    {
        ...
        XRCCTRL(*this, "txtProject", wxTextCtrl)
            ->SetValue(prj ? (prj->GetTitle() + _T("\n") + prj->GetFilename()) : _T("-"));
        XRCCTRL(*this, "txtProjectBasePath", wxTextCtrl)
            ->SetValue(prj ? prj->GetCommonTopLevelPath() : _T("-"));
        XRCCTRL(*this, "txtAbsName", wxTextCtrl)->SetValue(m_FileNameStr);
        XRCCTRL(*this, "txtRelName", wxTextCtrl)->SetValue(pf->relativeFilename);

        SetTitle(_("Properties of ") + wxString(_("\"")) + pf->relativeFilename
                 + wxString(_("\"")));
    }
...
ProjectFileOptionsDlg::ProjectFileOptionsDlg(wxWindow *parent, const wxString &fileName)
  : m_ProjectFile(nullptr), m_FileNameStr(fileName), m_FileName(), m_LastBuildStageCompilerSel(-1)
{
    wxXmlResource::Get()->LoadObject(this, parent, _T("dlgProjectFileOptions"),
                                     _T("wxScrollingDialog"));
    XRCCTRL(*this, "wxID_OK", wxButton)->SetDefault();
    ...
    if (event.IsChecked())
    {
        ...
        if (!ToggleFileReadOnly(true))
            Manager::Get()->GetLogManager()->DebugLog(
                F(_T("Unable to set file '%s' read-only (probably missing access rights)."),
                  m_FileNameStr.wx_str()));
    }
    else
    {
        // make writeable
        if (!ToggleFileReadOnly(false))
            Manager::Get()->GetLogManager()->DebugLog(
                F(_T("Unable to set file '%s' writeable (probably missing access rights)."),
                  m_FileNameStr.wx_str()));
    }
...
void ProjectFileOptionsDlg::FillGeneralProperties()
{
    ...
    if (lang != HL_NONE)
    {
        ...
        CountLines(m_FileName, colour_set->GetCommentToken(lang), code_lines, codecomments_lines,
                   comment_lines, empty_lines, total_lines);
        XRCCTRL(*this, "staticTotalLines", wxStaticText)
            ->SetLabel(wxString::Format(_T("%ld"), total_lines));
        XRCCTRL(*this, "staticEmptyLines", wxStaticText)
            ->SetLabel(wxString::Format(_T("%ld"), empty_lines));
        XRCCTRL(*this, "staticActualLines", wxStaticText)
            ->SetLabel(wxString::Format(_T("%ld"), code_lines + codecomments_lines));
        XRCCTRL(*this, "staticCommentLines", wxStaticText)
            ->SetLabel(wxString::Format(_T("%ld"), comment_lines));
    ...
    if (file.IsOpened())
    {
        ...
        XRCCTRL(*this, "staticFileSize", wxStaticText)
            ->SetLabel(wxString::Format(_("%ld Bytes"), length));
        XRCCTRL(*this, "staticFileSize", wxStaticText)->GetContainingSizer()->Layout();
        file.Close();
    }
    XRCCTRL(*this, "chkReadOnly", wxCheckBox)->SetValue(!m_FileName.IsFileWritable());
    wxDateTime modTime = m_FileName.GetModificationTime();
    XRCCTRL(*this, "staticDateTimeStamp", wxStaticText)
        ->SetLabel(wxString::Format(
            _("%02hd/%02hd/%d %02hd:%02hd:%02hd"), modTime.GetDay(), modTime.GetMonth() + 1,
            modTime.GetYear(), modTime.GetHour(), // seems I have to add 1 for the month ?
            modTime.GetMinute(),
            modTime.GetSecond())); // because the return value of GetMonth() is an enum
}
...
void ProjectFileOptionsDlg::FillCompilers()
{
    ...
    m_LastBuildStageCompilerSel =
        CompilerFactory::GetCompilerIndex(m_ProjectFile->GetParentProject()->GetCompilerID());
...
void ProjectFileOptionsDlg::UpdateBuildCommand()
{
    ...
    XRCCTRL(*this, "chkBuildStage", wxCheckBox)
        ->SetValue(m_ProjectFile->customBuild[compiler->GetID()].useCustomBuildCommand);
    XRCCTRL(*this, "txtBuildStage", wxTextCtrl)
        ->SetValue(m_ProjectFile->customBuild[compiler->GetID()].buildCommand);
}
...
void ProjectFileOptionsDlg::SaveBuildCommandSelection()
{
    ...
    if (compiler)
    {
        m_ProjectFile->customBuild[compiler->GetID()].useCustomBuildCommand =
            XRCCTRL(*this, "chkBuildStage", wxCheckBox)->GetValue();
        m_ProjectFile->customBuild[compiler->GetID()].buildCommand =
            XRCCTRL(*this, "txtBuildStage", wxTextCtrl)->GetValue();
    }
}
...
bool ProjectFileOptionsDlg::ToggleFileReadOnly(bool setReadOnly)
{
    ...
    {
        Manager::Get()->GetLogManager()->DebugLog(
            F(_T("Error calling chmod : errno = %d."), errno));
        return false; // chmod error
    }
