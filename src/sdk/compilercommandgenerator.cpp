...
void CompilerCommandGenerator::Init(cbProject *project)
{
    ...
    for (int i = 0; i < project->GetBuildTargetsCount(); ++i)
    {
        ...
        if (!compiler || target->GetTargetType() == ttCommandsOnly
            || !target->SupportsCurrentPlatform())
        {
...
void CompilerCommandGenerator::GenerateCommandLine(wxString &macro, ProjectBuildTarget *target,
                                                   ProjectFile *pf, const wxString &file,
                                                   ...
                                                   const wxString &deps)
{
    ...
    Manager::Get()->GetLogManager()->DebugLog(
        F(_T("GenerateCommandLine[0]: macro='%s', file='%s', object='%s', flat_object='%s', ")
          _T("deps='%s'."),
    ...
    {
        Manager::Get()->GetLogManager()->DebugLog(
            F(_T("GenerateCommandLine: Required compiler executable (%s) not found! Check the ")
              _T("toolchain settings."),
              file.wx_str()));
        macro.Clear();
        return;
    }
...
#ifdef command_line_generation
    Manager::Get()->GetLogManager()->DebugLog(
        F(_T("GenerateCommandLine[1]: tmpIncludes='%s', tmpResIncludes='%s'."),
...
#ifdef command_line_generation
    Manager::Get()->GetLogManager()->DebugLog(
        F(_T("GenerateCommandLine[2]: tmpFile='%s', tmpDeps='%s', tmpObject='%s', ")
          _T("tmpFlatObject='%s',\ntmpFname.GetName='%s', tmpFname.GetPath='%s', ")
          _T("tmpFname.GetExt='%s'."),
          tmpFile.wx_str(), tmpDeps.wx_str(), tmpObject.wx_str(), tmpFlatObject.wx_str(),
          tmpFname.GetName().wx_str(), tmpFname.GetPath().wx_str(), tmpFname.GetExt().wx_str()));
#endif
...
#ifdef command_line_generation
    Manager::Get()->GetLogManager()->DebugLog(
        F(_T("GenerateCommandLine[3]: tmpFile='%s', tmpDeps='%s', tmpObject='%s', ")
          _T("tmpFlatObject='%s'."),
          tmpFile.wx_str(), tmpDeps.wx_str(), tmpObject.wx_str(), tmpFlatObject.wx_str()));
    Manager::Get()->GetLogManager()->DebugLog(
        F(_T("GenerateCommandLine[4]: macro='%s'."), macro.wx_str()));
#endif
...
#ifdef command_line_generation
    Manager::Get()->GetLogManager()->DebugLog(
        F(_T("GenerateCommandLine[5]: macro='%s'."), macro.wx_str()));
#endif
...
#ifdef command_line_generation
    Manager::Get()->GetLogManager()->DebugLog(
        F(_T("GenerateCommandLine[6]: macro='%s', file='%s', object='%s', flat_object='%s', ")
          _T("deps='%s'."),
          macro.wx_str(), file.wx_str(), object.wx_str(), flat_object.wx_str(), deps.wx_str()));

    Manager::Get()->GetLogManager()->DebugLog(F(
        _T("GenerateCommandLine[7]: m_Output[target]='%s, m_StaticOutput[target]='%s', ")
        _T("m_DefOutput[target]='%s'."),
        m_Output[target].wx_str(), m_StaticOutput[target].wx_str(), m_DefOutput[target].wx_str()));
#endif

    if (target
        && ((target->GetTargetType() == ttStaticLib) || (target->GetTargetType() == ttDynamicLib)))
    {
...
#ifdef command_line_generation
    Manager::Get()->GetLogManager()->DebugLog(
        F(_T("GenerateCommandLine[8]: macro='%s', file='%s', object='%s', flat_object='%s', ")
          _T("deps='%s'."),
          macro.wx_str(), file.wx_str(), object.wx_str(), flat_object.wx_str(), deps.wx_str()));
#endif
...
#ifdef command_line_generation
    Manager::Get()->GetLogManager()->DebugLog(
        F(_T("GenerateCommandLine[9]: macro='%s'."), macro.wx_str()));
#endif
}
                                              ...
                                              const wxString &funcName)
{
    ...
    for (size_t i = 0; i < scripts.GetCount(); ++i)
    {
        ...
        script_nomacro = wxFileName(script_nomacro).IsAbsolute()
                             ? script_nomacro
                             : project->GetBasePath() + wxFILE_SEP_PATH + script_nomacro;
...
#ifdef command_line_generation
    Manager::Get()->GetLogManager()->DebugLog(
        F(_T("SetupOutputFilenames[0]: m_Output[target]='%s'."), m_Output[target].wx_str()));
#endif
    ...
    switch (target->GetTargetType())
    {
        case ttDynamicLib:
        {
            ...
            if ((PrefixPolicy == tgfpPlatformDefault)
                ...
                importLibraryFileName.SetName(compiler->GetSwitches().libPrefix
                                              + importLibraryFileName.GetName());
            ...
            if (ExtensionPolicy == tgfpPlatformDefault)
            {
                ...
                if (!current_ext.IsSameAs(requested_ext, false))
                    importLibraryFileName.SetFullName(importLibraryFileName.GetFullName()
                                                      + wxFILE_SEP_EXT + requested_ext);
            }
...
#ifdef command_line_generation
            Manager::Get()->GetLogManager()->DebugLog(
                F(_T("SetupOutputFilenames[1]: m_StaticOutput[target]='%s'."),
                  m_StaticOutput[target].wx_str()));
#endif
            ...
            if ((PrefixPolicy == tgfpPlatformDefault)
                ...
                definitionFileFileName.SetName(compiler->GetSwitches().libPrefix
                                               + definitionFileFileName.GetName());
            ...
            if (ExtensionPolicy == tgfpPlatformDefault)
            {
                ...
                if (!current_ext.IsSameAs(requested_ext, false))
                    definitionFileFileName.SetFullName(definitionFileFileName.GetFullName()
                                                       + wxFILE_SEP_EXT + requested_ext);
            }
...
#ifdef command_line_generation
            Manager::Get()->GetLogManager()->DebugLog(
                F(_T("SetupOutputFilenames[2]: result='%s'."), result.wx_str()));
#endif
        }
        break;
        ...
        default:
        {
            ...
            // if $(VARIABLE) expands to /bar/ then wxFileName will still consider
            // $(VARIABLE)libfoo.a a filename, not a fully qualified path, so we will prepend lib to
            // /bar/libfoo.a incorrectly NOTE (thomas#1#): A better solution might be to use a
            // regex, but finding an universal regex might not be easy...
...
#ifdef command_line_generation
            Manager::Get()->GetLogManager()->DebugLog(
                F(_T("SetupOutputFilenames[3]: m_StaticOutput[target]='%s'."),
                  m_StaticOutput[target].wx_str()));
#endif
            ...
            fname.SetExt(_T("def"));
            result = UnixFilename(fname.GetFullPath());
            QuoteStringIfNeeded(result); // NOTE (thomas#1#): Do we really need to call
                                         // QuoteStringIfNeeded that often? ReplaceMacros already
                                         // does it, and we do it twice again without ever possibly
                                         // adding whitespace
            FixPathSeparators(compiler, result);

#ifdef command_line_generation
            Manager::Get()->GetLogManager()->DebugLog(
                F(_T("SetupOutputFilenames[4]: result='%s'."), result.wx_str()));
#endif
        }
        break;
    }
...
#ifdef command_line_generation
    Manager::Get()->GetLogManager()->DebugLog(
        F(_T("SetupOutputFilenames[5]: result='%s'."), result.wx_str()));
#endif
    return result;
}
                                                              ...
                                                              ProjectBuildTarget *target)
{
    ...
    if (target)
    {
        ...
        if (Manager::Get()
                ->GetConfigManager(_T("compiler"))
                ->ReadBool(_T("/include_prj_cwd"), false))
            searchDirs.Add(target->GetParentProject()->GetBasePath());
        if (Manager::Get()
                ->GetConfigManager(_T("compiler"))
                ->ReadBool(_T("/include_file_cwd"), false))
            searchDirs.Add(_T("."));
        m_CompilerSearchDirs.insert(m_CompilerSearchDirs.end(), std::make_pair(target, searchDirs));

        // decide order
        result =
            GetOrderedOptions(target, ortIncludeDirs, target->GetParentProject()->GetIncludeDirs(),
    ...
    for (unsigned int x = 0; x < result.GetCount(); ++x)
    {
        ...
        if (tmp.Trim().IsEmpty())
            Manager::Get()->GetLogManager()->DebugLogError(
                _T("Warning: Compiler include folder evaluates to empty value."));
    }
                                                                ...
                                                                ProjectBuildTarget *target)
{
    ...
    if (target)
    {
        ...
        // decide order
        result = GetOrderedOptions(target, ortLibDirs, target->GetParentProject()->GetLibDirs(),
                                   target->GetLibDirs());
    }
    // compiler dirs
    ...
    for (unsigned int x = 0; x < result.GetCount(); ++x)
    {
        ...
        if (tmp.Trim().IsEmpty())
            Manager::Get()->GetLogManager()->DebugLogError(
                _T("Warning: Linker include folder evaluates to empty value."));
    }
                                                                      ...
                                                                      ProjectBuildTarget *target)
{
    ...
    if (target) // decide order
        result = GetOrderedOptions(target, ortResDirs,
    ...
    for (unsigned int x = 0; x < result.GetCount(); ++x)
    {
        ...
        if (tmp.Trim().IsEmpty())
            Manager::Get()->GetLogManager()->DebugLogError(
                _T("Warning: Resource compiler include folder evaluates to empty value."));
    }

    return result;
}

wxString CompilerCommandGenerator::MakeOptString(const wxArrayString &arr, const wxString &opt,
                                                 wxChar separator)
{
    wxString result;
    bool subseq(false);
                                                      ...
                                                      ProjectBuildTarget *target)
{
    if (compiler->GetSwitches().linkerNeedsPathResolved)
        return wxString();
    return MakeOptString(GetOrderedLibrariesDirs(compiler, target), compiler->GetSwitches().libDirs,
                                                        ...
                                                        ProjectBuildTarget *target)
{
    ...
    if (target)
    {
        ...
        // project options
        wxString pstr =
            GetStringFromArray(target->GetParentProject()->GetCompilerOptions(), _T(' '))
            << _T(" ");
...
wxString CompilerCommandGenerator::FixupLinkLibraries(Compiler *compiler, const wxString &lib)
{
    ...
    QuoteStringIfNeeded(result);
    FixPathSeparators(compiler, result);
    // run replacements on libs only if no slashes in name (which means it's a relative or absolute
    // path)
    if (result.Find('/') == -1 && result.Find('\\') == -1)
    {
        // 'result' prefix
        bool hadLibPrefix = false;
        if (!compiler->GetSwitches().linkerNeedsLibPrefix && !libPrefix.IsEmpty()
        ...
        // extension
        if (!compiler->GetSwitches().linkerNeedsLibExtension && result.Length() > libExt.Length()
                                                      ...
                                                      ProjectBuildTarget *target)
{
    ...
    if (target) // decide order
        libs = GetOrderedOptions(target, ortLinkerOptions,
                                 target->GetParentProject()->GetLinkLibs(), target->GetLinkLibs());
...
wxString
    CompilerCommandGenerator::SetupResourceCompilerOptions(cb_unused Compiler *compiler,
                                                           cb_unused ProjectBuildTarget *target)
{
    wxString result;
    ...
    if (target)
    {
        ...
        // project options
        wxString pstr =
            GetStringFromArray(target->GetParentProject()->GetResourceCompilerOptions(), _T(' '));
                                                          ...
                                                          const wxString &inc_switch)
{
    ...
    for (size_t x = 0; x < inc_dirs.GetCount(); ++x)
    {
        ...
        if (platform::windows && compiler->GetSwitches().Use83Paths)
        {
            wxFileName fn(UnquoteStringIfNeeded(inc_dir),
                          wxEmptyString); // explicitly assign as path
            if (fn.DirExists())
