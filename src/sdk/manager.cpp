...
static wxString GetCodeblocksEventName(wxEventType type)
{
    ...
    else if (type == cbEVT_APP_START_SHUTDOWN)
        name = _T("cbEVT_APP_START_SHUTDOWN");
    else if (type == cbEVT_APP_ACTIVATED)
        name = _T("cbEVT_APP_ACTIVATED");
    else if (type == cbEVT_APP_DEACTIVATED)
        name = _T("cbEVT_APP_DEACTIVATED");
    else if (type == cbEVT_PLUGIN_ATTACHED)
        name = _T("cbEVT_PLUGIN_ATTACHED");
    else if (type == cbEVT_PLUGIN_RELEASED)
        name = _T("cbEVT_PLUGIN_RELEASED");
    else if (type == cbEVT_PLUGIN_INSTALLED)
        name = _T("cbEVT_PLUGIN_INSTALLED");
    else if (type == cbEVT_PLUGIN_UNINSTALLED)
        name = _T("cbEVT_PLUGIN_UNINSTALLED");
    else if (type == cbEVT_PLUGIN_LOADING_COMPLETE)
        name = _T("cbEVT_PLUGIN_LOADING_COMPLETE");
    else if (type == cbEVT_EDITOR_CLOSE)
        name = _T("cbEVT_EDITOR_CLOSE");
    else if (type == cbEVT_EDITOR_OPEN)
        name = _T("cbEVT_EDITOR_OPEN");
    else if (type == cbEVT_EDITOR_SWITCHED)
        name = _T("cbEVT_EDITOR_SWITCHED");
    else if (type == cbEVT_EDITOR_ACTIVATED)
        name = _T("cbEVT_EDITOR_ACTIVATED");
    else if (type == cbEVT_EDITOR_DEACTIVATED)
        name = _T("cbEVT_EDITOR_DEACTIVATED");
    else if (type == cbEVT_EDITOR_BEFORE_SAVE)
        name = _T("cbEVT_EDITOR_BEFORE_SAVE");
    else if (type == cbEVT_EDITOR_SAVE)
        name = _T("cbEVT_EDITOR_SAVE");
    else if (type == cbEVT_EDITOR_MODIFIED)
        name = _T("cbEVT_EDITOR_MODIFIED");
    else if (type == cbEVT_EDITOR_TOOLTIP)
        name = _T("cbEVT_EDITOR_TOOLTIP");
    else if (type == cbEVT_EDITOR_TOOLTIP_CANCEL)
        name = _T("cbEVT_EDITOR_TOOLTIP_CANCEL");
    else if (type == cbEVT_EDITOR_SPLIT)
        name = _T("cbEVT_EDITOR_SPLIT");
    else if (type == cbEVT_EDITOR_UNSPLIT)
        name = _T("cbEVT_EDITOR_UNSPLIT");
    else if (type == cbEVT_EDITOR_UPDATE_UI)
        name = _T("cbEVT_EDITOR_UPDATE_UI");
    else if (type == cbEVT_PROJECT_NEW)
        name = _T("cbEVT_PROJECT_NEW");
    else if (type == cbEVT_PROJECT_CLOSE)
        name = _T("cbEVT_PROJECT_CLOSE");
    else if (type == cbEVT_PROJECT_OPEN)
        name = _T("cbEVT_PROJECT_OPEN");
    else if (type == cbEVT_PROJECT_SAVE)
        name = _T("cbEVT_PROJECT_SAVE");
    else if (type == cbEVT_PROJECT_ACTIVATE)
        name = _T("cbEVT_PROJECT_ACTIVATE");
    else if (type == cbEVT_PROJECT_BEGIN_ADD_FILES)
        name = _T("cbEVT_PROJECT_BEGIN_ADD_FILES");
    else if (type == cbEVT_PROJECT_END_ADD_FILES)
        name = _T("cbEVT_PROJECT_END_ADD_FILES");
    else if (type == cbEVT_PROJECT_BEGIN_REMOVE_FILES)
        name = _T("cbEVT_PROJECT_BEGIN_REMOVE_FILES");
    else if (type == cbEVT_PROJECT_END_REMOVE_FILES)
        name = _T("cbEVT_PROJECT_END_REMOVE_FILES");
    else if (type == cbEVT_PROJECT_FILE_ADDED)
        name = _T("cbEVT_PROJECT_FILE_ADDED");
    else if (type == cbEVT_PROJECT_FILE_REMOVED)
        name = _T("cbEVT_PROJECT_FILE_REMOVED");
    else if (type == cbEVT_PROJECT_POPUP_MENU)
        name = _T("cbEVT_PROJECT_POPUP_MENU");
    else if (type == cbEVT_PROJECT_TARGETS_MODIFIED)
        name = _T("cbEVT_PROJECT_TARGETS_MODIFIED");
    else if (type == cbEVT_PROJECT_RENAMED)
        name = _T("cbEVT_PROJECT_RENAMED");
    else if (type == cbEVT_WORKSPACE_CHANGED)
        name = _T("cbEVT_WORKSPACE_CHANGED");
    else if (type == cbEVT_BUILDTARGET_ADDED)
        name = _T("cbEVT_BUILDTARGET_ADDED");
    else if (type == cbEVT_BUILDTARGET_REMOVED)
        name = _T("cbEVT_BUILDTARGET_REMOVED");
    else if (type == cbEVT_BUILDTARGET_RENAMED)
        name = _T("cbEVT_BUILDTARGET_RENAMED");
    else if (type == cbEVT_BUILDTARGET_SELECTED)
        name = _T("cbEVT_BUILDTARGET_SELECTED");
    else if (type == cbEVT_PIPEDPROCESS_STDOUT)
        name = _T("cbEVT_PIPEDPROCESS_STDOUT");
    else if (type == cbEVT_PIPEDPROCESS_STDERR)
        name = _T("cbEVT_PIPEDPROCESS_STDERR");
    else if (type == cbEVT_PIPEDPROCESS_TERMINATED)
        name = _T("cbEVT_PIPEDPROCESS_TERMINATED");
    else if (type == cbEVT_THREADTASK_STARTED)
        name = _T("cbEVT_THREADTASK_STARTED");
    else if (type == cbEVT_THREADTASK_ENDED)
        name = _T("cbEVT_THREADTASK_ENDED");
    else if (type == cbEVT_THREADTASK_ALLDONE)
        name = _T("cbEVT_THREADTASK_ALLDONE");
    else if (type == cbEVT_MENUBAR_CREATE_BEGIN)
        name = _T("cbEVT_MENUBAR_CREATE_BEGIN");
    else if (type == cbEVT_MENUBAR_CREATE_END)
        name = _T("cbEVT_MENUBAR_CREATE_END");
    else if (type == cbEVT_COMPILER_STARTED)
        name = _T("cbEVT_COMPILER_STARTED");
    else if (type == cbEVT_COMPILER_FINISHED)
        name = _T("cbEVT_COMPILER_FINISHED");
    else if (type == cbEVT_COMPILER_SET_BUILD_OPTIONS)
        name = _T("cbEVT_COMPILER_SET_BUILD_OPTIONS");
    else if (type == cbEVT_CLEAN_PROJECT_STARTED)
        name = _T("cbEVT_CLEAN_PROJECT_STARTED");
    else if (type == cbEVT_CLEAN_WORKSPACE_STARTED)
        name = _T("cbEVT_CLEAN_WORKSPACE_STARTED");
    else if (type == cbEVT_DEBUGGER_STARTED)
        name = _T("cbEVT_DEBUGGER_STARTED");
    else if (type == cbEVT_DEBUGGER_PAUSED)
        name = _T("cbEVT_DEBUGGER_PAUSED");
    else if (type == cbEVT_DEBUGGER_FINISHED)
...
Manager::~Manager()
{
    ...
    for (DockEventSinksMap::iterator mit = m_DockEventSinks.begin(); mit != m_DockEventSinks.end();
         ++mit)
    {
        while (mit->second.size())
        {
    ...
    for (LayoutEventSinksMap::iterator mit = m_LayoutEventSinks.begin();
         mit != m_LayoutEventSinks.end(); ++mit)
    {
        while (mit->second.size())
        {
            delete (*(mit->second.begin()));
    ...
    for (LogEventSinksMap::iterator mit = m_LogEventSinks.begin(); mit != m_LogEventSinks.end();
         ++mit)
    {
        while (mit->second.size())
        {
        ...
        for (EventSinksArray::iterator it = mit->second.begin(); it != mit->second.end(); ++it)
        {
            ...
            if (sw.Time() > 10) // only print a handler run longer than 10 ms
            {
                ...
                Manager::Get()->GetLogManager()->DebugLog(
                    F(_("%s take %ld ms"), msg.wx_str(), sw.Time()));
            }
#endif // PPRCESS_EVENT_PERFORMANCE_MEASURE
        }
    }
...
bool Manager::ProcessEvent(CodeBlocksLayoutEvent &event)
{
    ...
    {
        for (LayoutEventSinksArray::iterator it = mit->second.begin(); it != mit->second.end();
             ++it)
            (*it)->Call(event);
    }
    return true;
...
{
    bool smallToolBar = Manager::Get()->GetConfigManager(_T("app"))->ReadBool(
        _T("/environment/toolbar_size"), true);

    wxSize size =
        smallToolBar ? wxSize(16, 16) : (platform::macosx ? wxSize(32, 32) : wxSize(22, 22));
    wxToolBar *toolbar =
        new wxToolBar(GetAppFrame(), -1, wxDefaultPosition, size, wxTB_FLAT | wxTB_NODIVIDER);
...
bool Manager::LoadResource(const wxString &file)
{
    ...
    {
        Get()->GetLogManager()->LogError(_("Manager failed to access XRC resource '") + resourceFile
                                         + _("'."));
        return false;
    }
    ...
    try
    {
        ...
        if (!wxXmlResource::Get()->Load(memoryFile))
            Get()->GetLogManager()->LogError(_("Manager failed to load XRC resource '")
                                             + resourceFile + _("'."));
        delete[] buf;
        return true;
    }
    ...
    {
        delete[] buf;
        Get()->GetLogManager()->LogError(_("Manager hardly failed to load XRC resource '")
                                         + resourceFile + _("'."));
        return false;
    }
}
