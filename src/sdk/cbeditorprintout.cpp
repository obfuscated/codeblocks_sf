...
cbEditorPrintout::cbEditorPrintout(const wxString &title, cbStyledTextCtrl *control,
                                   bool selectionOnly)
  : wxPrintout(title), m_TextControl(control)
{
    // ctor
    m_SelStart = 0;
    ...
    if (dc)
    {
        ...
        else
        {
            Manager::Get()->GetLogManager()->DebugLog(
                F(_T("OnPrintPage ERROR: page = %d , maxpage = %d"), page, maxpage));
            return false;
        }

        // Manager::Get()->GetLogManager()->DebugLog(_T("OnPrintPage: page %d , m_printed %d"),
        // page, m_printed);
        m_printed =
            m_TextControl->FormatRange(1, m_printed, m_SelEnd, dc, dc, m_printRect, m_pageRect);
        return true;
    }
    return false;
}
...
void cbEditorPrintout::GetPageInfo(int *minPage, int *maxPage, int *selPageFrom, int *selPageTo)
{
    ...
    *selPageTo = 0;
    ...
    // We have to divide through 254 instead of 25.4, because GetSize() of paperDB returns tenth of
    // millimeters
    ...
    if (dc)
    {
        ...
        {
            // Manager::Get()->GetLogManager()->DebugLog(_T("CountPages: PageCount %d , m_printed
            // %d"), m_pPageSelStart->GetCount(), m_printed);
            m_printed =
                m_TextControl->FormatRange(0, m_printed, m_SelEnd, dc, dc, m_printRect, m_pageRect);
...
bool cbEditorPrintout::OnBeginDocument(int startPage, int endPage)
{
    ...
    {
        Manager::Get()->GetLogManager()->DebugLog(
            F(_T("OnBeginDocument ERROR: startPage %d , endPage %d , maxpage %d "), startPage,
              endPage, maxpage));
        return false;
    }
    return result;
