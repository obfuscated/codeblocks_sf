...
After downloading the sources, compiling them and installing the plugin to \codeblocks just go to \menu{Project,Autoversioning} menu. A pop up window like this will appear:
...
This lets you enter every change made to the project to generate a \file{ChangesLog.txt} file.
...
This dialog is accessible from the menu \menu{Project,Changes Log}. Also if checked Show changes editor when incrementing version on the changes log settings, the window will open to let you enter the list of changes after a modification to the project sources or an incrementation event. Below is an example screenshot:
